...
async function testDifyIntegration() {
  ...
  try {
    ...
    const { data: links, error: linkError } = await supabase
      ...
      .eq('status', 'active')
      .or(
        `title.ilike.%iPhone%,title.ilike.%电池%,description.ilike.%iPhone%,description.ilike.%电池%`
      )
      .order('priority', { ascending: false })
      .limit(3);
    ...
    for (const link of links) {
      ...
      scrapedResults.push({
        ...
        url: link.url,
        content:
          link.description || `这是关于${link.title}的详细维修指南内容...`,
    ...
    for (let i = 0; i < 5; i++) {
      concurrentTests.push(
        supabase.from('unified_link_library').select('count').limit(1)
      );
    }
    ...
    console.log(
      `✅ 并发测试完成，5个并发请求耗时: ${stressEnd - stressStart}ms`
    );
