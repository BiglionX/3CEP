...
const diagnostics = {
  ...
  testPaths: ['/', '/profile', '/profile/dashboard', '/unified-center-test'],
  requiredFiles: [
...
async function runHttpTests() {
  ...
  for (const testPath of diagnostics.testPaths) {
    try {
      ...
      if (result.success) {
        console.log(
          `   ${status} ${testPath} - ${result.statusCode} ${result.statusMessage}`
        );
      } else {
...
function checkServerProcess() {
  ...
  return new Promise(resolve => {
    exec('tasklist /fi "imagename eq node.exe"', (error, stdout) => {
      ...
      const nodeProcesses = lines.filter(
        line => line.includes('node.exe') && line.includes('next')
      );
...
async function runProcessCheck() {
  try {
    ...
    if (result.success) {
      console.log(
        `   ✅ 检测到 ${result.processes} 个Node.js进程正在运行Next.js`
      );
