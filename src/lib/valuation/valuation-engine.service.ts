...
class SimpleMLClient implements MLModelClient {
  ...
  async checkModelAvailability() {
    try {
      // 这里应该检查实际的ML服务
      // 暂时模拟可用状?      this.isModelAvailable = true;
    } catch (error) {
...
/**
 * 设备估值引擎服? * 基于eReuse RdeviceScore核心思想实现的基础估值算? */

export interface ValuationResult {
  baseValue: number; // 基础价?  componentScore: number; // 部件评分
  conditionMultiplier: number; // 成色乘数
  finalValue: number; // 最终估?  breakdown: ValuationBreakdown; // 详细分解
...
export interface ValuationBreakdown {
  ...
  // ML模型相关信息
  mlPrediction?: number; // ML模型预测?  ruleBasedValue?: number; // 规则引擎估?  strategyUsed?: string; // 使用的策?}
...
export class ValuationEngineService {
  ...
  /**
   * 计算混合估值（ML模型 + 规则引擎?   * @param deviceProfile 设备档案
   * @param condition 设备成色状?   * @param marketPrice 当前市场价格
    ...
    try {
      ...
      if (useMLModel && this.mlClient.isAvailable()) {
        try {
          ...
          // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`🤖 ML模型预测: ¥${mlPrediction.toFixed(2)}`);
        } catch (error) {
          console.warn('ML模型预测失败，回退到规则引?', error.message);
        }
      }

      // 规则引擎估?      ruleBasedValue = await this.calculateBaseValue(
        deviceProfile,
        condition,
        marketPrice
      );
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`📏 规则引擎估? ¥${ruleBasedValue.finalValue.toFixed(2)}`);
      ...
      if (mlPrediction !== null) {
        // 使用加权平均（ML 70%, 规则 30%�?        finalValue = mlPrediction * 0.7 + ruleBasedValue.finalValue * 0.3;
        strategyUsed = 'hybrid_ml_weighted';
      } else {
        // 仅使用规则引?        finalValue = ruleBasedValue.finalValue;
        strategyUsed = 'rule_based_only';
      }

      // 构建混合估值结?      const hybridResult: ValuationResult = {
        ...ruleBasedValue,
      ...
      console.error('混合估值计算失?', error);
      // 完全回退到规则引?      return this.calculateBaseValue(deviceProfile, condition, marketPrice);
    }
  }
  ...
  private extractDeviceFeatures(
    ...
    const features = {
      ...
      category_encoded: this.encodeCategory(
        deviceProfile.productCategory || ''
      ),
      ...
      // 成色状态（如果提供?      screen_condition: condition
        ? this.encodeScreenCondition(condition.screen)
        : 0.8,
      battery_health: condition
        ? this.encodeBatteryHealth(condition.battery)
        : 0.8,
      body_condition: condition
        ? this.encodeBodyCondition(condition.body)
        : 0.8,
      functionality_score: condition
        ? this.encodeFunctionality(condition.functionality)
        : 0.9,
    ...
    features.storage_age_ratio =
      features.storage_gb / (features.age_months || 1);
    features.repair_frequency =
      features.total_repair_count / (features.age_months || 1);

    return features;
  }

  /**
   * 计算设备基础估?   * @param deviceProfile 设备档案
   * @param condition 设备成色状?   * @param marketPrice 当前市场价格（可选）
   * @returns 估值结?   */
  public async calculateBaseValue(
    ...
    try {
      ...
      // 4. 计算成色乘数
      const conditionMultiplier = this.calculateConditionMultiplier(
        condition || this.getDefaultCondition()
      );

      // 5. 计算各项调整因子
      const brandMultiplier = this.getBrandMultiplier(
        deviceProfile.brandName || ''
      );
      ...
      // 6. 综合计算最终价?      const baseValue = originalPrice - depreciation;
      ...
      throw new Error(
        `估值计算失? ${error instanceof Error ? error.message : '未知错误'}`
      );
    }
  }
   ...
   * 获取设备原始价格
   */
  private async getOriginalPrice(
    deviceProfile: DeviceProfile,
    marketPrice?: number
  ): Promise<number> {
    // 如果提供了市场价格，直接使用
    ...
    // 否则根据型号和年份估算价?    const basePrice = this.estimateBasePrice(deviceProfile);
    return basePrice;
  }
  ...
  private calculateDepreciation(
    ...
    // 计算设备年龄（月?    const manufactureDate = deviceProfile.manufacturingDate
    ...
    // 应用最大年限限?    const effectiveYears = Math.min(yearsOld, maxYears);

    // 计算折旧?    const depreciationRate = Math.min(effectiveYears * annualRate, 0.9); // 最多贬?0%
  ...
  private calculateComponentScore(deviceProfile: DeviceProfile): number {
    ...
    if (specs.processor || specs.cpu) {
      ...
      if (
        cpuSpec.includes('m3') ||
        cpuSpec.includes('a17') ||
        cpuSpec.includes('snapdragon 8')
      ) {
        cpuScore = 1.0;
      } else if (
        cpuSpec.includes('m2') ||
        cpuSpec.includes('a16') ||
        cpuSpec.includes('snapdragon 7')
      ) {
        cpuScore = 0.8;
      } else if (
        cpuSpec.includes('m1') ||
        cpuSpec.includes('a15') ||
        cpuSpec.includes('snapdragon 6')
      ) {
        cpuScore = 0.6;
      }
  ...
  private calculateAgeInMonths(manufacturingDate?: Date): number {
    if (!manufacturingDate) return 12; // 默认1�?
  ...
  private calculateConditionMultiplier(condition: DeviceCondition): number {
    ...
    const functionMultiplier =
      multipliers.functionality[condition.functionality];

    // 加权平均计算
    return (
      screenMultiplier * 0.3 +
  ...
  private getCategoryMultiplier(category: string): number {
    ...
    if (categoryLower.includes('笔记?) || categoryLower.includes('laptop')) {
  ...
  private getAgeMultiplier(deviceProfile: DeviceProfile): number {
    ...
    if (monthsOld <= brackets.recent.max_months)
      return brackets.recent.multiplier;
    if (monthsOld <= brackets.moderate.max_months)
      return brackets.moderate.multiplier;
    if (monthsOld <= brackets.older.max_months)
      return brackets.older.multiplier;
