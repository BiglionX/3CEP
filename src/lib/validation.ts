/**
 * 企业服务表单验证和错误处理系? * 提供统一的表单验证、错误处理和用户体验
 */
...
export const commonValidations = {
  ...
  // 描述文本验证
  description: z.string().max(1000, '描述不能超过1000个字?),
};
...
export const enterpriseValidations = {
  ...
  // 智能体配置验?  agentConfig: z.object({
    name: z
      .string()
      .min(2, '智能体名称至少需?个字?)
      .max(30, '智能体名称不能超?0个字?),
    description: commonValidations.description,
    capabilities: z.array(z.string()).min(1, '至少需要选择一个能?),
    model: z.string().min(1, '请选择AI模型'),
    temperature: z.number().min(0, '温度值不能小?').max(2, '温度值不能大?'),
    maxTokens: z
      .number()
      .int()
      .min(100, '最大token数不能少?00')
      .max(4096, '最大token数不能超?096'),
  }),

  // 企业用户注册验证
  enterpriseRegister: z
    .object({
      ...
      industry: z.string().min(1, '请选择所属行?),
  ...
  // 供应商信息验?  supplierInfo: z.object({
    ...
    email: commonValidations.email,
    phone: commonValidations.phone,
    address: z.string().min(5, '地址信息至少需?个字?),
    businessScope: z.array(z.string()).min(1, '至少需要选择一个业务范?),
...
// 错误处理?export class ErrorHandler {
  // 处理表单验证错误
  static handleFormErrors(error: any): FormError[] {
    ...
    if (error instanceof z.ZodError) {
      ...
      // 处理API返回的字段错?      Object.entries(error.fieldErrors as Record<string, string[]>).forEach(
        ([field, messages]) => {
  ...
  static handleApiError(error: any): ApiError {
    if (error.response) {
      // 服务器返回错误响?      const { status, data } = error.response;
  ...
  // 处理并返回错误信?  static processError(error: any, context?: string): ApiError {
...
// 自定义验证钩?export function useValidation() {
  // 验证表单数据
...
// 导出常用的验证组?export const validationSchemas = {
  ...commonValidations,
