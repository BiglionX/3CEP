...
export class ImageProcessorService {
  ...
  async processImageRequest(rawRequest: RawProcurementRequest): Promise<{
    ...
    try {
      ...
      // 2. 商品检测（如果有相应API）
      if (this.computerVisionApiKey) {
        processingSteps.push("执行商品检测");
        ...
        processingSteps.push(
          `商品检测完成，识别到${detectedProducts.length}个商品`
        );
      }

      // 3. 内容分析和清理
      processingSteps.push("内容分析和清理");
  ...
  private async performOcr(imageUrl: string): Promise<OcrResult> {
    // 这里可以集成多种OCR服务，如Google Vision API、百度OCR、腾讯OCR等
    // 目前提供基础实现，可以根据实际情况替换
  ...
  private async callExternalOcrService(imageUrl: string): Promise<OcrResult> {
    ...
    try {
      ...
      const words = textAnnotations.slice(1).map((word: any) => ({
        ...
        boundingBox:
          word.boundingPoly?.vertices?.flatMap((v: any) => [v.x, v.y]) || [],
      ...
      console.warn("外部OCR服务调用失败，使用模拟数据:", error);
      return await this.simulateOcrResult(imageUrl);
    }
  }
  ...
  private cleanExtractedText(text: string): string {
    ...
    cleaned = cleaned
      ...
      .replace(/[\u201C\u201D]/g, '"') // 替换弯双引号
      .replace(/\u2013|\u2014/g, "-") // 替换破折号
      .replace(/[^\x20-\x7E\u4e00-\u9fff]/g, ""); // 移除非ASCII和非中文字符
