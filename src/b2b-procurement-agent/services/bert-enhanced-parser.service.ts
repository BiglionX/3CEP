/**
 * BERT增强版采购需求解析服? * 基于混合架构：BERT + 规则引擎
 */
...
export class BertEnhancedParserService {
  ...
  private mockBertPrediction(text: string): BertPrediction {
    ...
    // 模拟实体识别
    const quantityMatches = text.match(/(\d+)\s*(?:台|件|个|套|箱|�?/g);
    if (quantityMatches) {
    ...
    const productPatterns = [
      {
        pattern: /(?:电脑|计算机|pc|笔记本|台式?/gi,
        label: 'PRODUCT',
        value: 'computer',
      },
      {
        pattern: /(?:手机|智能手机|移动电话)/gi,
        label: 'PRODUCT',
        value: 'mobile',
      },
      { pattern: /(?:服务器|主机)/gi, label: 'PRODUCT', value: 'server' },
      {
        pattern: /(?:显示器|屏幕|液晶?/gi,
        label: 'PRODUCT',
        value: 'display',
      },
    ];
  ...
  async parseDemand(
    ...
    try {
      // 1. BERT模型预测
      const bertResult = await this.bertModel.predict(
      ...
      // 错误回退到纯规则引擎
      console.warn('BERT解析失败，回退到规则引?', error);
  ...
  private fusePredictions(
    ...
    if (bertResult.confidence > 0.9) {
      ...
      return this.buildFromBertResult(
        bertResult,
        rawRequest,
      ...
      // 低置信度，主要采用规则引擎结?      return this.enhanceRuleResult(ruleResult, bertResult);
    }
  }
  ...
  private buildFromBertResult(
    ...
    // 提取产品信息
    const productEntities = bertResult.entities.filter(
      e => e.label === 'PRODUCT'
    );
    const quantityEntities = bertResult.entities.filter(
      e => e.label === 'QUANTITY'
    );

    // 简单匹配策?    productEntities.forEach((product, index) => {
  ...
  private hybridMerge(
    ...
    // 优先采用BERT识别的产品信?    const bertProducts = bertResult.entities.filter(e => e.label === 'PRODUCT');
    if (bertProducts.length > 0) {
      ...
      return {
        ...
        aiConfidence: Math.round(
          (bertResult.confidence * 0.7 +
            (ruleResult.aiConfidence / 100) * 0.3) *
            100
        ),
      };
    }
  ...
  private enhanceRuleResult(
    ...
    // 使用规则结果，但可以利用BERT的一些辅助信?    return {
      ...ruleResult,
      aiConfidence: Math.min(100, ruleResult.aiConfidence + 5), // 小幅提升置信?    };
  }
  ...
  private extractUrgency(text: string): UrgencyLevel {
    ...
    if (
      lowerText.includes('紧?) ||
      lowerText.includes('急需') ||
      lowerText.includes('马上')
    ) {
      return UrgencyLevel.URGENT;
    } else if (lowerText.includes('尽快') || lowerText.includes('早日')) {
      return UrgencyLevel.HIGH;
    } else if (lowerText.includes('一?) || lowerText.includes('普?)) {
  ...
  private extractBudget(text: string): ParsedProcurementRequest['budgetRange'] {
    const budgetMatch = text.match(
      /(?:预算|价格|费用|成本)\s*[为是]\s*(\d+(?:\.\d+)?)\s*(?:元|万元|rmb|usd|\$)/i
    );
    if (budgetMatch) {
