...
import { quotationConsumptionService, QUOTATION_CONSUMPTION_CONFIG } from '../../fcx-system';
...
export class SmartProcurementAgentService {
  ...
  async createSmartQuotationFromHistory(
    ...
    try {
      ...
      // 2. 应用订单修改
      const modifiedItems = this.applyModifications(order.items, modifications || []);
      
      // 3. 分析历史供应商模式
      const historicalAnalysis = await this.analyzeHistoricalSupplierPatterns(userId);
      ...
      // 5. 计算FCX消耗
      const fcxEstimate = await quotationConsumptionService.estimateQuotationCost(userId, {
  ...
  async executeSmartQuotation(
    ...
    try {
      // 1. 检查FCX余额和限额
      const fcxCheck = await quotationConsumptionService.checkDailyLimits(userId);
      ...
      const consumptionParams = {
        supplierCount: quotationPlan.supplierGroups.length,
        itemCount: quotationPlan.supplierGroups.reduce((sum, group) => sum + group.items.length, 0),
        isUrgent: false,
        useCustomTemplate: false,
        enableAutoFollowUp: true,
        isBatchOperation: quotationPlan.supplierGroups.length > 3
      ...
      const consumptionDetail = quotationConsumptionService.calculateConsumption(consumptionParams);
      
      // 3. 扣除FCX
      const deductionResult = await quotationConsumptionService.deductFcxForQuotation(
        userId,
        consumptionDetail,
        quotationPlan.id,
        '智能询价消耗'
      ...
      // 4. 执行询价（模拟）
      const executedRequests = await this.simulateQuotationExecution(quotationPlan);
  ...
  async autoCompleteQuotation(
    ...
    try {
      // 1. 获取询价请求和报价
      const quotationRequest = await this.getQuotationRequestById(quotationRequestId);
      if (!quotationRequest) {
        return {
          success: false,
      ...
      // 2. 智能分析和推荐
      const analysisResult = this.analyzeQuotesIntelligently(quotes, quotationRequest.items);
      
      // 3. 生成完成状态
      const completionStatus = quotes.length >= 3 ? 'completed' : 'partial';
  ...
  async modifyAndResendOrder(
    ...
    try {
      ...
      // 2. 应用修改
      const modifiedItems = this.applyModifications(originalOrder.items, modifications);
      ...
      // 4. 基于新订单创建智能询价计划
      const result = await this.createSmartQuotationFromHistory(
        newOrder.id,
        userId,
        true,
        modifications
  ...
  private applyModifications(items: any[], modifications: OrderModification[]): any[] {
    ...
    for (const mod of modifications) {
      switch (mod.type) {
        ...
        case 'remove':
          modifiedItems = modifiedItems.filter(item => item.product_id !== mod.productId);
          break;
          
        case 'modify':
          const itemIndex = modifiedItems.findIndex(item => item.product_id === mod.productId);
  ...
  private async analyzeHistoricalSupplierPatterns(userId: string): Promise<HistoricalAnalysis> {
    // 模拟历史数据分析
    // 在实际项目中这里会查询数据库分析历史供应商模式
  ...
  private generateSmartQuotationPlan(
    ...
    const totalEstimatedCost = supplierGroups.reduce((sum, group) => sum + group.estimatedCost, 0);
  ...
  private async simulateQuotationExecution(plan: SmartQuotationPlan): Promise<Array<any>> {
    // 模拟询价执行过程
    ...
    for (const group of plan.supplierGroups) {
      ...
      console.log(`向供应商 ${group.supplierName} 发送询价，ID: ${requestId}`);
    }
  ...
  private analyzeQuotesIntelligently(quotes: any[], items: any[]) {
    ...
    const bestQuotes = sortedQuotes.slice(0, 3); // 选择最优的3个报价
    
    const recommendations = [
      `推荐选择供应商 ${bestQuotes[0]?.supplier_name || '未知'}，价格最优`,
      `建议对比前3名供应商的服务条款`,
      `注意检查交货时间和质量保证`
    ];
    
    const estimatedSavings = sortedQuotes.length > 1 
      ? sortedQuotes[0].total_amount - sortedQuotes[sortedQuotes.length - 1].total_amount
      : 0;
