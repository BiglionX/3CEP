...
export class DemandParserService {
  ...
  async parseDemand(rawRequest: RawProcurementRequest): Promise<ParsedProcurementRequest> {
    ...
    try {
      // 1. 文本预处理
      const cleanedText = this.preprocessText(rawRequest.rawDescription);
      ...
      // 4. 结构化转换
      const structuredDemand = this.structureDemand(entities, relationships, rawRequest);
      
      // 5. 质量评估
      const confidence = this.calculateConfidence(structuredDemand, rawRequest.rawDescription);
      ...
      // 6. 记录处理日志
      await this.logProcessing(rawRequest.id, 'parsing', rawRequest.rawDescription, parsedRequest, Date.now() - startTime, true);
      ...
      throw new Error(`需求解析失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  private async extractEntities(text: string): Promise<Entity[]> {
    ...
    // 数量识别
    const quantityPattern = /(\d+(?:\.\d+)?)\s*(?:个|件|台|套|批|箱|盒|包|米|千克|公斤|吨)/g;
    let match;
    ...
    const productPatterns = [
      { pattern: /(?:电脑|计算机|pc|笔记本|台式机)/gi, category: 'computer' },
      { pattern: /(?:手机|智能手机|移动电话)/gi, category: 'mobile' },
      { pattern: /(?:服务器|主机)/gi, category: 'server' },
      { pattern: /(?:显示器|屏幕|液晶屏)/gi, category: 'display' },
      { pattern: /(?:打印机|打印设备)/gi, category: 'printer' },
      { pattern: /(?:网络设备|路由器|交换机)/gi, category: 'network' },
    ...
    // 预算识别
    const budgetPattern = /(?:预算|价格|费用|成本)\s*[为是]\s*(\d+(?:\.\d+)?)\s*(?:元|万元|rmb|usd|\$)/gi;
    ...
    // 时间期限识别
    const timePattern = /(?:截止|最晚|限期|交付)\s*(?:时间|日期)?\s*[为是]\s*([\d月日天周]+)/gi;
  ...
  private extractRelationships(entities: Entity[]): Relationship[] {
    ...
    quantities.forEach(quantity => {
      const closestProduct = products.reduce((closest, product) => {
        ...
        const distance2 = closest ? Math.abs(quantity.position - closest.position) : Infinity;
  ...
  private structureDemand(
    ...
    // 根据关系构建采购物品
    const quantityProductRelations = relationships.filter(r => r.type === 'quantity_product');
    ...
    // 确定紧急程度
    const urgencyEntity = entities.find(e => e.type === 'urgency');
    ...
    const budgetRange = budgetEntities.length > 0 ? {
  ...
  private calculateConfidence(
    ...
    // 紧急程度识别加分
    if (parsedRequest.urgency && parsedRequest.urgency !== UrgencyLevel.MEDIUM) {
      confidence += 10;
    }
    ...
    // 特殊要求识别加分
    if (parsedRequest.specialRequirements && parsedRequest.specialRequirements.length > 0) {
      confidence += 10;
    }
