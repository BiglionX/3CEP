...
export interface CrowdfundingPledge {
  ...
  notes: string | null;
  // FCX支付相关字段
  fcx_payment_amount: number | null; // 使用FCX支付的金额
  fcx_deduction_amount: number | null; // FCX抵扣金额
...
export class CrowdfundingPledgeService {
  ...
  static async createPledge(pledgeData: PledgeCreateData, userId: string) {
    try {
      ...
      if (project.status !== "active") {
        throw new Error("项目不在进行中状态");
      }

      // 检查项目是否已结束
      if (new Date(project.end_date) < new Date()) {
        throw new Error("项目已结束");
      }
      ...
      // 更新项目当前金额（临时方案，实际应该在支付成功后更新）
      await this.updateProjectAmount(pledgeData.project_id, pledgeData.amount);
  ...
  // 获取项目的所有支持记录
  static async getProjectPledges(
    projectId: string,
    page: number = 1,
    limit: number = 50
  ) {
  ...
  static async checkUserSupported(userId: string, projectId: string) {
    try {
      ...
      console.error("检查用户支持状态失败:", error);
      return false;
    }
  }
