...
export async function getPendingReviews(): Promise<{ data: Review[]; error: string | null }> {
  try {
    ...
    return { data: [], error: '获取待审核文档失败' };
  }
}

export async function getReviewById(id: string): Promise<{ data: Review | null; error: string | null }> {
  try {
    const { data, error } = await supabase
      .from('document_reviews')
      .select(`
...
export async function assignReviewer(
  ...
  try {
    ...
    return { success: false, error: '分配审核员失败' };
  }
}

export async function getReviewStats(): Promise<{ data: ReviewStats | null; error: string | null }> {
  try {
    // 获取总体统计
...
export async function getReviewers(): Promise<{ data: Reviewer[]; error: string | null }> {
  try {
    const { data, error } = await supabase
      .from('reviewers')
      .select(`
