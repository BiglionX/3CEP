...
export class AIDiagnosisService {
  ...
  async diagnose(request: DiagnosisRequest): Promise<DiagnosisResult> {
    try {
      ...
      // 2. 基于知识库的初步匹配
      const knowledgeMatches = await this.searchKnowledgeBase(processedSymptoms, request.deviceId);
      
      // 3. 如果有高置信度匹配，直接返回
      if (knowledgeMatches.length > 0 && knowledgeMatches[0].confidence >= 0.8) {
   ...
   * 搜索本地知识库
   */
  private async searchKnowledgeBase(symptoms: string, deviceId: string): Promise<Array<{ fault: FaultKnowledge; confidence: number }>> {
    // 这里应该调用实际的知识库搜索服务
    // 暂时返回模拟数据
    const mockResults = [
      {
        fault: {
          ...
          name: '屏幕碎裂',
          category: '屏幕故障',
          commonSymptoms: ['屏幕有裂痕', '触摸不灵敏', '显示异常'],
          diagnosticQuestions: {
            question: '屏幕是否有明显裂痕？',
            options: [
   ...
   * 调用AI大模型进行诊断
   */
  private async callAIDiagnosis(request: DiagnosisRequest, knowledgeMatches: any[]): Promise<any> {
    const systemPrompt = `
...
${
  ...
  private formatKnowledgeBasedResult(match: any): DiagnosisResult {
    return {
      ...
      estimatedTime: match.fault.solutionSteps.reduce((sum: number, step: Step) => sum + step.estimatedDuration, 0),
      difficultyLevel: match.fault.difficultyLevel
    };
  }
   ...
   * 后处理AI结果
   */
  private postProcessResult(aiResult: any, knowledgeMatches: any[]): DiagnosisResult {
    // 确保必需字段存在
    ...
    if (result.confidence < 0.6 && knowledgeMatches.length > 0) {
      const knowledgeResult = this.formatKnowledgeBasedResult(knowledgeMatches[0]);
      result.confidence = Math.max(result.confidence, knowledgeResult.confidence * 0.8);
   ...
   * 获取故障建议
   */
  async getFaultSuggestions(deviceId: string, userInput: string): Promise<FaultKnowledge[]> {
   ...
   * 生成解决方案步骤
   */
  async generateSolutionSteps(faultId: string, deviceInfo: any): Promise<Step[]> {
