...
export class ConfidenceService {
  ...
  async assessConfidence(deviceModel: string): Promise<ConfidenceAssessment> {
    try {
      ...
      const { shouldFallback, fallbackReason } =
        this.shouldFallbackToRuleEngine(
          overallConfidence,
          sampleCountScore,
          freshnessScore
        );
  ...
  async getConfidenceReport(deviceModels: string[]): Promise<{
    ...
    const highConfidence = assessments.filter(
      a => a.assessment.overallConfidence > 0.8
    ).length;
    const mediumConfidence = assessments.filter(
      a =>
        a.assessment.overallConfidence >= 0.5 &&
        a.assessment.overallConfidence <= 0.8
    ).length;
    const lowConfidence = assessments.filter(
      a => a.assessment.overallConfidence < 0.5
    ).length;

    const totalConfidence = assessments.reduce(
      (sum, a) => sum + a.assessment.overallConfidence,
      0
    );
    const avgConfidence =
      assessments.length > 0 ? totalConfidence / assessments.length : 0;
