...
interface MarketWeightedConfig {
  ...
  minFreshnessThreshold: number; // 最低新鲜度阈值
  sampleCountThreshold: number;  // 最小样本量阈值
  weightingStrategy: 'linear' | 'exponential' | 'threshold'; // 加权策略
}
...
export class MarketWeightedEngineService {
  ...
  async calculateMarketWeightedValue(
    ...
    try {
      // 1. 获取折旧基线估值
      const baselineValuation = await depreciationEngineService.calculateBaselineValue(
        deviceProfile, 
        condition, 
        marketPrice
      );
      ...
      // 2. 获取市场参考价
      const marketReference = await this.getMarketReferencePrice(deviceProfile.productModel);
      ...
      // 回退到纯折旧估值
      return depreciationEngineService.calculateBaselineValue(deviceProfile, condition, marketPrice);
    }
  }
   ...
   * 获取市场参考价
   */
  private async getMarketReferencePrice(deviceModel: string): Promise<MarketReferencePrice> {
    try {
      // 获取最新的市场数据
      ...
      // 计算置信度
      const confidence = this.calculateMarketConfidence(bestData);
  ...
  private calculateMarketAdjustment(
    ...
    // 计算新鲜度系数
    const freshnessCoefficient = this.calculateFreshnessCoefficient(marketReference.daysOld);
    ...
    // 限制调整幅度，避免过度偏离
    const maxAdjustment = 0.3; // 最大±30%调整
    const clampedRatio = Math.max(1 - maxAdjustment, Math.min(1 + maxAdjustment, adjustmentRatio));
  ...
  private applyMarketWeighting(
    ...
    // 根据置信度确定市场权重
    const marketWeight = this.calculateMarketWeight(marketConfidence);
    const baselineWeight = 1 - marketWeight;
    
    // 加权计算
    const weightedValue = (baselineValue * baselineWeight) + 
  ...
  private calculateMarketConfidence(marketData: any): number {
    ...
    // 价格稳定性贡献 (0-0.3)
    const priceRange = (marketData.maxPrice - marketData.minPrice) / marketData.avgPrice;
    if (priceRange <= 0.2) confidence += 0.3; // 价格差异≤20%
  ...
  updateConfig(newConfig: Partial<MarketWeightedConfig>): void {
    ...
    console.log('⚙️ 市场加权引擎配置已更新:', this.config);
  }
  ...
  async analyzeMarketDataQuality(deviceModel: string): Promise<{
    ...
    try {
      ...
      let recommendedAction: 'use_market' | 'fallback_to_baseline' | 'insufficient_data' = 'use_market';
