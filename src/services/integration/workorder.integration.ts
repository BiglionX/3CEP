...
export class WorkOrderIntegration {
   ...
   * @param userId 操作用户ID
   */
  async recordRepairFromWorkOrder(workOrder: any, userId: string): Promise<void> {
    try {
      // 构建维修事件数据
      const repairData = {
      ...
      console.log(`工单 ${workOrder.work_order_number} 的维修事件已记录到设备生命周期`);
    } catch (error) {
   ...
   * @param userId 操作用户ID
   */
  async recordPartReplacement(workOrder: any, partInfo: any, userId: string): Promise<void> {
    try {
      const partData = {
        qrcodeId: workOrder.device_qrcode_id,
      ...
      console.log(`配件更换事件已记录: ${partInfo.part_name}`);
    } catch (error) {
   ...
   * @param userId 操作用户ID
   */
  async batchProcessCompletedWorkOrders(workOrders: any[], userId: string): Promise<{
    successCount: number;
    failureCount: number;
    errors: string[];
  }> {
   ...
   * @param userId 操作用户ID
   */
  async createDeviceProfileFromWorkOrder(workOrder: any, userId: string): Promise<void> {
    try {
      // 这里可以根据工单信息创建设备档案
      // 实际实现需要根据具体的工单数据结构调整
      
      console.log(`从工单 ${workOrder.work_order_number} 创建设备档案`);
    } catch (error) {
      console.error('从工单创建设备档案失败:', error);
      throw error;
    }
  }
   ...
   * @param userId 操作用户ID
   */
  async syncWorkOrderStatusChange(workOrder: any, newStatus: string, userId: string): Promise<void> {
    try {
      // 根据工单状态变化记录相应的生命周期事件
      ...
      console.log(`工单状态变更已同步: ${workOrder.work_order_number} -> ${newStatus}`);
    } catch (error) {
      console.error('同步工单状态变更失败:', error);
      throw error;
    }
  }
  ...
  async getDeviceWorkOrderHistory(qrcodeId: string): Promise<any[]> {
    try {
      // 查询与该设备相关的生命周期事件中的工单信息
      const events = await this.lifecycleService.getDeviceLifecycleHistory(qrcodeId);
      
      // 过滤出包含工单信息的事件
      const workOrderEvents = events.filter(event => 
        event.metadata?.workOrderId || event.metadata?.workOrderNumber
      );

      return workOrderEvents.map(event => ({
        eventId: event.id,
        workOrderId: event.metadata?.workOrderId,
        workOrderNumber: event.metadata?.workOrderNumber,
        eventType: event.eventType,
        eventSubtype: event.eventSubtype,
        timestamp: event.eventTimestamp,
        technician: event.eventData?.technician,
        cost: event.eventData?.cost,
        notes: event.notes
      }));
    } catch (error) {
