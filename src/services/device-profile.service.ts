...
export class DeviceProfileService {
   ...
   * @returns 创建的设备档案
   */
  async createDeviceProfile(profileData: CreateDeviceProfileParams): Promise<DeviceProfile> {
    try {
      // 计算保修到期日期
      ...
      const { data, error } = await this.supabase
        ...
        .insert({
          ...
          current_status: profileData.currentStatus || DeviceStatus.MANUFACTURED,
  ...
  async updateDeviceProfile(
    qrcodeId: string, 
    updates: Partial<Omit<DeviceProfile, 'id' | 'qrcodeId' | 'createdAt' | 'updatedAt'>>
  ...
  async updateDeviceStatus(
    ...
    try {
      ...
      // 如果是激活状态，记录首次激活时间
      if (status === DeviceStatus.ACTIVATED && !metadata?.firstActivatedAt) {
      ...
      if (error) throw new Error(`更新设备状态失败: ${error.message}`);
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        remainingDays: isUnderWarranty 
          ...
          : 0,
        message: isUnderWarranty ? '在保' : '已过保',
  ...
  async getDeviceStatistics(): Promise<any> {
    try {
      ...
      if (profileError) throw new Error(`获取档案数据失败: ${profileError.message}`);
      ...
      if (repairError) throw new Error(`获取维修统计失败: ${repairError.message}`);

      const totalDevices = repairStats?.length || 0;
      const avgRepairCount = totalDevices > 0 
        ? repairStats!.reduce((sum, item) => sum + (item.total_repair_count || 0), 0) / totalDevices
        : 0;
      
      const avgPartReplacementCount = totalDevices > 0
        ? repairStats!.reduce((sum, item) => sum + (item.total_part_replacement_count || 0), 0) / totalDevices
        : 0;
      ...
      if (transferError) throw new Error(`获取转移统计失败: ${transferError.message}`);

      const totalTransfers = transferData?.reduce(
        (sum, item) => sum + (item.total_transfer_count || 0), 0
      ) || 0;
      ...
      return {
        ...
        averagePartReplacementCount: parseFloat(avgPartReplacementCount.toFixed(2)),
        totalTransfers
      };
    } catch (error) {
   ...
   * @returns 匹配的设备档案列表
   */
  async searchDeviceProfiles(searchTerm: string, limit: number = 20): Promise<DeviceProfile[]> {
    try {
      const { data, error } = await this.supabase
        .from('device_profiles')
        .select('*')
        .or(`product_model.ilike.%${searchTerm}%,brand_name.ilike.%${searchTerm}%,serial_number.ilike.%${searchTerm}%`)
        .limit(limit)
   ...
   * @returns 即将过保的设备列表
   */
  async getExpiringWarrantyDevices(days: number = 30): Promise<DeviceProfile[]> {
    try {
  ...
  private mapToDeviceProfile(record: any): DeviceProfile {
    return {
      ...
      manufacturingDate: record.manufacturing_date ? new Date(record.manufacturing_date) : undefined,
      firstActivatedAt: record.first_activated_at ? new Date(record.first_activated_at) : undefined,
      warrantyStartDate: record.warranty_start_date ? new Date(record.warranty_start_date) : undefined,
      warrantyExpiry: record.warranty_expiry ? new Date(record.warranty_expiry) : undefined,
      warrantyPeriod: record.warranty_period,
      currentStatus: record.current_status as DeviceStatus,
      lastEventAt: record.last_event_at ? new Date(record.last_event_at) : undefined,
