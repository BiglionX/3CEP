...
// 诊断历史缓存（生产环境应该使用Redis�?const diagnosisSessions = new Map<string, any[]>();
...
1. 分析用户描述的症?2. 结合该产品的常见故障模式
3. 提供可能的原因分?4. 给出具体的解决方案或建议
...
请用简洁明了的语言回答，并在适当时候询问更多细节。`;
  ...
  static async processDiagnosis(
    ...
    try {
      ...
      // 模拟AI响应（生产环境应替换为真实的AI API调用?      const aiResponse = this.generateMockResponse(userMessage, productInfo);
  ...
  static generateSuggestions(
    ...
    if (productInfo) {
      suggestions.push(
        `查看${productInfo.brand} ${productInfo.model}官方故障排除指南`
      );
      suggestions.push('联系官方客服获取专业支持');
    }

    // 返回最?个建?    return [...new Set(suggestions)].slice(0, 5);
  }
  ...
  // 格式化持续时?  static formatDuration(ms: number): string {
...
1. 电池电量不足 - 请尝试充?0分钟后再开?2. 系统卡死 - 可以尝试强制重启（按住电源键10秒）
3. 硬件故障 - 建议联系售后检?
请问您已经尝试过哪些操作系统`;
    }
...
您是在什么情况下发现没有声音的？`;
    ...
    if (lowerMessage.includes('发热') || lowerMessage.includes('�?)) {
