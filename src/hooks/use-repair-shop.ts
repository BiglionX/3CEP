...
export const useWorkOrdersPaginated = (
  ...
  return useQuery({
    queryKey: [...QUERY_KEYS.WORK_ORDERS, 'paginated', filters, initialPage, pageSize],
    queryFn: async () => {
...
export const useWorkOrdersInfinite = (
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }: any: { pageParam: number }) => {
      const response = await repairShopApi.getWorkOrders(filters, { 
        page: pageParam, 
        pageSize 
      });
    ...
    getNextPageParam: (lastPage: { data: WorkOrder[]; nextPage: number; hasMore: boolean }) => 
...
export const useCreateWorkOrder = () => {
  ...
  return useMutation({
    mutationFn: (workOrder: Omit<WorkOrder, 'id' | 'createdAt' | 'updatedAt' | 'orderNumber' | 'status' | 'priority' | 'price' | 'partsCost' | 'laborCost' | 'totalPrice' | 'paymentStatus'>) => 
...
export const useShopStatistics = (shopId: string) => {
  return useQuery({
    ...
    enabled: !!shopId,
    select: (response) => response.data,
    staleTime: 30 * 1000, // 统计数据相对稳定?0秒缓?
  })
}
...
export const useDeviceTypes = () => {
  return useQuery({
    ...
    select: (response) => response.data,
    staleTime: 30 * 60 * 1000, // 设备类型相对固定?0分钟缓存
  })
}
...
export const useFaultTypes = (deviceTypeId?: string) => {
  return useQuery({
    ...
    select: (response) => response.data,
    staleTime: 30 * 60 * 1000, // 故障类型相对固定?0分钟缓存
  })
}
...
export const usePricingTemplates = (category?: string) => {
  return useQuery({
    ...
    select: (response) => response.data,
    staleTime: 15 * 60 * 1000, // 价格模板相对稳定?5分钟缓存
  })
}
...
export const useNotifications = (userId: string) => {
  return useQuery({
    ...
    // 通知需要更频繁的更?
    staleTime: 30 * 1000,
    refetchInterval: 60 * 1000, // 每分钟自动刷?
  })
}
 ...
 * 获取工单列表和统计数据的组合Hook
 */
export const useDashboardData = (shopId: string, filters?: WorkOrderFilters) => {
...
export const useCreateOrderData = (shopId: string) => {
  ...
  return {
    ...
    loading: deviceTypesQuery.isLoading || techniciansQuery.isLoading || pricingTemplatesQuery.isLoading,
    error: deviceTypesQuery.error || techniciansQuery.error || pricingTemplatesQuery.error,
  }
}