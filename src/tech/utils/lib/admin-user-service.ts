...
export class AdminUserService {
  ...
  static async getAllUsers(): Promise<AdminUser[]> {
    try {
      ...
      if (currentRole !== 'admin') {
        throw new Error('只有超级管理员可以查看用户列?);
      }
  ...
  // 创建管理员用?  static async createUser(userData: {
    email: string;
    role: UserRole;
    user_id?: string;
    ...
    try {
      ...
      // 检查当前用户权?      const currentRole = await AuthService.getUserRole(currentUser.id);
      if (currentRole !== 'admin') {
  ...
  // 更新管理员用?  static async updateUser(
    userId: string,
    ...
    try {
      ...
      console.error('更新管理员用户失?', error);
      return false;
    }
  }

  // 删除管理员用?  static async deleteUser(userId: string): Promise<boolean> {
    try {
      const currentUser = await AuthService.getCurrentUser();
      if (!currentUser) return false;
      ...
      console.error('删除管理员用户失?', error);
      return false;
    }
  }

  // 更新用户状?  static async updateUserStatus(
    userId: string,
    isActive: boolean
  ): Promise<boolean> {
    try {
