...
export class WmsShipmentService {
  ...
  async createShipment(request: ShipmentRequest): Promise<ShipmentResult> {
    try {
      // 1. 获取仓库WMS连接信息
      const warehouseConnection = await this.getWarehouseWmsConnection(request.warehouseId);
      ...
      if (!wmsResult.success) {
        return {
          success: false,
          errorMessage: wmsResult.errorMessage || 'WMS系统创建发货单失败'
        };
      }
      ...
      if (!updateResult.success) {
        console.warn('更新本地订单状态失败:', updateResult.errorMessage);
      }
   ...
   * 查询发货状态
   */
  async getShipmentStatus(wmsOrderId: string): Promise<ShipmentTracking | null> {
    try {
      // 1. 从本地数据库查询
      ...
      if (localTracking) {
        return {
          ...
          estimatedDelivery: localTracking.estimated_delivery ? new Date(localTracking.estimated_delivery) : undefined,
          actualDelivery: localTracking.actual_delivery ? new Date(localTracking.actual_delivery) : undefined,
   ...
   * 批量查询发货状态
   */
  async getBatchShipmentStatus(wmsOrderIds: string[]): Promise<Record<string, ShipmentTracking>> {
  ...
  async updateShipmentTracking(trackingData: {
    ...
    try {
      ...
      // 2. 更新关联的兑换订单状态
      await this.updateRelatedExchangeOrderStatus(trackingData.wmsOrderId, trackingData.status);
  ...
  async handleWmsCallback(callbackData: any): Promise<boolean> {
    try {
      console.log('收到WMS回调:', callbackData);
  ...
  private async updateLocalOrderStatus(
    ...
    try {
      ...
      if (error) {
        return {
          success: false,
          errorMessage: `更新订单状态失败: ${error.message}`
        };
      }
   ...
   * 查询WMS系统发货状态
   */
  private async queryWmsShipmentStatus(wmsOrderId: string): Promise<ShipmentTracking | null> {
    try {
      // 这里应该调用具体的WMS API
      // 暂时返回模拟数据
      console.log(`查询WMS发货状态: ${wmsOrderId}`);
      ...
      console.error('查询WMS发货状态错误:', error);
      return null;
    }
  }
   ...
   * 同步WMS跟踪信息到本地
   */
  private async syncWmsTrackingToLocal(tracking: ShipmentTracking): Promise<void> {
    try {
      await supabase
        .from('shipment_tracking')
        .insert({
   ...
   * 更新关联的兑换订单状态
   */
  private async updateRelatedExchangeOrderStatus(wmsOrderId: string, wmsStatus: string): Promise<void> {
    try {
      // 映射WMS状态到本地订单状态
      ...
      console.error('更新关联订单状态错误:', error);
    }
  }
  ...
  private async sendShipmentNotification(trackingData: any): Promise<void> {
    try {
      ...
      // 这里应该集成消息推送服务
      console.log(`发送发货通知给用户 ${order.user_id}: 订单 ${order.order_number} 状态更新为 ${trackingData.status}`);
      ...
      await supabase
        ...
        .insert({
          ...
          title: '订单发货状态更新',
          content: `您的订单 ${order.order_number} as any 状态已更新为: ${trackingData.status}`,
  ...
  private async callWmsApi(
    ...
    endpoint: string,
    data: any
  ): Promise<{ success: boolean; wmsOrderId?: string; trackingNumber?: string; errorMessage?: string; details?: any }> {
    try {
      // 构造API请求
      ...
      // 这里应该根据具体的WMS提供商构造请求
      // 暂时返回模拟成功响应
      console.log(`调用WMS API: ${apiUrl}`, data);
   ...
   * 验证WMS回调签名
   */
  private async verifyWmsCallbackSignature(callbackData: any): Promise<boolean> {
    // 实际项目中应该实现具体的签名验证逻辑
    // 这里简单返回true作为示例
    return true;
  }
  ...
  // 各种回调处理方法
  private async handleOrderCreatedCallback(callbackData: any): Promise<boolean> {
    console.log('处理订单创建回调:', callbackData);
    // 实现具体逻辑
    return true;
  }

  private async handleOrderShippedCallback(callbackData: any): Promise<boolean> {
    console.log('处理订单发货回调:', callbackData);
    // 更新发货状态和物流信息
  ...
  private async handleOrderDeliveredCallback(callbackData: any): Promise<boolean> {
    console.log('处理订单送达回调:', callbackData);
    // 更新为已送达状态
    await this.updateShipmentTracking({
      wmsOrderId: callbackData.wmsOrderId,
      status: 'delivered',
  ...
  private async handleOrderCancelledCallback(callbackData: any): Promise<boolean> {
    console.log('处理订单取消回调:', callbackData);
    // 更新为已取消状态
    await this.updateShipmentTracking({
      wmsOrderId: callbackData.wmsOrderId,
      status: 'cancelled'
