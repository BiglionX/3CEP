...
interface DepreciationConfig {
  ...
  maxYears: number; // 最大折旧年?  salvageValueRatio: number; // 残值比?}

// 设备类别配置
interface CategoryConfig {
  depreciationRate: number; // 该类别的折旧率调?  baseLifeYears: number; // 基础使用寿命
  premiumMultiplier: number; // 高端产品系数
}

// 品牌价值系?interface BrandValueConfig {
  [brand: string]: number;
}
...
export class DepreciationEngineService {
  ...
  private initializeConfig(): void {
    ...
    this.depreciationConfig = {
      ...
      maxYears: 8, // 最大折?�?      salvageValueRatio: 0.1, // 残值为原价?0%
    };
  ...
  async calculateBaselineValue(
    ...
    try {
      // 1. 获取基准价格
      const originalPrice =
        marketPrice || this.estimateOriginalPrice(deviceProfile);
  ...
  private estimateOriginalPrice(deviceProfile: DeviceProfile): number {
    ...
    // 基于型号和年份估算价?    if (model.includes('iphone')) {
  ...
  private calculateDepreciation(
    ...
    const now = new Date();
    const ageInYears =
      (now.getTime() - manufactureDate.getTime()) / (1000 * 60 * 60 * 24 * 365);

    // 获取类别配置
    const category = deviceProfile.productCategory || '智能手机';
    const categoryConfig =
      this.categoryConfigs[category] || this.categoryConfigs['智能手机'];

    // 计算调整后的折旧?    const adjustedAnnualRate =
      this.depreciationConfig.annualRate * categoryConfig.depreciationRate;
    ...
    // 确保不超过最大折?    const maxValueLoss =
      originalPrice * (1 - this.depreciationConfig.salvageValueRatio);
  ...
  private calculateConditionMultiplier(condition?: DeviceCondition): number {
    ...
    multiplier +=
      weights.functionality * (conditionScores[condition.functionality] || 0.9);
  ...
  private getAgeMultiplier(deviceProfile: DeviceProfile): number {
    ...
    const ageInYears =
      (new Date().getTime() - manufactureDate.getTime()) /
      (1000 * 60 * 60 * 24 * 365);
