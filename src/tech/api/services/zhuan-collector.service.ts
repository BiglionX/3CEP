...
class ThirdPartyApiClient {
  ...
  async getPriceData(models: string[], page: number = 1, limit: number = 50): Promise<ThirdPartyApiResponse> {
    try {
      console.log(`📤 发送请求到第三方API: ${this.baseUrl}/prices/search`);
      
      const response = await fetch(`${this.baseUrl}/prices/search`, {
        method: 'POST',
        headers: {
      ...
      console.log(`📥 收到第三方API响应: ${response.status}`);
      ...
      // 模拟数据（当第三方API不可用时）
      console.warn('⚠️ 第三方API不可用，使用模拟数据');
...
export class ZhuanCollectorService {
  ...
  async startCollection(): Promise<void> {
    if (this.isRunning) {
      console.log('🔄 数据采集任务已在运行中');
      return;
    }

    this.isRunning = true;
    console.log('🚀 启动转转数据采集任务');
    ...
    try {
      ...
      console.log(`✅ 数据采集任务已启动，间隔: ${this.config.intervalMinutes}分钟`);
    } catch (error) {
  ...
  private async collectDataBatch(): Promise<void> {
    ...
    try {
      // 按批次处理目标型号
      for (let i = 0; i < this.config.targetModels.length; i += this.config.batchSize) {
        const batchModels = this.config.targetModels.slice(i, i + this.config.batchSize);
  ...
  private async collectModelsData(models: string[]): Promise<void> {
    try {
      console.log(`📥 采集型号数据: ${models.join(', ')}`);
      ...
      // 转换为市场价格数据格式
      const marketPrices: MarketPriceCreateParams[] = response.data.items.map(item => ({
      ...
      if (marketPrices.length > 0) {
        ...
        console.log(`💾 成功保存 ${marketPrices.length} 条转转数据`);
      }
      ...
      if (this.config.retryAttempts > 0) {
        console.log(`🔁 尝试重试... (剩余 ${this.config.retryAttempts} 次)`);
  ...
  async triggerManualCollection(models?: string[]): Promise<void> {
    ...
    console.log(`🎯 手动触发采集: ${targetModels.join(', ')}`);
...
export const DEFAULT_ZHUAN_CONFIG: CollectionTaskConfig = {
  ...
  batchSize: 5, // 每批处理5个型号
  retryAttempts: 3 // 重试3次
};

// 导出单例实例
export const zhuanCollectorService = new ZhuanCollectorService(DEFAULT_ZHUAN_CONFIG);