...
export class PaymentGatewayService {
  ...
  constructor() {
    ...
    this.stripePublishableKey = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY || '';
   ...
   * 处理支付回调
   */
  async handlePaymentCallback(provider: string, data: any): Promise<PaymentResult> {
    switch (provider) {
      case 'stripe':
   ...
   * 创建Stripe支付会话
   */
  private async createStripeSession(request: PaymentRequest): Promise<PaymentSession> {
   ...
   * 创建微信支付会话
   */
  private async createWeChatPaySession(request: PaymentRequest): Promise<PaymentSession> {
   ...
   * 创建支付宝会话
   */
  private async createAlipaySession(request: PaymentRequest): Promise<PaymentSession> {
    ...
    try {
      ...
      console.error('创建支付宝会话失败:', error);
      throw new Error('创建支付宝会话失败');
    }
  }
  ...
  private async handleStripeCallback(data: any): Promise<PaymentResult> {
    try {
      ...
      return {
        ...
        tokensAdded: this.calculateTokens(data.amount_total / 100), // Stripe金额以分为单位
        status: isSuccess ? 'succeeded' : 'failed',
        errorMessage: isSuccess ? undefined : '支付未完成'
      };
  ...
  private async handleWeChatPayCallback(data: any): Promise<PaymentResult> {
    try {
      ...
      const isSuccess = data.return_code === 'SUCCESS' && data.result_code === 'SUCCESS';
      
      return {
        success: isSuccess,
        transactionId: data.transaction_id,
        tokensAdded: this.calculateTokens(parseFloat(data.total_fee) / 100), // 微信支付金额以分为单位
        status: isSuccess ? 'succeeded' : 'failed',
  ...
  private calculateTokens(amount: number): number {
    ...
    const baseRate = 100; // 每美元100个Token
    
    // 应用奖励比例（如果是套餐购买）
    const bonusMultiplier = 1.0; // 暂时不考虑奖励
   ...
   * 获取支持的支付方式
   */
  getSupportedPaymentMethods(): Array<{ id: string; name: string; icon: string }> {
    const methods = [
      {
        id: 'stripe',
        name: '信用卡/借记卡',
