...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
    try {
      ...
      if (options.jitter) {
        delay *= (0.5 + Math.random() * 0.5); // 添加50%的抖动
      }
...
export function MonitorPerformance(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
  ...
  descriptor.value = async function (...args: any[]) {
    ...
    try {
      ...
      // 记录慢查询警告
      if (duration > 5000) {
        logger.warn(`慢查询警告: ${propertyKey} 执行时间 ${duration}ms`, { traceId });
      }
...
export class ErrorHandler {
  ...
  static handleError(error: unknown, context?: Record<string, any>): never {
    if (error instanceof MLPredictionError) {
      ...
      // 根据错误类型决定是否重试
      if (error.retryable) {
        throw new Error(`可重试错误: ${error.message}`);
      } else {
...
export class MetricsCollector {
  ...
  recordPrediction(type: 'demand' | 'price', success: boolean, responseTime: number): void {
    ...
    // 更新平均响应时间
    const totalTime = metrics.avgResponseTime * (metrics.predictionCount - 1) + responseTime;
  ...
  private async updateDatabaseMetrics(key: string, metrics: Metrics): Promise<void> {
    try {
      if (typeof window === 'undefined' && process.env.SUPABASE_SERVICE_ROLE_KEY) {
        const { createClient } = await import('@supabase/supabase-js');
        const supabase = createClient(
...
// 使用示例的类型守卫
export function isMLPredictionError(error: unknown): error is MLPredictionError {
