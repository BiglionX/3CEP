...
export class MarketDataService {
  ...
  async createMarketPrice(data: MarketPriceCreateParams): Promise<MarketPrice> {
    try {
      const freshnessScore = data.freshnessScore ?? this.calculateFreshnessScore(new Date());
   ...
   * 批量创建市场价格记录
   */
  async createMarketPrices(prices: MarketPriceCreateParams[]): Promise<MarketPrice[]> {
    try {
      const priceData = prices.map(price => ({
        ...
        source: price.source,
        freshness_score: price.freshnessScore ?? this.calculateFreshnessScore(new Date())
      }));
   ...
   * 查询市场价格记录
   */
  async getMarketPrices(params: MarketPriceQueryParams = {}): Promise<MarketPrice[]> {
    try {
   ...
   * 根据设备型号获取最新的市场价格统计
   */
  async getLatestMarketData(deviceModel: string): Promise<AggregatedMarketData> {
    try {
      // 获取各平台最新数据
      const xianyuData = await this.getLatestPlatformData(deviceModel, 'xianyu');
      const zhuanTurnData = await this.getLatestPlatformData(deviceModel, 'zhuan_turn');
      
      // 计算聚合数据
      const aggregateData = this.calculateAggregateData([xianyuData, zhuanTurnData].filter(Boolean));
      
      // 确定最佳数据源
      const bestSource = this.determineBestSource(xianyuData, zhuanTurnData, aggregateData);
      const confidenceScore = this.calculateConfidenceScore(xianyuData, zhuanTurnData);
   ...
   * 更新市场价格记录
   */
  async updateMarketPrice(id: string, data: MarketPriceUpdateParams): Promise<MarketPrice> {
    try {
      ...
      if (data.medianPrice !== undefined) updateData.median_price = data.medianPrice;
      if (data.sampleCount !== undefined) updateData.sample_count = data.sampleCount;
      if (data.freshnessScore !== undefined) updateData.freshness_score = data.freshnessScore;
  ...
  // 私有辅助方法
  private async getLatestPlatformData(deviceModel: string, source: 'xianyu' | 'zhuan_turn'): Promise<PriceStatistics | null> {
    try {
      const { data: result, error } = await this.supabase
        .from('market_prices')
        .select('*')
      ...
      return {
        deviceModel: (result as any).device_model,
        avgPrice: (result as any).avg_price || 0,
        minPrice: (result as any).min_price || 0,
        maxPrice: (result as any).max_price || 0,
        medianPrice: (result as any).median_price || 0,
        sampleCount: (result as any).sample_count,
        source: (result as any).source,
        freshnessScore: (result as any).freshness_score,
        collectedAt: new Date((result as any).collected_at)
      };
    } catch (error) {
  ...
  private calculateAggregateData(platformData: (PriceStatistics | null)[]): PriceStatistics | null {
    ...
    return {
      ...
      avgPrice: parseFloat((avgPrices.reduce((a, b) => a + b, 0) / avgPrices.length).toFixed(2)),
      minPrice: Math.min(...minPrices),
      maxPrice: Math.max(...maxPrices),
      medianPrice: parseFloat((medianPrices.reduce((a, b) => a + b, 0) / medianPrices.length).toFixed(2)),
      sampleCount: sampleCounts.reduce((a, b) => a + b, 0),
      source: 'aggregate',
      freshnessScore: parseFloat((validData.map(d => d.freshnessScore).reduce((a, b) => a + b, 0) / validData.length).toFixed(2)),
  ...
  private determineBestSource(
    ...
    // 优先选择样本量大的数据源
    const sources = [
      { data: xianyuData, name: 'xianyu' as const, score: xianyuData?.sampleCount || 0 },
      { data: zhuanTurnData, name: 'zhuan_turn' as const, score: zhuanTurnData?.sampleCount || 0 }
  ...
  private calculateConfidenceScore(
    ...
    const totalSamples = (xianyuData?.sampleCount || 0) + (zhuanTurnData?.sampleCount || 0);
    
    // 基础置信度基于数据源数量
    let confidence = validSources * 0.3;
    
    // 样本量加分
    if (totalSamples >= 20) confidence += 0.4;
    else if (totalSamples >= 10) confidence += 0.2;
    else if (totalSamples >= 5) confidence += 0.1;
    
    // 新鲜度加分
    const avgFreshness = [
      xianyuData?.freshnessScore || 0,
      zhuanTurnData?.freshnessScore || 0
