...
export class MLPredictionService {
  ...
  async predictPrice(
    ...
    try {
      // 1. 收集历史价格数据
      const historicalData = await this.collectPriceHistory(
        productId,
        platform,
        horizonDays
      );
      ...
      throw new Error(
        `价格预测失败: ${error instanceof Error ? error.message : '未知错误'}`
      );
    }
  }
  ...
  private async collectPriceHistory(
    ...
    try {
      ...
      const { data, error } = await query.order('monitored_at', {
        ascending: true,
      });
...
- 最高价? ¥${priceStats.max.toFixed(2)}
- 最低价? ¥${priceStats.min.toFixed(2)}
- 价格波动? ${(priceStats.volatility * 100).toFixed(2)}%

【市场环境?- 竞争对手数量: ${priceStats.avgCompetitors.toFixed(1)}�?- 平均销? ${priceStats.avgVolume.toFixed(0)}�?�?
【预测要求?- 预测周期: ${horizonDays}�?- 包含价格区间和置信度
- 考虑市场竞争因素
...
请按照以下JSON格式返回预测结果?{
  ...
  "summary": {
    "priceTrend": "上涨|下跌|稳定",
    "expectedChange": 预期变化百分?float),
    "volatilityOutlook": "高|中|�?,
    "confidence": 整体置信?float, 0-1)
  },
  "marketInsights": {
...
严格按照上述格式返回，不要包含其他内容?`;
  ...
  /**
   * 降级到本地预测算?   */
  private async fallbackToLocalPrediction(
    prompt: string,
    type: 'demand' | 'price'
  ): Promise<string> {
    console.warn(`降级到本?{type}预测算法`);
    ...
    if (type === 'demand') {
      const dailyPredictions = futureDates.map(date => ({
        date,
        quantity: Math.floor(80 + Math.random() * 40), // 80-120�?        confidence: 0.8,
        lowerBound: 70,
        upperBound: 130,
      }));
      ...
      return JSON.stringify({
        predictedDemand: {
          ...
          summary: {
            totalQuantity: dailyPredictions.reduce(
              (sum, p) => sum + p.quantity,
              0
            ),
            averageDaily:
              dailyPredictions.reduce((sum, p) => sum + p.quantity, 0) /
              dailyPredictions.length,
            trend: 'stable',
            confidence: 0.85,
          },
        },
    ...
    try {
      ...
      return {
        predictions: result.predictedDemand.daily.map((day: any) => ({
          ...
          lowerBound: Math.max(
            0,
            Math.round(day.lowerBound || day.quantity * 0.8)
          ),
          upperBound: Math.max(
            0,
            Math.round(day.upperBound || day.quantity * 1.2)
          ),
        })),
        summary: {
          totalQuantity: Math.max(
            0,
            Math.round(result.predictedDemand.summary.totalQuantity)
          ),
          averageDaily: Math.max(
            0,
            result.predictedDemand.summary.averageDaily
          ),
          trend: result.predictedDemand.summary.trend,
          confidence: Math.min(
            1,
            Math.max(0, result.predictedDemand.summary.confidence)
          ),
        },
  ...
  private analyzeTrend(data: HistoricalSalesData[]) {
    if ((data as any)?.data.length < 2)
      return { direction: 'stable', slope: 0, volatility: 0 };
    ...
    const volatility =
      Math.sqrt(
        quantities.reduce((sum, q) => sum + Math.pow(q - avg, 2), 0) / n
      ) / avg;
  ...
  private calculatePriceStatistics(data: PriceHistoryData[]) {
    ...
    const volatility =
      Math.sqrt(
        prices.reduce((sum, p) => sum + Math.pow(p - average, 2), 0) /
          prices.length
      ) / average;
  ...
  private generateDefaultDemandPrediction(
    days: number
  ): DemandPredictionResult {
    return {
