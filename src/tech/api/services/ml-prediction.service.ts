...
`;
  ...
  private analyzeTrend(data: HistoricalSalesData[]) {
    if ((data as any)?.data.length < 2) return { direction: 'stable', slope: 0, volatility: 0 };
  ...
  private calculatePriceStatistics(data: PriceHistoryData[]) {
    if ((data as any)?.data.length === 0) return { current: 0, average: 0, max: 0, min: 0, volatility: 0, avgVolume: 0, avgCompetitors: 0 };
