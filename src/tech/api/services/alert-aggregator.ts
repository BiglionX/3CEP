...
export class AlertAggregator {
  ...
  async sendAlert(event: AlertEvent): Promise<void> {
    ...
    // 检查抑制规则
    if (this.config.enableSuppression && this.isSuppressed(event)) {
      console.log(`🔇 告警被抑制: ${event.title}`);
    ...
    if (this.isDuplicate(event)) {
      console.log(`🔄 告警去重: ${event.title}`);
      return;
    }

    // 检查冷却期
    if (this.isInCooldown(event)) {
      console.log(`❄️  告警冷却中: ${event.title}`);
      return;
    }
    ...
    const sendPromises = channels.map(async (channelName) => {
      ...
      if (service && service.isEnabled()) {
        try {
          ...
          console.log(`✅ 通过 ${channelName} 发送告警: ${event.title}`);
        } catch (error) {
          console.error(`❌ ${channelName} 发送失败:`, error);
        }
      }
    });
    ...
    // 设置冷却期
    this.setCooldown(event, applicableRules);
  ...
  private determineChannels(event: AlertEvent, rules: AlertRule[]): string[] {
    ...
    // 合并渠道并去重
    const allChannels = [...new Set([...ruleChannels, ...severityChannels])];

    // 过滤掉未注册的服务
    return allChannels.filter((channel) => this.services.has(channel));
  }
  ...
  private updateStatistics(event: AlertEvent): void {
    ...
    // 按严重程度统计
    this.statistics.alertsBySeverity[event.severity] =
      (this.statistics.alertsBySeverity[event.severity] || 0) + 1;

    // 按来源统计
    this.statistics.alertsBySource[event.source] =
  ...
  start(): void {
    if (this.isRunning) {
      console.log("⚠️  告警聚合服务已在运行");
      return;
    }

    this.isRunning = true;
    console.log("🚀 告警聚合服务启动");
...
export const DEFAULT_ALERT_RULES: AlertRule[] = [
  {
    ...
    escalationPolicy: {
      initialDelay: 10 * 60 * 1000, // 10分钟后升级
      repeatInterval: 30 * 60 * 1000, // 每30分钟重复
      maxNotifications: 3,
      escalateTo: ["phone"],
    },
  },
...
export const DEFAULT_SUPPRESSION_RULES: SuppressionRule[] = [
  {
    ...
    condition: (event) => {
      ...
      // 在凌晨2-4点抑制非紧急告警
      return hour >= 2 && hour < 4 && !["emergency"].includes(event.severity);
    },
