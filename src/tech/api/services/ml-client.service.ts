/**
 * V-ML-05: Node.js ML模型客户端服务
 * 封装调用Python微服务的HTTP客户端
 */

// 使用fetch替代axios以避免依赖问题
class MLModelClient {
  ...
  constructor(config: any = {}) {
    ...
    this.retryDelay = config.retryDelay || 1000; // 1秒重试间隔
  }
  ...
  private handleError(error: any) {
    ...
    if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
      return new Error('无法连接到ML服务，请检查服务是否运行');
    }
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 404:
          return new Error(`ML服务端点不存在: ${message}`);
        case 503:
          return new Error(`ML服务不可用: ${message}`);
        case 500:
   ...
   * 带重试机制的请求
   */
  private async requestWithRetry(method: string, url: string, data: any = null, attempt: number = 1): Promise<any> {
    try {
      ...
      console.log(`🔍 ML Client Request: ${method} ${fullUrl}`);
      
      const response = await fetch(fullUrl, requestOptions);
      clearTimeout(timeoutId);
      
      console.log(`✅ ML Client Response: ${response.status} ${url}`);
      ...
      console.log(`⏳ 第${attempt}次请求失败，${this.retryDelay}ms后重试...`);
  ...
  async predictPrice(deviceData: any) {
    try {
      ...
      // 标准化数据格式
      const requestData = this.normalizeDeviceData(deviceData);
      
      const response = await this.requestWithRetry('POST', '/predict', requestData);
    ...
    try {
      ...
      const normalizedDevices = devices.map((device, index) => {
        try {
          ...
          throw new Error(`第${index + 1}个设备数据无效: ${(error as Error).message}`);
        }
      });

      const requestData = { devices: normalizedDevices };
      const response = await this.requestWithRetry('POST', '/predict/batch', requestData);
  ...
  validateDeviceData(data: any) {
    ...
    // 数值范围验证
    if (data.deviceAgeMonths < 0 || data.deviceAgeMonths > 120) {
      throw new Error('设备年龄应在0-120个月范围内');
    }

    if (data.storageGb <= 0 || data.storageGb > 2048) {
      throw new Error('存储容量应在合理范围内');
    }

    if (data.ramGb <= 0 || data.ramGb > 64) {
      throw new Error('内存容量应在合理范围内');
    }

    if (data.batteryHealthPercent < 0 || data.batteryHealthPercent > 100) {
      throw new Error('电池健康度应在0-100范围内');
    }
  }
  ...
  normalizeDeviceData(data: any) {
    return {
      ...
      market_avg_price: data.marketAvgPrice ? Number(data.marketAvgPrice) : null,
      market_min_price: data.marketMinPrice ? Number(data.marketMinPrice) : null,
      market_max_price: data.marketMaxPrice ? Number(data.marketMaxPrice) : null,
      market_sample_count: data.marketSampleCount ? Number(data.marketSampleCount) : null
    };
  }
