...
// 数据库配?export interface DatabaseConfig {
  host: string;
  port: number;
  username: string;
  password: string;
...
// 系统配置主接?export interface FcxSystemConfig {
...
// 从环境变量加载配?export function loadConfig(): FcxSystemConfig {
  ...
  const config: FcxSystemConfig = {
    ...
    database: {
      ...
      port: parseInt(
        process.env.DB_PORT || DEFAULT_CONFIG.database.port.toString()
      ),
      ...
      poolSize: parseInt(
        process.env.DB_POOL_SIZE || DEFAULT_CONFIG.database.poolSize.toString()
      ),
    },
    redis: {
      host: process.env.REDIS_HOST || DEFAULT_CONFIG.redis.host,
      port: parseInt(
        process.env.REDIS_PORT || DEFAULT_CONFIG.redis.port.toString()
      ),
    ...
    payment: {
      stripePublicKey:
        process.env.STRIPE_PUBLIC_KEY || DEFAULT_CONFIG.payment.stripePublicKey,
      stripeSecretKey:
        process.env.STRIPE_SECRET_KEY || DEFAULT_CONFIG.payment.stripeSecretKey,
      paypalClientId:
        process.env.PAYPAL_CLIENT_ID || DEFAULT_CONFIG.payment.paypalClientId,
      paypalSecret:
        process.env.PAYPAL_SECRET || DEFAULT_CONFIG.payment.paypalSecret,
      alipayAppId:
        process.env.ALIPAY_APP_ID || DEFAULT_CONFIG.payment.alipayAppId,
      alipayPrivateKey:
        process.env.ALIPAY_PRIVATE_KEY ||
        DEFAULT_CONFIG.payment.alipayPrivateKey,
    },
    logging: {
    ...
    security: {
      ...
      jwtExpiresIn:
        process.env.JWT_EXPIRES_IN || DEFAULT_CONFIG.security.jwtExpiresIn,
      bcryptSaltRounds: parseInt(
        process.env.BCRYPT_SALT_ROUNDS ||
          DEFAULT_CONFIG.security.bcryptSaltRounds.toString()
      ),
      rateLimitWindowMs: parseInt(
        process.env.RATE_LIMIT_WINDOW_MS ||
          DEFAULT_CONFIG.security.rateLimitWindowMs.toString()
      ),
      rateLimitMaxRequests: parseInt(
        process.env.RATE_LIMIT_MAX_REQUESTS ||
          DEFAULT_CONFIG.security.rateLimitMaxRequests.toString()
      ),
    },
  };
...
// 验证配置完整?export function validateConfig(config: FcxSystemConfig): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
