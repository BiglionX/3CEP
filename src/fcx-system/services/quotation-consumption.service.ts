...
export const QUOTATION_CONSUMPTION_CONFIG = {
  // 基础询价消?  BASE_QUOTATION_COST: 10, // 每次询价基础消?0 FCX

  // 供应商数量相关消?  PER_SUPPLIER_COST: 2, // 每个供应商额外消? FCX
  MAX_SUPPLIERS_FREE: 3, // �?个供应商不额外收?  MAX_SUPPLIERS_LIMIT: 20, // 最多同时询?0个供应商

  // 商品数量相关消?  PER_ITEM_COST: 1, // 每个商品项额外消? FCX
  MAX_ITEMS_FREE: 5, // �?个商品项不额外收?
  // 特殊功能消?  URGENT_QUOTATION_COST: 20, // 加急询价额外消?0 FCX
  CUSTOM_TEMPLATE_COST: 15, // 自定义模板额外消?5 FCX
  AUTO_FOLLOW_UP_COST: 5, // 自动跟进额外消? FCX

  // 批量操作消?  BATCH_QUOTATION_DISCOUNT: 0.8, // 批量询价8折优?  MIN_BATCH_SIZE: 5, // 批量询价最小数?
  // 每日限额
  DAILY_QUOTATION_LIMIT: 50, // 每日最多询?0�?  DAILY_FCX_SPEND_LIMIT: 500, // 每日FCX消耗上?} as const;

/**
 * 询价消耗明细接? */
interface QuotationConsumptionDetail {
  baseCost: number; // 基础费用
  supplierCost: number; // 供应商相关费?  itemCountCost: number; // 商品数量费用
  urgentCost: number; // 加急费?  templateCost: number; // 模板费用
  autoFollowUpCost: number; // 自动跟进费用
  batchDiscount: number; // 批量折扣
  totalCost: number; // 总费?  breakdown: Array<{
    item: string;
    cost: number;
    description: string;
  }>;
...
interface QuotationParameters {
  supplierCount: number; // 供应商数?  itemCount: number; // 商品项数?  isUrgent?: boolean; // 是否加?  useCustomTemplate?: boolean; // 是否使用自定义模?  enableAutoFollowUp?: boolean; // 是否启用自动跟进
...
export class QuotationConsumptionService {
  ...
  calculateConsumption(
    ...
    if (params.supplierCount > config.MAX_SUPPLIERS_FREE) {
      ...
      breakdown.push({
        item: `额外供应商费?(${extraSuppliers}�?`,
        cost: supplierCost,
        description: `超出免费额度?{extraSuppliers}个供应商`,
      });
    }
    ...
    if (params.itemCount > config.MAX_ITEMS_FREE) {
      ...
      breakdown.push({
        item: `额外商品项费?(${extraItems}�?`,
        cost: itemCountCost,
        description: `超出免费额度?{extraItems}个商品项`,
      });
    }

    // 4. 加急费?    let urgentCost = 0;
    if (params.isUrgent) {
    ...
    if (params.isBatchOperation) {
      ...
      breakdown.push({
        item: '批量操作折扣',
        cost: -batchDiscount,
        description: '批量询价享受8折优?,
      });
    }

    const totalCost = Math.max(1, Math.round(subtotal)); // 最少消? FCX
  ...
  async deductFcxForQuotation(
    ...
    try {
      // 模拟扣除FCX的过?      // 在实际项目中这里会调用真实的FCX账户服务

      // 1. 检查余额（模拟?      const hasSufficientBalance = await this.simulateBalanceCheck(
        userId,
        consumptionDetail.totalCost
      );

      if (!hasSufficientBalance) {
        return {
          success: false,
          errorMessage: `FCX余额不足，需?{consumptionDetail.totalCost} FCX`,
        };
      }
      ...
      // 3. 记录交易（模拟）
      await this.simulateTransactionRecord(
        userId,
        consumptionDetail.totalCost,
        quotationRequestId,
        description
  ...
  /**
   * 模拟余额检?   */
  private async simulateBalanceCheck(
    userId: string,
    requiredAmount: number
  ): Promise<boolean> {
  ...
  private async simulateTransactionRecord(
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
      `记录FCX交易: 用户${userId}, 消?{amount} FCX, 询价ID:${quotationRequestId}, 描述:${description}`
    )}
  ...
  async checkDailyLimits(userId: string): Promise<{
    ...
    try {
      ...
      // 获取今日FCX消?      const accountIdResult = await supabase
        .from('fcx_accounts')
        .select('id')
        .eq('user_id', userId)
        .single();
      ...
      // 检查询价次数限?      if (quotationCountToday >= config.DAILY_QUOTATION_LIMIT) {
        errors.push(`今日询价次数已达上限 (${config.DAILY_QUOTATION_LIMIT}�?`);
        canSendQuotation = false;
      }

      // 检查FCX消耗限?      if (fcxSpentToday >= config.DAILY_FCX_SPEND_LIMIT) {
        errors.push(
          `今日FCX消耗已达上?(${config.DAILY_FCX_SPEND_LIMIT} FCX)`
        );
        canSendQuotation = false;
      }
  ...
  > {
    try {
      // 计算消耗明?      const consumptionDetail = this.calculateConsumption(params);

      // 检查余?      const balanceCheck = await this.checkUserBalance(
        userId,
        consumptionDetail.totalCost
      );

      // 检查每日限?      const dailyLimits = await this.checkDailyLimits(userId);

      return {
        ...consumptionDetail,
        canAfford:
          balanceCheck.hasSufficientBalance && dailyLimits.canSendQuotation,
        currentBalance: balanceCheck.currentBalance,
        dailyLimits,
      };
    } catch (error) {
      console.error('预估询价消耗错?', error);
      throw error;
    }
  }
