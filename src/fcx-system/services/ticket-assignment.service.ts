...
export class TicketAssignmentService {
  ...
  async loadEngineers(): Promise<void> {
    try {
      // 模拟从数据库加载工程师数据
      this.engineers = await this.fetchActiveEngineers();
    } catch (error) {
  ...
  async assignTicket(
    ...
    try {
      // 1. 数据预处理
      const availableEngineers = this.filterAvailableEngineers(params);
      ...
      switch (params.strategy) {
        case AssignmentStrategy.SKILL_MATCH:
          assignments = this.skillBasedAssignment(ticket, availableEngineers, params);
          break;
        case AssignmentStrategy.LOCATION_BASED:
          assignments = this.locationBasedAssignment(ticket, availableEngineers, params);
          break;
        case AssignmentStrategy.LOAD_BALANCED:
          assignments = this.loadBalancedAssignment(ticket, availableEngineers, params);
          break;
        case AssignmentStrategy.EXPERIENCE_BASED:
          assignments = this.experienceBasedAssignment(ticket, availableEngineers, params);
          break;
        default:
          assignments = this.hybridAssignment(ticket, availableEngineers, params);
      }
      ...
      // 4. 更新工程师负载
      await this.updateEngineerLoad(bestAssignment.assignedEngineerId);
  ...
  private skillBasedAssignment(
    ...
    return engineers
      .map(engineer => {
        const skillScore = this.calculateSkillMatchScore(ticket.requiredSkills, engineer.skills);
        ...
        return {
          ...
          estimatedTravelTime: this.calculateTravelTime(ticket.location, engineer.location),
          estimatedWorkTime: this.estimateWorkTime(ticket, engineer),
          totalEstimatedTime: 0,
          reasons: this.generateAssignmentReasons(skillScore, experienceScore, ratingScore),
          alternativeEngineers: []
        };
      })
      .map(result => ({
        ...result,
        totalEstimatedTime: result.estimatedTravelTime + result.estimatedWorkTime
      }))
  ...
  private locationBasedAssignment(
    ...
    return engineers
      .map(engineer => {
        const distance = this.calculateDistance(ticket.location, engineer.location);
        const travelTime = this.calculateTravelTime(ticket.location, engineer.location);
        ...
        return {
          ...
          estimatedWorkTime: this.estimateWorkTime(ticket, engineer),
          totalEstimatedTime: 0,
          reasons: [`距离最近 (${distance.toFixed(1)}km)`, `路程时间 ${travelTime}分钟`],
          alternativeEngineers: []
        };
      })
      .map(result => ({
  ...
  private experienceBasedAssignment(
    ...
    return engineers
      .map(engineer => {
        ...
        const completedTicketsScore = Math.min(1, engineer.completedTickets / 1000);
        ...
        return {
          ...
          reasons: [
            ...
            `成功率 ${(engineer.successRate).toFixed(1)}%`,
  ...
  private hybridAssignment(
    ...
    return engineers
      .map(engineer => {
        const skillScore = this.calculateSkillMatchScore(ticket.requiredSkills, engineer.skills);
        const locationScore = Math.max(0, 1 - (this.calculateDistance(ticket.location, engineer.location) / params.maxDistanceKm));
        const loadScore = 1 - (engineer.currentLoad / engineer.maxCapacity);
        const experienceScore = engineer.experienceYears / 20;
        const ratingScore = engineer.rating / 5;
        ...
        return {
          ...
          reasons: this.generateHybridReasons(
            skillScore, locationScore, loadScore, experienceScore, ratingScore
          ),
          alternativeEngineers: []
        };
      })
   ...
   * 筛选可用工程师
   */
  private filterAvailableEngineers(params: AssignmentAlgorithmParams): Engineer[] {
    return this.engineers.filter(engineer => {
      if (params.excludeOffline && engineer.availability.status !== EngineerStatus.AVAILABLE) {
        return false;
      }

      if (params.excludeOverloaded && (engineer.currentLoad >= engineer.maxCapacity * params.maxLoadFactor)) {
        return false;
      }
   ...
   * 计算技能匹配分数
   */
  private calculateSkillMatchScore(requiredSkills: EngineerSkill[], engineerSkills: EngineerSkill[]): number {
    if (requiredSkills.length === 0) return 1;
    
    const matchedSkills = requiredSkills.filter(skill => engineerSkills.includes(skill));
   ...
   * 检查是否具备所需技能
   */
  private hasRequiredSkills(engineerSkills: EngineerSkill[], requiredSkills: EngineerSkill[]): boolean {
   ...
   * 估算工作时间(分钟)
   */
  private estimateWorkTime(ticket: ExtendedRepairOrder, engineer: Engineer): number {
    let baseTime = 60;
   ...
   * 生成分配理由
   */
  private generateAssignmentReasons(skillScore: number, experienceScore: number, ratingScore: number): string[] {
    const reasons: string[] = [];
    
    if (skillScore > 0.8) reasons.push('技能高度匹配');
    else if (skillScore > 0.5) reasons.push('技能基本匹配');
    else reasons.push('技能部分匹配');
  ...
  private async updateEngineerLoad(engineerId: string): Promise<void> {
    ...
    if (engineer) {
      ...
      console.log(`更新工程师 ${engineer.name} 负载至 ${engineer.currentLoad}`);
    }
  }
