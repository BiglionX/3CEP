...
export class SLAMonitorService {
  ...
  private notificationCallbacks: Array<
    (rule: SLAMonitorRule, eventType: string) => void
  > = [];
   ...
   * 为工单创建SLA监控规则
   */
  async createSLAMonitorRule(
    ticket: ExtendedRepairOrder
  ): Promise<SLAMonitorRule> {
    try {
      ...
      // 启动定时检?      this.scheduleSLAChecks(rule);
  ...
  async checkSLAStatus(ticketId: string): Promise<{
    ...
    const rule = Array.from(this.monitorRules.values()).find(
    ...
    // 检查是否需要发送警?    const warningTriggered = this.shouldSendWarning(rule, remainingMinutes);

    // 检查是否需要升?    const escalationNeeded = this.shouldEscalate(rule, remainingMinutes);
  ...
  async handleSLAViolation(ticketId: string): Promise<void> {
    ...
    if (!rule) {
      console.warn(`未找到工?${ticketId} 的SLA规则`);
      return;
    }

    // 记录违规
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`🚨 SLA违规: 工单 ${ticketId} 超时`)// 执行升级规则
  ...
  registerNotificationCallback(
    callback: (rule: SLAMonitorRule, eventType: string) => void
  ): void {
  ...
  async cancelSLAMonitor(ticketId: string): Promise<void> {
    ...
    if (rule) {
      ...
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`取消工单 ${ticketId} 的SLA监控`)}
  }
  ...
  async batchCheckSLA(): Promise<void> {
    const activeRules = Array.from(this.monitorRules.values()).filter(
      rule => rule.isActive
    );
    ...
    for (const rule of activeRules) {
      try {
        ...
        console.error(`检查工?${rule.ticketId} SLA状态失?`, error);
      }
    }
  }
  ...
  private calculateSLADeadline(ticket: ExtendedRepairOrder): Date {
    ...
    // 根据SLA级别设置不同的响应时?    const slaHours = this.getSLAHours(ticket.slaLevel);

    // 根据紧急程度调整时?    const priorityMultiplier = this.getPriorityMultiplier(ticket.priority);

    const adjustedHours = slaHours * priorityMultiplier;
    const deadline = new Date(
      baseTime.getTime() + adjustedHours * 60 * 60 * 1000
    );
  ...
  private getWarningThreshold(slaLevel: SLALevel): number {
    switch (slaLevel) {
      case SLALevel.VIP:
        return 30; // 30分钟前提?      case SLALevel.PREMIUM:
        return 60; // 1小时前提?      case SLALevel.PRIORITY:
        return 120; // 2小时前提?      case SLALevel.STANDARD:
        return 240; // 4小时前提?      default:
        return 240;
    }
  }
  ...
  private generateEscalationRules(
    slaLevel: SLALevel
  ): SLAMonitorRule['escalationRules'] {
    const rules = [];
    ...
    switch (slaLevel) {
        ...
        rules.push(
          {
            ...
            messageTemplate: 'VIP工单即将超时，请立即处理',
          },
          {
            delayMinutes: 30,
            action: 'escalate' as const,
            targetRole: 'supervisor' as const,
            messageTemplate: 'VIP工单已超?5分钟，请立即介入',
          },
          {
            delayMinutes: 60,
            action: 'notify_admin' as const,
            targetRole: 'admin' as const,
            messageTemplate: 'VIP工单严重超时，需要管理员处理',
          }
        );
        break;
        ...
        rules.push(
          {
            ...
            action: 'remind' as const,
            targetRole: 'engineer' as const,
            messageTemplate: '高级工单即将超时，请尽快处理',
          },
          {
            delayMinutes: 120,
            action: 'escalate' as const,
            targetRole: 'supervisor' as const,
            messageTemplate: '高级工单已超时，请主管协?,
          }
        );
        break;
      ...
      case SLALevel.PRIORITY:
        rules.push({
          ...
          action: 'remind' as const,
          targetRole: 'engineer' as const,
          messageTemplate: '优先工单即将超时，请注意处理',
        });
        break;
  ...
  private shouldSendWarning(
    ...
    // 检查是否到达警告阈?    return remainingMinutes <= rule.warningThreshold && remainingMinutes > 0;
  }
  ...
  private shouldEscalate(
    ...
    for (const escalationRule of rule.escalationRules) {
      ...
      const breachTime = new Date(
        rule.deadline.getTime() + delayMinutes * 60 * 1000
      );

      // 检查是否已经超过升级时?      if (now >= breachTime) {
        // 检查是否已经执行过此升?        const alreadyExecuted = rule.notificationsSent.some(
          n =>
            n.type === 'escalation' &&
            n.message.includes(escalationRule.messageTemplate)
        );
  ...
  private async executeEscalation(rule: SLAMonitorRule): Promise<void> {
    ...
    for (const escalationRule of rule.escalationRules) {
      ...
      if (now >= breachTime) {
        // 执行相应的升级动?        switch (escalationRule.action) {
          case 'remind':
  ...
  private async sendReminder(
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`发送提醒给工程? ${escalationRule.messageTemplate}`)await this.recordNotification(
      rule,
      'warning',
      'engineer',
      escalationRule.messageTemplate
    );
  }
  ...
  private async escalateToSupervisor(
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`升级到主? ${escalationRule.messageTemplate}`)await this.recordNotification(
      rule,
      'escalation',
      'supervisor',
      escalationRule.messageTemplate
   ...
   * 重新分配工单
   */
  private async reassignTicket(
    rule: SLAMonitorRule,
    escalationRule: any
  ): Promise<void> {
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`重新分配工单: ${rule.ticketId}`)await this.recordNotification(
      rule,
      'escalation',
      'system',
      '工单已重新分配给其他工程?
  ...
  /**
   * 通知管理?   */
  private async notifyAdministrator(
    rule: SLAMonitorRule,
    escalationRule: any
  ): Promise<void> {
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`通知管理? ${escalationRule.messageTemplate}`)await this.recordNotification(
      rule,
      'escalation',
      'admin',
      escalationRule.messageTemplate
   ...
   * 发送SLA通知
   */
  private async sendSLANotifications(
    rule: SLAMonitorRule,
    eventType: string
  ): Promise<void> {
    // 调用所有注册的通知回调
  ...
  private scheduleSLAChecks(rule: SLAMonitorRule): void {
    ...
    const warningTime = new Date(
      rule.deadline.getTime() - rule.warningThreshold * 60 * 1000
    );
    const now = new Date();
