...
export class AllianceService implements IAllianceService {
   ...
   * 维修店加入联盟
   */
  async joinAlliance(shopId: string, stakingAmount: number): Promise<ExtendedRepairShop> {
    try {
      // 1. 验证质押金额
      ...
      // 3. 查找店铺关联的FCX账户
      let fcxAccount = await this.accountService.getAccountByUserId(shop.userId!);
      ...
      // 4. 验证账户余额是否足够
      const accountBalance = await this.accountService.getBalance(fcxAccount.id);
      if (accountBalance.availableBalance < stakingAmount) {
        throw new Error('账户余额不足，无法完成质押');
      }
      ...
      const transaction = await this.transactionService.createTransaction({
        ...
        referenceId: shopId,
        memo: `维修店加盟质押 ${stakingAmount} FCX`
      });
      ...
      if (error) {
        // 如果更新失败，需要回滚冻结操作
        await this.accountService.unfreeze(fcxAccount.id, stakingAmount);
  ...
  async leaveAlliance(shopId: string): Promise<ExtendedRepairShop> {
    try {
      ...
      if (!shop.isAllianceMember) {
        throw new Error('该店铺不是联盟成员');
      }

      // 2. 查找店铺关联的FCX账户
      const fcxAccount = await this.accountService.getAccountByUserId(shop.userId!);
      if (!fcxAccount) {
   ...
   * 获取联盟成员列表
   */
  async listAllianceMembers(level?: AllianceLevel): Promise<ExtendedRepairShop[]> {
    try {
      let query = supabase
        .from('repair_shops')
        .select('*')
   ...
   * 获取联盟排行榜
   */
  async getRankings(limit: number = 50): Promise<Array<{
    shopId: string;
    shopName: string;
  ...
  async validateAllianceQualification(shopId: string): Promise<boolean> {
    try {
      ...
      // 检查基本条件
      const hasValidInfo = data.name && data.contact_person && data.phone && data.address;
   ...
   * 内部方法：获取维修店信息
   */
  private async getRepairShop(shopId: string): Promise<ExtendedRepairShop | null> {
    try {
      const { data, error } = await supabase
        .from('repair_shops')
        .select('*')
