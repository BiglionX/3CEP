...
export class Fcx2RewardService implements IFcx2RewardService {
  ...
  async calculateOrderReward(
    ...
    try {
      ...
      if (!isValid) {
        throw new Error('不满足奖励发放条?);
      }

      // 2. 获取维修店当前等?      const shopLevel = await this.getShopLevel(order.repairShopId!);
      if (!shopLevel) {
        throw new Error('无法获取维修店等级信?);
      }

      // 3. 计算基础奖励（锁定金额的百分比）
      const baseReward =
        (order.fcxAmountLocked || 0) * REWARD_CONSTANTS.BASE_PERCENTAGE;
      ...
      // 6. 计算最终奖?      const totalReward = baseReward * ratingMultiplier;
  ...
  async calculateLevelBonus(
    ...
    try {
      // 将枚举值转换为常量键名
      const levelKey =
        currentLevel.toUpperCase() as keyof typeof REWARD_CONSTANTS.LEVEL_BONUS;
  ...
  getRatingMultiplier(rating: number): number {
    // 评分低于最低要求，无奖?    if (rating < REWARD_CONSTANTS.MIN_RATING_FOR_REWARD) {
      return 0;
    }

    // 线性计算倍数?.0�?1.0倍，5.0�?最大倍数
    ...
    // 线性插值计?    const ratio = (rating - minRating) / (maxRating - minRating);
  ...
  async validateRewardConditions(
    ...
    try {
      // 1. 工单必须已完?      if (order.status !== OrderStatus.COMPLETED) {
        return false;
      }

      // 2. 必须有评?      if (rating === null || rating === undefined) {
        return false;
      }

      // 3. 评分不能为负?      if (rating < 0) {
        return false;
      }

      // 4. 必须有锁定金?      if (!order.fcxAmountLocked || order.fcxAmountLocked <= 0) {
        return false;
      }

      // 5. 维修店必须存在且为联盟成?      const shopLevel = await this.getShopLevel(order.repairShopId!);
      if (shopLevel === null) {
        return false;
      }
  ...
  async processBatchRewards(
    orders: Array<{ order: RepairOrder; rating: number }>
  ): Promise<
    Array<{
      order: RepairOrder;
      rating: number;
