...
export enum AssignmentStrategy {
  SKILL_MATCH = 'skill_match',     // 技能匹配优先
  LOCATION_BASED = 'location_based', // 位置就近优先
...
export interface TicketAssignment {
  ...
  strategyUsed: AssignmentStrategy; // 使用的分配策略
...
export interface ExtendedRepairOrder {
  ...
  estimatedCompletionAt: Date | null; // 预计完成时间
  ...
  customerUrgency: number; // 客户紧急程度 (1-5)
...
export interface SLAMonitorRule {
  ...
  slaLevel: SLALevel;
  deadline: Date;
  warningThreshold: number; // 提前提醒阈值(分钟)
  escalationRules: Array<{
    delayMinutes: number; // 延迟分钟数
    action: 'remind' | 'escalate' | 'reassign' | 'notify_admin';
...
export interface AutoSettlementConfig {
  id: string;
  isEnabled: boolean;
  minRatingForFullPayment: number; // 获得全额付款的最低评分
  partialPaymentThreshold: number; // 部分付款阈值
  penaltyRate: number; // 超时罚金比例
...
export interface AssignmentAlgorithmParams {
  ...
  maxDistanceKm: number; // 最大距离限制
  maxLoadFactor: number; // 最大负载因子
  skillWeight: number; // 技能匹配权重 (0-1)
  locationWeight: number; // 位置权重 (0-1)
  experienceWeight: number; // 经验权重 (0-1)
  ratingWeight: number; // 评分权重 (0-1)
  excludeOffline: boolean; // 是否排除离线工程师
  excludeOverloaded: boolean; // 是否排除超载工程师
}
