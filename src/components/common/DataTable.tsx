...
export function DataTable<T>({
  ...
  const getValue = (record: T, column: Column<T>): any => {
    if (typeof column.key === 'string' && column.key.includes('.')) {
      // 处理嵌套属性
      return column.key.split('.').reduce((obj: any, key: string) => obj?.[key], record)
    }
    return (record as any)[column.key]
  }

  const renderCell = (record: T, column: Column<T>, index: number): React.ReactNode => {
    ...
    return (
      <div className="flex items-center justify-between px-4 py-3 border-t">
        <div className="text-sm text-muted-foreground">
          显示第 {startItem} 到 {endItem} 条，共 {pagination.total} 条记录
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {actions && actions.length > 0 && <TableHead className="text-right">操作</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
