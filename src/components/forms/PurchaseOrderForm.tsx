...
import { enterpriseValidations, type FormError } from '@/lib/validation';
...
export function PurchaseOrderForm() {
  ...
  const formMethods = useForm<PurchaseOrderFormData>({
    resolver: zodResolver(enterpriseValidations.purchaseOrder) as any,
    defaultValues: {
  ...
  const calculateTotalAmount = () => {
    const items: PurchaseOrderFormData['items'] = formData.items || [];
    const total = items.reduce(
      (sum: number, item: PurchaseOrderFormData['items'][number]) =>
  ...
  // 表单提交处理
  const onSubmit = async (_data: PurchaseOrderFormData) => {
    try {
      setIsSubmitting(true);
      setFormErrors([]);
      setSubmitStatus('idle');

      // 验证表单数据 - 使用 zod resolver 自动验证
  ...
  const getFieldError = (fieldName: string) => {
    return (
      errors[fieldName as keyof PurchaseOrderFormData]?.message ||
      ...
      <CardContent>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {formData.items.map(
              (
                item: PurchaseOrderFormData['items'][number],
                index: number
              ): React.ReactNode => (
