...
export default function DynamicModuleMenu({
  ...
  const [sortBy, setSortBy] = useState<'priority' | 'name' | 'recent'>('priority')
  ...
  const renderModuleItem = (module: ModuleConfig) => {
    ...
    const baseClasses = "flex items-center w-full text-left p-3 rounded-lg transition-all duration-200 group"
    ...
    return (
      ...
      >
        ...
        {isActive && (
          <div className="w-1 h-8 bg-blue-700 rounded-full ml-2" />
        )}
      </Link>
    )
  }
  ...
  // 渲染分类组
  const renderCategoryGroup = (categoryKey: string, categoryData: { 
    name: string; 
    ...
    return (
      <div key={categoryKey} className="mb-2">
        ...
        {(isExpanded || mode === 'icons') && (
          <div className={`space-y-1 ${mode === 'icons' ? 'grid grid-cols-3 gap-2' : ''}`}>
