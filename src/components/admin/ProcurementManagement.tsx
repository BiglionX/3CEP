...
// 暂时移除Card和Badge组件导入
import { Plus, Search, Filter, Eye, Edit, Truck, CheckCircle, XCircle } from 'lucide-react';
...
export default function ProcurementManagement() {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { variant: "default" | "secondary" | "destructive" | "outline", icon: React.ReactNode }> = {
      pending: { variant: 'secondary', icon: <Plus className="w-3 h-3 mr-1" /> },
      confirmed: { variant: 'default', icon: <CheckCircle className="w-3 h-3 mr-1" /> },
      shipping: { variant: 'outline', icon: <Truck className="w-3 h-3 mr-1" /> },
      received: { variant: 'default', icon: <CheckCircle className="w-3 h-3 mr-1" /> },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="p-6 border-b">
          ...
          <p className="text-gray-600 mt-1">共 {filteredOrders.length} 个订单</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">预计到货</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredOrders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{order.supplier?.name || '未知供应商'}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{order.warehouse?.name || '未知仓库'}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{order.items.reduce((sum, item) => sum + item.quantity, 0)} 件</td>
                  <td className="px-6 py-4 whitespace-nowrap font-semibold">{formatCurrency(order.totalAmount)}</td>
