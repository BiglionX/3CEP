...
// 导出客户端接?export type { LogisticsCarrierClient };

// 生成UUID的简单实?function generateUUID(): string {
...
// 17track客户端适配?export class Track17Client implements LogisticsCarrierClient {
  ...
  private convertToUnifiedFormat(data: any): ShipmentTracking {
    ...
    return {
      ...
      carrierName: data.carrier_name || '未知承运?,
      status: latestStatus,
      origin: data?.origin_info?.AcceptTime
        ? new Date(data.track_info.origin_info.AcceptTime).toLocaleDateString()
        : '',
      destination: '',
      estimatedDelivery: data?.destination_info?.EstimatedDeliveryTime
...
// 快递鸟客户端适配?export class KdniaoClient implements LogisticsCarrierClient {
  private config: CarrierConfig;
  ...
  async getTrackingInfo(
    ...
    try {
      const requestData = {
        OrderCode: '',
        ShipperCode: '', // 空值表示自动识?        LogisticCode: trackingNumber,
      };
  ...
  private convertToUnifiedFormat(data: any): ShipmentTracking {
    ...
    return {
      ...
      carrierName: data.ShipperName || '未知承运?,
      status: latestStatus,
...
// 顺丰速运官方客户端适配?export class SfExpressClient implements LogisticsCarrierClient {
  private config: CarrierConfig;
  ...
  validateTrackingNumber(trackingNumber: string): boolean {
    // 顺丰运单号通常?2位数?    return /^\d{12}$/.test(trackingNumber);
  }
  ...
  private convertToUnifiedFormat(data: any): ShipmentTracking {
    ...
    // 按时间排?    timeline.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
  ...
  private mapStatus(remark: string): TrackingStatus {
    if (remark.includes('已签?)) return TrackingStatus.DELIVERED;
    if (remark.includes('派?)) return TrackingStatus.OUT_FOR_DELIVERY;
