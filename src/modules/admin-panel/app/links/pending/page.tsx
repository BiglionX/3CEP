...
export default function PendingLinksPage() {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, { text: string; className: string }> = {
      pending_review: { text: '待审核', className: 'bg-yellow-100 text-yellow-800' },
      promoted: { text: '已发布', className: 'bg-green-100 text-green-800' },
    ...
    try {
      ...
      const response = await fetch('/api/admin/articles/drafts', {
        ...
        body: JSON.stringify({
          ...
          summary: link.description?.substring(0, 200) || '',
          coverImageUrl: link.image_url,
          tags: link.ai_tags?.tags || [],
                  ...
                  <TableRow key={link.id} className="border-b last:border-b-0">
                    ...
                    <TableCell>
                      <div className="space-y-1">
                        {link.ai_tags?.tags?.map((tag, index) => (
                          <span 
                            key={index}
                        ...
                        {link.ai_tags?.confidence && (
                          <div className="text-xs text-gray-500">
                            置信度: {(link.ai_tags.confidence * 100).toFixed(0)}%
                          </div>
                        )}
                      </div>
                    </TableCell>
              ...
              <div className="space-y-6 py-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div><span className="font-medium">子分类:</span> {previewLink.sub_category || '无'}</div>
                      <div><span className="font-medium">抓取时间:</span> {formatDate(previewLink.scraped_at)}</div>
                      <div><span className="font-medium">点赞数:</span> {previewLink.likes}</div>
                      <div><span className="font-medium">浏览量:</span> {previewLink.views}</div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {previewLink.ai_tags?.tags?.map((tag, index) => (
                      <span 
                        key={index}
                  ...
                  {previewLink.ai_tags?.confidence && (
                    <div className="mt-2 text-sm text-gray-600">
                      置信度: {(previewLink.ai_tags.confidence * 100).toFixed(2)}%
                    </div>
                  )}
                </div>
