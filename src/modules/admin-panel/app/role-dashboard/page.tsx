...
import { SystemHealthCard, BusinessKpiCard } from '@/components/ui/SystemHealthCards';
...
export default function RoleAwareDashboard() {
  ...
  const systemHealthData = {
    server: {
      ...
      metrics: [
        ...
        { name: '内存使用率', value: '68', unit: '%', status: 'normal' },
        { name: '磁盘使用率', value: '32', unit: '%', status: 'normal' },
    ...
    database: {
      status: 'warning',
      metrics: [
        { name: '连接数', value: '156', unit: '个', status: 'normal' },
        { name: '查询延迟', value: '156', unit: 'ms', status: 'warning' },
        { name: '缓存命中率', value: '87', unit: '%', status: 'normal' },
        { name: '备份状态', value: '正常', unit: '', status: 'normal' }
      ],
      lastUpdated: new Date().toISOString()
    }
  };
  ...
  const analystData = {
    trends: [
      { name: '用户增长趋势', value: '+15%', status: 'success', icon: <TrendingUp /> },
      { name: '收入增长率', value: '+8%', status: 'warning', icon: <TrendingUp /> },
      { name: '市场占有率', value: '12.5%', status: 'info', icon: <BarChart3 /> },
      { name: '用户留存率', value: '78%', status: 'success', icon: <Users /> }
    ],
    forecasts: [
      { metric: '下季度收入预测', value: '¥4,200,000', confidence: '85%' },
  ...
  return (
    <div className="space-y-6">
      ...
      <RoleGuard roles={['admin']}>
        <div className="space-y-6">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 mb-4">系统健康监控</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <SystemHealthCard
                ...
                onRefresh={() => console.log('刷新数据库状态')}
              />
            </div>
          </div>
          
          <div>
            <h2 className="text-lg font-semibold text-gray-900 mb-4">实时告警</h2>
            <AlertsList 
              alerts={alerts}
              onAlertClick={(id) => console.log('查看告警:', id)}
            />
          </div>
        </div>
      </RoleGuard>
      ...
      <RoleGuard roles={['content_reviewer', 'shop_reviewer', 'finance']}>
        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-4">业务关键指标</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <BusinessKpiCard
      ...
      <RoleGuard roles={['content_reviewer', 'shop_reviewer']}>
        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-4">数据分析与趋势</h2>
                ...
                {analystData.forecasts.map((forecast, index) => (
                  <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-gray-500">置信度: {forecast.confidence}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <RoleGuard roles={['viewer']}>
        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-4">租户运营指标</h2>
          <BusinessKpiCard
