...
const FcxExchangePage = () => {
  ...
  const addToCart = (part: Part) => {
    ...
    if (existingItem) {
      ...
      setCart(cart.map(item => 
        item.id === part.id 
          ? { ...item, quantity: item.quantity + 1, subtotal: (item.quantity + 1) * part.fcx_price }
          : item
      ));
    } else {
  ...
  const getOrderStatusBadge = (status: string) => {
    const statusMap: Record<string, { color: string; icon: React.ReactNode }> = {
      pending: { color: 'bg-yellow-100 text-yellow-800', icon: <Package className="w-4 h-4 mr-1" /> },
      confirmed: { color: 'bg-blue-100 text-blue-800', icon: <CheckCircle className="w-4 h-4 mr-1" /> },
      processing: { color: 'bg-purple-100 text-purple-800', icon: <Truck className="w-4 h-4 mr-1" /> },
      ...
      failed: { color: 'bg-red-100 text-red-800', icon: <XCircle className="w-4 h-4 mr-1" /> }
    };
            ...
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{userFcxBalance.toLocaleString()}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-500">{part.brand} {part.model}</p>
                            <p className="text-sm text-gray-500">型号: {part.part_number}</p>
                            ...
                            <div className="mt-2 flex items-center justify-between">
                              <div>
                                ...
                                <span className="text-sm text-gray-500 ml-2">/{part.unit}</span>
                              </div>
                              
                              <div className="text-right">
                                <p className="text-sm text-gray-500">库存: {part.stock_quantity}</p>
                                <Button
                                  size="sm"
                  ...
                  <div className="space-y-3 mb-4">
                    {cart.map(item => (
                      <div key={item.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-medium text-sm">{item.subtotal.toLocaleString()} FCX</p>
                          <Button
                            size="icon"
                            variant="ghost"
                    ...
                    {!canCheckout() && cart.length > 0 && (
                      <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md">
                        <p className="text-red-700 text-sm">
                          FCX余额不足，当前余额: {userFcxBalance.toLocaleString()} FCX
                        </p>
                      </div>
                    )}
                        ...
                        <div className="space-y-4">
                          <div className="bg-gray-50 p-4 rounded-lg">
                            ...
                            <div className="space-y-1 text-sm">
                              ...
                              <div className="flex justify-between">
                                <span>剩余余额:</span>
                                <span>{(userFcxBalance - calculateCartTotal()).toLocaleString()} FCX</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <Button 
                            ...
                            disabled={!shippingAddress.name || !shippingAddress.phone}
                          >
                            确认兑换
                          </Button>
                        </div>
                ...
                <TableBody>
                  {orders.map(order => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell>{order.estimated_delivery_time} 小时</TableCell>
