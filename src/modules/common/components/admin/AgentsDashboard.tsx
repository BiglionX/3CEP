...
export default function AgentsDashboard() {
  ...
  const loadAgents = async () => {
    try {
      ...
      // 合并注册信息和状态信息
      const mergedAgents: AgentInfo[] = registryData.data.map((reg: AgentRegistration) => {
        const status = statusData.data.find((s: AgentStatus) => s.name === reg.name);
        return {
          registration: reg,
  ...
  const filteredAgents = agents.filter(agent => {
    const matchesSearch = agent.registration.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         agent.registration.domain.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesDomain = !filters.domain || agent.registration.domain === filters.domain;
    const matchesType = !filters.type || agent.registration.type === filters.type;
    const matchesStatus = !filters.status || agent.status.status === filters.status;
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          >
            ...
            {Array.from(new Set(agents.map(a => a.registration.domain))).map(domain => (
          ...
          >
            ...
            {Array.from(new Set(agents.map(a => a.registration.type))).map(type => (
          ...
          >
            ...
            {Array.from(new Set(agents.map(a => a.status.status))).map(status => (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">领域</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">类型</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">状态</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">安全等级</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredAgents.map((agent) => (
                <tr key={agent.registration.name} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(agent.status.status)}`}>
                      {agent.status.status === 'online' ? '在线' : agent.status.status === 'degraded' ? '降级' : '离线'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {getSecurityLevelDisplay(agent.registration.metadata.security_level)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        ...
        {filteredAgents.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            <svg className="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.47.881-6.08 2.32M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
