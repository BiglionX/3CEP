...
const UpgradeRecommendationCard: React.FC<{
  ...
  const getRecommendationLevel = (score: number) => {
    if (score >= 0.8) return { level: '强烈推荐', color: 'text-red-600', bg: 'bg-red-50' };
    if (score >= 0.6) return { level: '推荐', color: 'text-orange-600', bg: 'bg-orange-50' };
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-medium text-blue-800">升级折扣</span>
            </div>
...
const UpgradeRecommendationList: React.FC<UpgradeRecommendationProps> = ({
  ...
  const handleRecommendationClick = async (recommendation: UpgradeRecommendation) => {
    try {
      // 记录点击
    ...
    return (
      <Card className={`${className} border-red-200`}>
        <CardContent className="p-6 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">获取推荐失败</h3>
