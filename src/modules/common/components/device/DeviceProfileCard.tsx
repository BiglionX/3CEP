...
import { DeviceStatus, STATUS_COLORS, DEVICE_STATUS_LABELS } from '@/lib/constants/lifecycle';
...
export default function DeviceProfileCard({ qrcodeId, className = '' }: DeviceProfileCardProps) {
          ...
          <div>
            ...
            <dl className="space-y-2">
              <div>
                ...
                <dd className="text-sm font-medium text-gray-900">{profile.productModel}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-600">品牌</dt>
                <dd className="text-sm font-medium text-gray-900">{profile.brandName || '未知'}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-600">产品类别</dt>
                <dd className="text-sm font-medium text-gray-900">{profile.productCategory || '未分类'}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-600">序列号</dt>
                <dd className="text-sm font-medium text-gray-900">{profile.serialNumber || '无'}</dd>
              </div>
            </dl>
          </div>
          ...
          <div>
            ...
            <dl className="space-y-2">
              <div>
                ...
                <dd className="text-sm font-medium text-gray-900">{formatDate(profile.manufacturingDate)}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-600">首次激活</dt>
                <dd className="text-sm font-medium text-gray-900">{formatDateTime(profile.firstActivatedAt)}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-600">最后事件</dt>
                <dd className="text-sm font-medium text-gray-900">{formatDateTime(profile.lastEventAt)}</dd>
              </div>
              <div>
                <dt className="text-sm text-gray-600">当前位置</dt>
                <dd className="text-sm font-medium text-gray-900">{profile.currentLocation || '未知'}</dd>
              </div>
            </dl>
          </div>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-gray-200">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-3 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{profile.totalRepairCount}</div>
              <div className="text-xs text-blue-500">维修次数</div>
            </div>
            <div className="text-center p-3 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{profile.totalPartReplacementCount}</div>
              <div className="text-xs text-green-500">换件次数</div>
            </div>
            <div className="text-center p-3 bg-purple-50 rounded-lg">
              <div className="text-2xl font-bold text-purple-600">{profile.totalTransferCount}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <dd className="text-sm font-medium text-gray-900">
                {profile.warrantyPeriod ? `${profile.warrantyPeriod}个月` : '无保修信息'}
              </dd>
            </div>
            <div>
              <dt className="text-sm text-gray-600">保修开始</dt>
              <dd className="text-sm font-medium text-gray-900">{formatDate(profile.warrantyStartDate)}</dd>
            </div>
            <div>
              <dt className="text-sm text-gray-600">保修结束</dt>
              <dd className="text-sm font-medium text-gray-900">{formatDate(profile.warrantyExpiry)}</dd>
            </div>
          </div>
        </div>
      </div>
