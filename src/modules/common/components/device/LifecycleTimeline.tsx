...
export default function LifecycleTimeline({ 
  ...
  const getEventDescription = (event: any): string => {
    const baseDescription = DEVICE_EVENT_TYPE_LABELS[event.eventType as DeviceEventType] || event.eventType;
    
    if (event.eventSubtype) {
      if (event.eventType === DeviceEventType.REPAIRED) {
        const repairLabel = REPAIR_TYPE_LABELS[event.eventSubtype as keyof typeof REPAIR_TYPE_LABELS];
    ...
    <div className={`bg-white rounded-lg shadow-md ${className}`}>
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold text-gray-800">生命周期时间轴</h2>
      ...
      <div className="p-6">
        {events.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-sm mt-1">该设备还没有任何生命周期事件</p>
          </div>
        ) : (
              ...
              {events.map((event, index) => (
                <div key={event.id} className="relative flex group">
                  ...
                  <div className="flex flex-col items-center mr-4">
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full ${getEventColor(event.eventType)} text-white shadow-sm group-hover:scale-110 transition-transform`}>
                      <span className="text-lg">{getEventIcon(event.eventType as DeviceEventType)}</span>
                    </div>
