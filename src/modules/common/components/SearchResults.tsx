...
export default function SearchResults({ results, searchTerm, isLoading }: SearchResultsProps) {
  if (isLoading) {
    return (
      ...
      <div className="grid gap-4">
        {results.map((result) => (
          <div key={result.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
            ...
            <a href={result.url} className="text-blue-600 hover:underline text-sm">
              {result.url}
            </a>
          </div>
        ))}
