...
export enum LogisticsCarrier {
  ...
  EMS = "ems", // EMS邮政速递
  YUNDA = "yunda", // 韵达快递
  JD_LOGISTICS = "jd_logistics", // 京东物流
  DHL = "dhl", // DHL国际快递
...
export interface TrackShipmentDTO {
  trackingNumber: string; // 运单号
  carrier?: LogisticsCarrier; // 指定物流商（可选）
  autoDetect?: boolean; // 自动识别物流商
}

// 批量查询请求DTO
export interface BatchTrackDTO {
  trackingNumbers: string[]; // 运单号列表
  carrier?: LogisticsCarrier; // 指定物流商（可选）
}
...
export interface LogisticsTrackingConfig {
  ...
  autoDetectEnabled: boolean; // 是否启用自动识别
  cacheEnabled: boolean; // 是否启用缓存
  cacheTTL: number; // 缓存有效期（秒）
  carriers: CarrierConfig[]; // 物流商配置列表
}

// 轨迹缓存项
export interface TrackingCacheItem {
