...
export class InboundForecastService {
  ...
  async createForecast(
    ...
    try {
      ...
      if (forecastError)
        throw new Error(`创建预报单失败: ${forecastError.message}`);
  ...
  async updateForecastStatus(
    ...
    try {
      ...
      if (!this.isValidStatusTransition(oldStatus, dto.status)) {
        throw new Error(`无效的状态转换: ${oldStatus} -> ${dto.status}`);
      }
  ...
  async getForecastById(id: string): Promise<InboundForecast> {
    try {
      ...
      if (error) throw new Error(`查询预报单失败: ${error.message}`);
      if (!data) throw new Error("预报单不存在");

      return this.mapToInboundForecast(data);
    } catch (error) {
      console.error("获取预报单详情失败:", error);
      throw error;
    }
  }
  ...
  async listForecasts(
    ...
    try {
      ...
      if (error) throw new Error(`查询预报单列表失败: ${error.message}`);

      return data.map(this.mapToInboundForecastListItem);
    } catch (error) {
      console.error("查询预报单列表失败:", error);
      throw error;
    }
  }
  ...
  async getStatusHistory(
    ...
    try {
      ...
      if (error) throw new Error(`查询状态历史失败: ${error.message}`);
  ...
  async handleWMSCallback(callbackData: any): Promise<void> {
    try {
      ...
      // 验证预报单是否存在
      const forecast = await this.getForecastById(noticeId);

      // 更新预报单状态
      const statusMap: Record<string, InboundForecastStatus> = {
      ...
      if (error) throw new Error(`更新预报单状态失败: ${error.message}`);
  ...
  private isValidStatusTransition(
    ...
    > = {
      ...
      [InboundForecastStatus.RECEIVED]: [], // 已收货状态不能变更
      [InboundForecastStatus.CANCELLED]: [], // 已取消状态不能变更
    };
  ...
  private getNotificationContent(type: string, noticeId: string): string {
    return `您的入库预报单(${noticeId})有新的${type}通知`;
  }
}
