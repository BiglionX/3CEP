/**
 * 增强版智能分仓推荐算? * 专门为FCX兑换场景优化的仓库推荐引? */

import { WarehouseRecommendation } from '../../supply-chain/models/inventory.model';
...
interface FcxOptimizedRecommendation extends WarehouseRecommendation {
  fcxDiscountRate: number; // FCX会员折扣?  loyaltyBonus: number; // 忠诚度奖?  priorityScore: number; // 综合优先级得?  realTimeInventory: number; // 实时库存数量
}
...
export class FcxSmartWarehouseRecommender {
    ...
    try {
      ...
      const recommendations = await Promise.all(
        warehouses.map(async warehouse => {
          ...
          // 计算配送时?          const deliveryTime = this.estimateDeliveryTime(
            distance,
            warehouse.type
          );
      ...
      // 3. 排序和过?      const sortedRecommendations = this.rankFcxRecommendations(
        recommendations,
        request
      );

      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`🏪 FCX智能分仓推荐完成，耗时: ${Date.now()- startTime}ms`);
      return sortedRecommendations.slice(0, 8); // 返回?个推?    } catch (error) {
  ...
  private estimateDeliveryTime(
    ...
    // 基础配送时?    let baseTime = distance / 50; // 假设平均速度50km/h

    // 根据仓库类型调整
    switch (warehouseType) {
      case 'domestic':
        baseTime += 2; // 国内仓额?小时处理时间
        break;
      case 'overseas':
        baseTime += 24; // 海外仓额?4小时
        break;
      default:
        baseTime += 4;
    }
   ...
   * 获取实时库存信息
   */
  private async getRealTimeInventory(
    warehouseId: string,
    productIds: string[]
  ): Promise<{
    ...
    try {
      ...
      const availabilityRate =
        (data as any)?.data.length > 0
          ? inStockCount / (data as any)?.data.length
          : 0;
  ...
  private calculateFcxBenefits(
    ...
    // 忠诚度加?    const loyaltyBonus = Math.min(loyaltyScore / 1000, 0.05); // 最?%加成
    ...
    const totalBenefit = totalDiscount + loyaltyBonus * 0.5; // 忠诚度还有额外价?
    return {
  ...
  private calculateCompositeScores(
    ...
    // 优先级得?(用于紧急情况下的排?
    const priorityScore =
      inventoryScore * 0.4 + serviceScoreNum * 0.3 + timeScore * 0.3;
  ...
  private rankFcxRecommendations(
    ...
    return recommendations.sort((a, b) => {
      ...
      if (
        ...
        b.inventoryStatus !== 'out_of_stock'
      )
        return 1;
      if (
        a.inventoryStatus !== 'out_of_stock' &&
      ...
      switch (request.optimizationGoal) {
        ...
        case 'lowest_cost':
          return (
            a.shippingCost * (1 - a.fcxDiscountRate) -
            b.shippingCost * (1 - b.fcxDiscountRate)
          );
        case 'best_service':
