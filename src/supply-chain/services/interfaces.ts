...
export interface IWarehouseService {
   ...
   * 更新仓库信息
   */
  updateWarehouse(warehouseId: string, updates: Partial<CreateWarehouseDTO>): Promise<Warehouse>;
...
export interface IProductService {
   ...
   * 更新商品信息
   */
  updateProduct(productId: string, updates: Partial<CreateProductDTO>): Promise<Product>;
...
export interface IInventoryService {
   ...
   * 获取库存记录
   */
  getInventory(productId: string, warehouseId: string): Promise<InventoryRecord | null>;
   ...
   * 检查库存是否充足
   */
  checkAvailability(productId: string, warehouseId: string, requiredQuantity: number): Promise<boolean>;
   ...
   * 生成库存预警
   */
  generateLowStockAlerts(): Promise<Array<{
    productId: string;
    productName: string;
    warehouseId: string;
   ...
   * 智能补货建议
   */
  getReplenishmentSuggestions(): Promise<Array<{
    productId: string;
    productName: string;
...
export interface ISupplierService {
   ...
   * 审核供应商申请
   */
  reviewApplication(applicationId: string, decision: 'approve' | 'reject', remarks?: string): Promise<void>;
...
export interface IPurchaseOrderService {
   ...
   * 创建采购订单
   */
  createPurchaseOrder(items: Array<{
    productId: string;
    quantity: number;
    supplierId: string;
    unitPrice: number;
  }>, warehouseId: string): Promise<PurchaseOrder>;
   ...
   * 更新订单状态
   */
  updateOrderStatus(orderId: string, status: string, remarks?: string): Promise<PurchaseOrder>;
...
export interface IRecommendationService {
   ...
   * 智能分仓推荐
   */
  recommendWarehouses(userLocation: { lat: number; lng: number }, productIds: string[]): Promise<WarehouseRecommendation[]>;
  
  /**
   * 基于销量的补货建议
   */
  getSalesBasedReplenishment(productId: string, periodDays?: number): Promise<number>;
  
  /**
   * 供应商智能匹配
   */
  matchSuppliers(productRequirements: Array<{
    productId: string;
    quantity: number;
   ...
   * 库存优化建议
   */
  getInventoryOptimizationSuggestions(warehouseId: string): Promise<Array<{
    productId: string;
    currentStock: number;
