...
export class DemandForecastService {
  ...
  async predictDemand(
    ...
    try {
      ...
      if (historicalData.length < 30) {
        throw new Error('历史数据不足，至少需要30天的数据进行预测');
      }

      // 2. 数据预处理
      const timeSeriesData = this.preprocessData(historicalData);
      ...
      if (algorithm === 'arima') {
        ...
        forecastResult = await this.prophetForecast(timeSeriesData, horizonDays);
      }

      // 4. 分析趋势和季节性
      const trendAnalysis = this.analyzeTrend(timeSeriesData);
  ...
  private async getHistoricalSalesData(
    ...
    try {
      ...
      // 转换为数组格式
      return Array.from(dailySales.entries()).map(([dateStr, quantity]) => ({
      ...
      console.error('获取历史销售数据错误:', error);
      // 返回模拟数据用于测试
  ...
  private preprocessData(historicalData: SalesHistoryRecord[]): TimeSeriesData {
    ...
    // 提取日期和数值
    const dates = sortedData.map(record => record.date);
  ...
  private async arimaForecast(
    ...
    try {
      ...
      // 预测值
      const forecastMean = movingAverage * (1 + trend * horizon / 30);
      ...
      // 降级到简单移动平均
      return this.simpleMovingAverageForecast(data, horizon);
    }
  }
  ...
  private async prophetForecast(
    ...
    try {
      ...
      // 综合预测
      const baseForecast = trendComponent.forecast + seasonalComponent.average + holidayEffect;
      
      // 添加置信区间
      const residuals = values.map((val, i) => val - (trendComponent.values[i] + seasonalComponent.values[i]));
   ...
   * 趋势分析
   */
  private analyzeTrend(data: TimeSeriesData): { trend: 'increasing' | 'decreasing' | 'stable' } {
  ...
  private detectSeasonalPatterns(data: TimeSeriesData): number[] {
    ...
    // 检测月季节性
    const monthlyPattern = this.calculateMonthlySeasonality(dates, values);
  ...
  private async analyzeExternalFactors(
    ...
    if (holidays.length > 0) {
      factors.push({
        factor: '节假日效应',
        impact: 0.15, // 预期增加15%需求
        confidence: 0.8
      });
    }

    // 检查促销活动
    const promotions = await this.getActivePromotions(productId, startDate, endDate);
    if (promotions.length > 0) {
      factors.push({
        factor: '促销活动',
        impact: 0.25, // 预期增加25%需求
        confidence: 0.9
      });
    }
  ...
  private simpleMovingAverageForecast(data: TimeSeriesData, horizon: number): ForecastResult {
  ...
  private extractTrend(values: number[]): { values: number[]; forecast: number } {
    const trend = this.calculateTrend(values);
    const trendValues = values.map((val, i) => val * (1 + trend * i / values.length));
    const forecast = values[values.length - 1] * (1 + trend);
    
    return { values: trendValues, forecast };
  }

  private extractSeasonal(dates: Date[], values: number[]): { values: number[]; average: number } {
    // 简化的季节性提取
    ...
    const normalizedFactors = seasonalFactors.map(factor => factor / overallAverage);
  ...
  private async calculateHolidayEffect(dates: Date[]): Promise<number> {
    ...
    const hasHoliday = dates.some(date => {
      ...
      // 简单的节假日检查（春节、国庆等）
      return (month === 0 && day === 1) || // 元旦
             (month === 9 && day >= 1 && day <= 7); // 国庆节
    });
    
    return hasHoliday ? 50 : 0; // 节假日额外需求
  }

  private calculateWeeklySeasonality(dates: Date[], values: number[]): number[] {
  ...
  private calculateMonthlySeasonality(dates: Date[], values: number[]): number[] {
  ...
  private async getUpcomingHolidays(startDate: Date, endDate: Date): Promise<any[]> {
    // 模拟节假日数据
    return [];
  }

  private async getActivePromotions(productId: string, startDate: Date, endDate: Date): Promise<any[]> {
    // 模拟促销数据
    return [];
  }
