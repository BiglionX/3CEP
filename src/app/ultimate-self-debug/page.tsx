...
export default function UltimateSelfDebug() {
  ...
  const [results, setResults] = useState<Array<{phase: number; message: string; status: string; timestamp: string}>>([]);
  ...
  const runCompleteDiagnostic = async () => {
    ...
    try {
      ...
      if (typeof window === 'undefined') {
        log(1, '❌ 运行在服务器端，无法进行客户端测试', 'error');
        return;
      }
      log(1, '✅ 客户端环境正常', 'success');
      ...
      log(2, `登录成功 - 用户ID: ${loginResult.user?.id?.substring(0,8)}...`, 'success');
      log(2, `管理员状态: ${loginResult.user?.is_admin}`, 'data');
      log(2, `会话令牌: ${loginResult.session?.access_token ? '✓' : '✗'}`, 'data');
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            log(3, '✅ Next.js路由跳转指令已发送', 'success');
          } catch (error: unknown) {
            const errorMessage = error instanceof Error ? error.message : String(error);
      ...
      setTimeout(() => {
        ...
        try {
          ...
          log(3, '✅ Window.location跳转已执行', 'success');
        } catch (error: unknown) {
          const errorMessage = error instanceof Error ? error.message : String(error);
    ...
    <div className="min-h-screen bg-gray-900 text-white py-8">
      <div className="max-w-6xl mx-auto px-4">
        <div className="bg-gray-800 rounded-lg shadow-2xl p-6">
          <h1 className="text-3xl font-bold text-center mb-6 text-red-400">终极自我调试工具</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="lg:col-span-2 bg-gray-700 p-6 rounded-lg">
              ...
              <div className="text-sm text-gray-300">
                ...
                <p>• 包括Next.js路由、Window.location等多种方法</p>
                <p>• 会详细记录每一步的执行结果</p>
              </div>
            </div>
          ...
          <div className="mt-6 bg-red-900/30 border border-red-700 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <h4 className="font-medium text-red-300 mb-2">如果所有测试都失败：</h4>
