...
function ToastDemo() {
  ...
  const showSuccess = (title: string, message: string) => showToast(message, { type: FeedbackType.SUCCESS, title })
  const showError = (title: string, message: string) => showToast(message, { type: FeedbackType.ERROR, title })
  const showWarning = (title: string, message: string) => showToast(message, { type: FeedbackType.WARNING, title })
  const showInfo = (title: string, message: string) => showToast(message, { type: FeedbackType.INFO, title })
  const showLoading = (message: string) => showToast(message, { type: FeedbackType.LOADING, duration: 0 })
      ...
      <CardContent className="space-y-4">"
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">"
          ...
          <Button 
            ...
            size="sm"
          >
            <Loader2 className={`w-4 h-4 mr-2 ${loadingToastVisible  'animate-spin' : ''}`} />
      ...
      },
{
    ...
    showSuccess('批量删除完成', `成功删除 ${result.filter(r => r.success).length}/${result.length} 个文件`)
  }
  ...
  return ("
    <div className="max-w-6xl mx-auto p-6">"
      <div className="mb-8">"
        ...
        <p className="text-gray-600">展示Toast通知、确认对话框和批量操作反馈功能</p>
      </div>
"
