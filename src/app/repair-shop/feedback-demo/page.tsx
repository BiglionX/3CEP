...
function ToastDemo() {
  ...
  const handleWarningToast = () => {
    showWarning('请注?, '此操作不可撤销，请谨慎操作')
  }

  const handleInfoToast = () => {
    showInfo('系统提示', '新版本功能现已可?)
  }
  ...
  const handleCustomToast = () => {
    showSuccess('自定义图?, '这条消息使用了自定义图标', {
      icon: <Send className="w-4 h-4 text-purple-500" />,
      duration: 6000,
      action: {
        label: '撤回',
        onClick: () => // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('消息已撤?)}
    })
  }
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <Button 
            ...
            variant={loadingToastId ? "destructive" : "outline"} 
            size="sm"
          >
            <Loader2 className={`w-4 h-4 mr-2 ${loadingToastId ? 'animate-spin' : ''}`} />
            {loadingToastId ? '关闭加载' : '加载通知'}
          </Button>
...
function ConfirmDemo() {
  ...
  const handleDeleteConfirm = async () => {
    const confirmed = await showConfirm.confirm({
      ...
      onConfirm: async () => {
        ...
        // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('数据已删?)}
    })
    
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('用户选择:', confirmed ? '确认删除' : '取消删除')}
  ...
  const handleSaveConfirm = async () => {
    const confirmed = await showConfirm.confirm({
      ...
      onConfirm: async () => {
        ...
        // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('更改已保?)},
      onCancel: () => {
        // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('放弃更改')}
    })
  }
  ...
  const handleDangerousAction = async () => {
    ...
    if (confirmed) {
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('执行危险操作')}
  }
...
function FormDemo() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess('提交成功', '您的表单已成功提交，我们会尽快回复您')
...
function BatchOperationDemo() {
  ...
  const handleBatchProcess = async () => {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('批量处理结果:', result)}

  const handleBatchDelete = async () => {
    const result = await withBatchFeedback(
      items.slice(0, 3), // 只处理前3个项?
      async (item) => {
        // 模拟删除操作
        await new Promise(resolve => setTimeout(resolve, 800))
      },
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="bg-gray-50 rounded-lg p-4">
          <h4 className="font-medium mb-2">待处理项目列?/h4>
...
function FeedbackDemo() {
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">操作反馈系统演示</h1>
        <p className="text-gray-600">展示Toast通知、确认对话框和批量操作反馈功?/p>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              ...
              <div className="text-sm text-gray-600">确认对话?/div>
