...
function RepairShopPageContent() {
  ...
  // 根据模式选择数据?  const currentData: RepairShop[] = enableInfiniteScroll
    ? infiniteData
    : paginatedData?.data || [];
  ...
  // 包装刷新函数
  const handleRefresh = () => {
    clearErrors(); // 清除之前的错?    if (enableInfiniteScroll) {
      refreshInfinite();
    } else {
      refetchPaginated();
    }
        ...
        {hasCriticalError && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-medium text-red-800 mb-2">遇到一些问?/h3>
          ...
          <div className="py-8">
            ...
            {!hasNextPage && currentData.length > 0 && (
              <div className="text-center py-6 text-gray-500">
                <p>已加载全?{currentData.length} 家店?/p>
              </div>
            )}
        ...
        {!enableInfiniteScroll && stats.totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t mt-8">
            <div className="text-sm text-gray-700">
              显示?1 �?{Math.min(20, stats.total)} 条，�?{stats.total}{' '}
              条记?            </div>
            <div className="flex gap-2">
  ...
  return (
    <EnhancedErrorBoundary
      onError={(error, errorInfo) => {
        console.error('维修店页面错?', error, errorInfo);
      }}
      showDetails={false}
    >
