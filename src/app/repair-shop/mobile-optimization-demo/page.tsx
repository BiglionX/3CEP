...
const menuItems = [
  { id: 1, title: '个人资料', subtitle: '查看和编辑个人信息', icon: User },
  { id: 2, title: '设置', subtitle: '应用设置和偏好', icon: Settings },
...
function ScreenInfoDisplay() {
  ...
  const DeviceIcon = deviceIcons[screenInfo.deviceType] || Monitor;
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-purple-50 p-3 rounded-lg">
          ...
          <div className="text-lg font-semibold text-purple-600 flex items-center gap-2">
            ...
            {deviceLabels[screenInfo.deviceType] || '未知'}
          </div>
        </div>
          ...
          {[1, 2, 3, 4, 5, 6].map(item => (
            <MobileCard key={item} elevated>
              <div className="p-4">
                ...
                <p className="text-gray-500 text-sm">响应式网格项目描述</p>
              </div>
            </MobileCard>
          ))}
        </ResponsiveGrid>
...
// 响应式隐藏演示
function ResponsiveHideDemo() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>响应式显示控制</CardTitle>
        <CardDescription>根据设备类型控制元素的显示/隐藏</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="border rounded-lg p-4">
...
// 主演示组件
function MobileOptimizationDemo() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="md:hidden">
        <MobileHeader title="响应式优化演示" showBorder={false} />
      </div>
      ...
      <ResponsiveContainer maxWidth="desktop" className="py-6 md:pt-6">
        <div className="mb-8">
          ...
          <p className="text-gray-600">精细化断点设置和移动端专属组件演示</p>
        </div>
