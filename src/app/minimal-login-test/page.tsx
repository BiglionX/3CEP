...
export default function MinimalLoginTest() {
  ...
  const handleMinimalLogin = async () => {
    try {
      ...
      if (response.ok) {
        setTestResult(`✅ 登录成功! 用户: ${result.user?.email}, 管理员: ${result.user?.is_admin}`);
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-2xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">最小化登录跳转测试</h1>
          
          <div className="space-y-6">
            <div className="bg-blue-50 p-6 rounded-lg">
              <h2 className="text-xl font-semibold text-blue-800 mb-4">测试控制面板</h2>
            ...
            {testResult && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
                ...
                <p className="text-yellow-700 whitespace-pre-wrap">{testResult}</p>
              </div>
            )}
            ...
            <div className="bg-gray-50 p-6 rounded-lg">
              ...
              <div className="text-sm text-gray-600 space-y-2">
                <p><strong>当前路径:</strong> {typeof window !== 'undefined' ? window.location.pathname : 'N/A'}</p>
                <p><strong>当前主机:</strong> {typeof window !== 'undefined' ? window.location.host : 'N/A'}</p>
