      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-900">待审核文档</h2>
          </div>

          {reviews.length === 0 ? (
            <div className="text-center py-12">
              <AlertCircle className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <h3 className="text-lg font-medium text-gray-900 mb-2">暂无待审核文档</h3>
              <p className="text-gray-500">所有文档都已处理完毕</p>
            </div>
          ) : (
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-gray-600">{selectedReview.document?.title}</p>
                </div>
                
                <div>
                  <h4 className="font-medium text-gray-900">分类</h4>
                  <p className="text-gray-600">{selectedReview.document?.category}</p>
                </div>
                
                <div>
                  <h4 className="font-medium text-gray-900">语言</h4>
                  <p className="text-gray-600">{selectedReview.document?.language}</p>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-4 rounded mt-2 max-h-40 overflow-y-auto">
                    <p className="text-gray-600 text-sm whitespace-pre-wrap">
                      {selectedReview.document?.content.substring(0, 500)}...
                    </p>
                  </div>
                </div>
              </div>
      ...
      {showAssignModal && selectedReview && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full">
            <div className="p-6 border-b">
              <h3 className="text-xl font-semibold">分配审核员</h3>
            </div>
                ...
                >
                  ...
                  {reviewers.map((reviewer) => (
                    <option key={reviewer.id} value={reviewer.user_id}>
                      {reviewer.user?.user_metadata?.name || reviewer.user?.email}
                    </option>
                  ))}
                </select>
              </div>
