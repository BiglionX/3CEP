...
export default function FaultsDictPage() {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      'download',
      `faults_${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  const importFromCSV = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = e => {
      ...
      // 跳过标题行，处理数据源
      const importedFaults: Fault[] = lines
        .slice(1)
        .map((line, index) => {
          ...
          return {
            ...
            difficulty: (values[2].trim() || '中等') as
              | '简单'
              | '中等'
              | '困难',
    ...
    <div className="space-y-6">
            ...
            {filteredFaults.length === 0 ? (
              ...
              filteredFaults.map(fault => (
                <TableRow key={fault.id}>
                  ...
                  <TableCell>
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDifficultyColor(fault.difficulty)}`}
                    >
                      {fault.difficulty}
                    </span>
                  </TableCell>
      ...
      <div className="bg-white shadow rounded-lg p-4">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div>
            <p className="text-sm text-gray-600">简单难度</p>
            <p className="text-2xl font-bold text-green-600">
              {faults.filter(f => f.difficulty === '简单').length}
            </p>
          </div>
          <div>
