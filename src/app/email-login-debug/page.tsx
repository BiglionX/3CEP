...
export default function EmailLoginJumpDebug() {
  ...
  const [debugLogs, setDebugLogs] = useState<
    Array<{
      timestamp: string;
      message: string;
      type: 'info' | 'error' | 'success' | 'data' | 'start' | 'end';
    }>
  >([]);
  ...
  const log = (
    message: string,
    type: 'info' | 'error' | 'success' | 'data' | 'start' | 'end' = 'info'
  ) => {
    const logEntry = {
  ...
  const testEmailLoginJump = async () => {
    ...
    try {
      ...
      log(`响应状 ${response.status} ${response.statusText}`, 'data');
      ...
      log('4. 解析响应数据', 'info');
      log(`用户ID: ${result.id}`, 'data');
      log(`用户邮箱: ${result.email}`, 'data');
      log(`是否管理员 ${result.is_admin}`, 'data');
      log(`会话令牌长度: ${result.access_token.length || 0}`, 'data');
      ...
      const simulateOriginalJump = () => {
        ...
        if (result.is_admin) {
          ...
          try {
            ...
            log('Next.js路由跳转指令已发, 'success');
          } catch (routerError: unknown) {
            log(
              `Next.js路由跳转失败: ${(routerError as Error).message}`,
              'error'
            );
            ...
            try {
              ...
              log('window.location跳转已执, 'success');
            } catch (locationError: unknown) {
              log(
                `window.location跳转失败: ${(locationError as Error).message}`,
                'error'
              );
              ...
              setTimeout(() => {
                try {
                  window.location.href = targetRedirect;
                  log('延迟window.location跳转已执, 'success');
                } catch (delayError: unknown) {
                  log(
                    `延迟跳转失败: ${(delayError as Error).message}`,
                    'error'
                  );
                }
              }, 1000);
          ...
          try {
            ...
            log(`普通用户跳转失 ${(error as Error).message}`, 'error');
          }
        }
      };
