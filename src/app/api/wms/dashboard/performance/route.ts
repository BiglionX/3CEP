/**
 * WMS鏁堣兘鍒嗘瀽鐪嬫澘API鎺ュ彛
 * 鎻愪緵浠撳簱杩愯惀KPI鏁版嵁銆佽秼鍔垮垎鏋愬拰鍛婅淇℃伅
 */
...
export async function GET(request: Request) {
  try {
    ...
    // 鏃堕棿鑼冨洿绛?    if (startDateParam || endDateParam) {
      filters.dateRange = {
    ...
    // 濡傛灉娌℃湁璁剧疆鏃堕棿鑼冨洿锛屼娇鐢ㄩ粯璁ゅ€硷紙鏈€?0澶╋級
...
export async function POST(request: Request) {
  try {
    ...
    // 楠岃瘉璇锋眰?    const { filters } = body;

    if (!filters) {
      return NextResponse.json({ error: '缂哄皯杩囨护鏉′欢鍙傛暟' }, { status: 400 });
    }

    // 楠岃瘉蹇呴渶鐨勬椂闂磋寖鍥村弬?    if (!filters.dateRange) {
      return NextResponse.json(
        { error: '蹇呴』鎻愪緵鏃堕棿鑼冨洿鍙傛暟' },
        { status: 400 }
      );
    }
