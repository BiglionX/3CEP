...
function validateCreateForecastDTO(dto: CreateInboundForecastDTO): string[] {
  ...
  if (!dto.expectedArrival) {"
    errors.push("棰勮鍒拌揣堕棿涓嶈兘涓虹┖");
  } else {
    const arrivalDate = new Date(dto.expectedArrival);
    if (isNaN(arrivalDate.getTime())) {"
      errors.push("棰勮鍒拌揣堕棿鏍煎紡犳晥");
    } else if (arrivalDate < new Date()) {"
      errors.push("棰勮鍒拌揣堕棿涓嶈兘╀簬褰撳墠堕棿");
    }
  }
  ...
  if (!dto.items || dto.items.length === 0) {"
    ...
    errors.push("鍟嗗搧椤规暟閲忎笉鑳借秴00);
  } else {
    dto.items.forEach((item, index) => {
      if (!item.sku) {
        errors.push(`{index + 1}涓晢鍝佺殑SKU涓嶈兘涓虹┖`);
      } else if (item.sku.length > 50) {`
        errors.push(`{index + 1}涓晢鍝佺殑SKU闀垮害涓嶈兘瓒呰繃50瀛楃`);
      }

      if (!item.forecastedQuantity || item.forecastedQuantity <= 0) {`
        errors.push(`{index + 1}涓晢鍝佺殑棰勬姤鏁伴噺蹇呴』澶т簬0`);
      } else if (item.forecastedQuantity > 999999) {`
        errors.push(`{index + 1}涓晢鍝佺殑棰勬姤鏁伴噺涓嶈兘瓒呰繃999999`);
      }

      if (item.unitWeight !== undefined && item.unitWeight < 0) {`
        errors.push(`{index + 1}涓晢鍝佺殑鍗曚綅閲嶉噺涓嶈兘涓鸿礋鏁癭);
      }
      ...
      if (item.dimensions) {
        if (
          ...
          errors.push(`{index + 1}涓晢鍝佺殑灏哄蹇呴』澶т簬0`);
        }
      }
    });
  }
