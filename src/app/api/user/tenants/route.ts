...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 瑙ｆ瀽璇眰    const { tenantId } = await request.json();
    ...
    if (checkError || !userTenant) {
      return NextResponse.json({ error: '犳潈璁块棶璇ョ }, { status: 403 });
    }
    ...
    // 璁剧疆绉熸埛 cookie锛屾湁鏁堟湡 7     response.cookies.set('current-tenant-id', tenantId, {
