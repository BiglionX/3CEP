...
// 閰嶄欢牸鎵归噺鏌ヨ鍜屽疄舵洿export async function POST(request: Request) {
  try {
    ...
    for (const partId of part_ids) {
      try {
        ...
        console.warn(`鑾峰彇閰嶄欢 ${partId} 牸澶辫触:`, error);
...
async function getCachedPrice(partId: string) {
  try {
    // 鐢熸垚缂撳    const cacheKey = generateCacheKey('part_price', partId);

    // 灏濊瘯嶳edis鑾峰彇
    const cached = await cacheManager.get(cacheKey);
    if (cached) {
      console.log(`馃摝 Redis缂撳鍛戒腑: ${partId}`);
      return cached;
    }
    ...
    // 瀛樺叆Redis缂撳
    await cacheManager.set(cacheKey, result, 3600); // 1灏忔椂缂撳

    return result;
  } catch (error) {
    console.warn('鑾峰彇缂撳牸澶辫触:', error);
    return null;
  }
}
...
async function fetchRealTimePrice(partId: string) {
  try {
    ...
    if (partError || !partInfo) {
      throw new Error('閰嶄欢淇℃伅涓嶅);
    }

    // 妯℃嫙庡涓數鍟嗗钩鍙拌幏鍙栦环    const platforms = ['娣樺疂', '浜笢', '鎷煎, '澶╃尗'];
    const prices = [];
    let lowestPrice = Infinity;

    for (const platform of platforms) {
      // 妯℃嫙牸鑾峰彇 - 瀹為檯搴旇璋冪敤鐪熷疄鐨勭數鍟嗗钩鍙癆PI
      ...
      if (simulatedPrice > 0) {
        prices.push({
          ...
          url: `https://${platform.toLowerCase()}.com/searchq=${encodeURIComponent(
            partInfo.name
          )}`,
          seller: `${platform}瀹樻柟楄埌搴梎,
    ...
    console.error('鑾峰彇瀹炴椂牸澶辫触:', error);
    throw error;
  }
}
...
async function updatePriceCache(
  ...
  try {
    ...
    // 璁板綍缂撳鏇存柊ュ織
    console.log(`馃捑 牸缂撳宸叉洿 ${partId}`);
  } catch (error) {
    console.warn('鏇存柊牸缂撳寮傚父:', error);
  }
}
