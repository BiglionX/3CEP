...
async function demonstrateAsyncProcessing() {
  ...
  demoResults.capabilities.push({
    name: '甯傚満鍒嗘瀽寮傛澶勭悊',
    description: '鎵归噺澶勭悊甯傚満浠锋牸瓒嬪娍鍒嗘瀽浠诲姟',
    ...marketDemo,
  });
...
async function demoSupplierAnalysisAsync() {
  const requestData = {
    ...
    requirements: '闇€瑕侀珮璐ㄩ噺鐢靛瓙鍏冨櫒浠朵緵搴斿晢',
...
async function runAsyncTest() {
  ...
  try {
    ...
    testResults.tests.push({
      name: '鐘舵€佹煡璇㈡祴?,
      passed: !!status,
      details: `浠诲姟鐘? ${status ? (status.success ? '鎴愬姛' : '澶勭悊?) : '鏈壘?}`,
    });
