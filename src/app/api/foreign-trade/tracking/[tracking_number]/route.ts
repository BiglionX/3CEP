  ...
  try {
    ...
    const response = {
      ...
      data: {
        ...
        timeline: timeline || [],
        documents: documents || [],
        estimated_delivery_days: calculateEstimatedDays(shipment.planned_departure, shipment.estimated_arrival)
      }
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (status !== shipment.status) {
      await supabase
        ...
        .update({ 
          ...
          ...(status === 'in_transit' && !shipment.actual_departure ? { actual_departure: new Date().toISOString() } as any : {}),
          ...(status === 'delivered' && !shipment.actual_arrival ? { actual_arrival: new Date().toISOString() } : {})
        })
        .eq('id', shipment_id)
    }
    ...
    await supabase.from('audit_logs').insert({
      ...
      record_id: data.id,
      details: { tracking_number: shipment.tracking_number, status, location } as any
    })
...
export async function PUT(request: Request, { params }: RouteParams) {
  ...
  try {
    const { tracking_number } = await params
    const body = await request.json()
    const { status, location, description, coordinates, actual_departure, actual_arrival } = body
...
// 计算预计运输天数
function calculateEstimatedDays(plannedDeparture: string, estimatedArrival: string): number {
