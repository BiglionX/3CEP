...
export async function GET(request: Request) {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'appointments':
        csvData = await generateAppointmentsReport(supabase, startDate, endDate)
    ...
    console.error('生成CSV报表失败:', error)
    return NextResponse.json(
      { success: false, error: '生成CSV报表失败' },
      { status: 500 }
    )
  }
}
...
// 生成日报数据
async function generateDailyReport(supabase: any, startDate?: string | null, endDate?: string | null) {
  // 设置默认时间范围（最近7天）
  ...
  // 处理数据并生成CSV
  const rows = []
  rows.push(['日期', '新增热点链接', '新增文章', '新增预约', '新增店铺', '待审核链接'])
  ...
  // 统计热点链接
  hotLinksData.data?.forEach((item: any) => {
  ...
  // 统计文章
  articlesData.data?.forEach((item: any) => {
    const dateKey = item.created_at.split('T')[0]
    if (dateMap.has(dateKey)) {
  ...
  // 统计预约
  appointmentsData.data?.forEach((item: any) => {
    const dateKey = item.created_at.split('T')[0]
    if (dateMap.has(dateKey)) {
  ...
  // 统计店铺
  shopsData.data?.forEach((item: any) => {
    const dateKey = item.created_at.split('T')[0]
    if (dateMap.has(dateKey)) {
...
// 生成热点链接详细报表
async function generateHotLinksReport(supabase: any, startDate?: string | null, endDate?: string | null) {
  const end = endDate ? new Date(endDate) : new Date()
  ...
  const rows = []
  rows.push(['标题', '链接', '来源', '分类', '点赞数', '浏览量', '状态', '创建时间'])
...
// 生成预约详细报表
async function generateAppointmentsReport(supabase: any, startDate?: string | null, endDate?: string | null) {
  const end = endDate ? new Date(endDate) : new Date()
  const start = startDate ? new Date(startDate) : new Date(end.getTime() - 30 * 24 * 60 * 60 * 1000)
