...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        overview: {
          totalAgents: totalStats?.length || 0,
          onShelfAgents:
            totalStats?.filter(a => a.shelf_status === 'on_shelf').length || 0,
          approvedAgents:
            totalStats?.filter(a => a.review_status === 'approved').length || 0,
          pendingReview:
            totalStats?.filter(a => a.review_status === 'pending').length || 0,
        },
        sales: {
    ...
    console.error('智能体商店统计 API 错误:', error);
    return NextResponse.json(
