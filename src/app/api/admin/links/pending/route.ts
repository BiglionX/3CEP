...
// GET - 鑾峰彇寰呭鏍搁摼鎺ュ垪?export async function GET(request: Request) {
  try {
...
// POST - 鎵归噺鎿嶄綔锛堝彂甯冩垨椹冲洖?export async function POST(request: Request) {
  try {
    ...
    switch (action) {
      case 'publish':
        // 鍙戝竷鎿嶄綔锛氬垱寤烘枃绔犺崏绋垮苟鏇存柊閾炬帴鐘?        updateData.status = 'promoted';
