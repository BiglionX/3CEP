...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('鑾峰彇浠撳簱浣嶇疆澶辫触:', error);
    return NextResponse.json(
      { success: false, error: '鑾峰彇浠撳簱浣嶇疆澶辫触', data: [] },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: newLocation,
      message: '浠撳簱浣嶇疆鍒涘缓鎴愬姛',
    });
  } catch (error) {
    console.error('鍒涘缓浠撳簱浣嶇疆澶辫触:', error);
    return NextResponse.json(
      { success: false, error: '鍒涘缓浠撳簱浣嶇疆澶辫触' },
      { status: 500 }
    );
  }
}
