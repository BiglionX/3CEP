...
export async function POST(request: Request) {
  try {
    ...
    if (!emailRegex.test(email)) {
      return NextResponse.json(
        { success: false, error: '璇疯緭鍏ユ湁鏁堢殑閭鍦板潃' },
        { status: 400 }
      );
    }
    ...
    if (enterpriseError) {
      console.error('鍒涘缓浼佷笟璁板綍澶辫触:', enterpriseError);
      // 濡傛灉浼佷笟璁板綍鍒涘缓澶辫触锛屽垹闄ゅ凡鍒涘缓鐨勭敤?      await supabase.auth.admin.deleteUser(userData.user.id);
      return NextResponse.json(
        { success: false, error: '浼佷笟淇℃伅鍒涘缓澶辫触' },
        { status: 500 }
      );
    }
