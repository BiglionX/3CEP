 ...
 *           type: string
 *   post:
 *     summary: 鍒涘缓鎴栨洿鏂板憡璀﹁? *     description: 閫氳繃POST璇锋眰鍒涘缓鏂扮殑鍛婅瑙勫垯鎴栨洿鏂扮幇鏈夎? *     tags: [鏁版嵁涓績-鐩戞帶鍛婅]
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
export async function GET(request: Request) {
  try {
    ...
    console.log(`馃敂 鍛婅寮曟搸鏀跺埌璇锋眰: action=${action}, ruleId=${ruleId}`);
    ...
    console.error('锟?鍛婅寮曟搸鎵ц澶辫触:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function POST(request: Request) {
  try {
    ...
    console.log('馃摑 鏀跺埌鍛婅瑙勫垯鍒涘缓/鏇存柊璇锋眰:', body);
    ...
    if (action === 'create') {
      ...
      return NextResponse.json(
        { success: false, message: '鏃犳晥鐨勬搷浣滃弬? },
        { status: 400 }
      );
    }
    ...
    console.error('锟?鍛婅瑙勫垯鍒涘缓/鏇存柊澶辫触:', error);
    return NextResponse.json(
      {
        success: false,
...
async function createAlertRule(ruleData: any) {
  ...
  // 妯℃嫙淇濆瓨鍒版暟鎹簱
  console.log(`锟?鍛婅瑙勫垯鍒涘缓鎴愬姛: ${newRule.id}`);
...
async function updateAlertRule(ruleId: string, updates: any) {
  ...
  console.log(`锟?鍛婅瑙勫垯鏇存柊鎴愬姛: ${ruleId}`);
...
async function deleteAlertRule(ruleId: string) {
  console.log(`馃棏锟?鍒犻櫎鍛婅瑙勫垯: ${ruleId}`);
  ...
  const alertHistory = [
    {
      ...
      ruleId: 'rule-001',
      severity: 'critical',
      message: '鏁版嵁搴撹繛鎺ユ暟杈惧埌1050锛岃秴杩囬槇?000',
    ...
    },
    {
      ...
      ruleId: 'rule-002',
      severity: 'high',
      message: 'API鍝嶅簲鏃堕棿杈惧埌2150ms锛岃秴杩囬槇?000ms',
