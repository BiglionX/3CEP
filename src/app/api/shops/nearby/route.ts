...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (error) {
      console.error('查询附近店铺失败:', error)
      return NextResponse.json(
        { error: '查询附近店铺失败', details: error.message },
        { status: 500 }
      )
    }
      ...
      if (shop.latitude && shop.longitude) {
        ...
        if (distance <= radius) {
          shopsWithDistance.push({
            ...
            specialties: shop.specialties ? JSON.parse(shop.specialties as string) : [],
            rating: shop.rating,
    ...
    return NextResponse.json({
      ...
      message: `找到 ${result.length} 家附近的维修店铺`
    })
    
  } catch (error) {
    console.error('附近店铺API错误:', error)
    return NextResponse.json(
      { error: '服务器内部错误' },
      { status: 500 }
    )
