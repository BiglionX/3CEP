...
export default function JumpDebugTool() {
  ...
  const testLoginFlow = async () => {
    ...
    try {
      ...
      if (response.ok) {
        logResult('登录API', '✅', `登录成功 - 用户: ${result.user?.email}, 管理员: ${result.user?.is_admin}`);
    ...
    <div className="min-h-screen bg-gray-900 text-white py-8">
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-gray-800 rounded-lg shadow-2xl p-6">
          <h1 className="text-3xl font-bold text-center mb-6 text-red-400">跳转机制调试工具</h1>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="bg-gray-700 p-4 rounded-lg">
              <h2 className="text-xl font-semibold mb-4 text-blue-400">跳转测试</h2>
              <button
            ...
            <div className="bg-gray-700 p-4 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                <div><strong>步骤:</strong> {debugInfo.step}</div>
                <div><strong>状态:</strong> <span className={debugInfo.status.includes('✅') ? 'text-green-400' : debugInfo.status.includes('❌') ? 'text-red-400' : 'text-yellow-400'}>{debugInfo.status}</span></div>
                <div><strong>详情:</strong> {debugInfo.details}</div>
          ...
          {testResults.length > 0 && (
            <div className="bg-gray-700 rounded-lg p-4 mb-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-green-400">测试历史</h2>
                <button
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {testResults.slice(-10).map((result, index) => (
                  <div key={index} className="flex items-center space-x-3 p-2 bg-gray-600 rounded text-sm">
                    <span className={result.status.includes('✅') ? 'text-green-400' : result.status.includes('❌') ? 'text-red-400' : 'text-yellow-400'}>
                      {result.status}
                    </span>
                    <div className="flex-1">
                      <span className="font-medium">{result.step}:</span> {result.details}
                    </div>
