...
export default function LogisticsPage() {
  ...
  const getStatusIcon = (status: Shipment['status']) => {
    switch (status) {
      ...
      case 'in_transit': return <Ship className="h-4 w-4 text-blue-500" />;
      case 'customs': return <Anchor className="h-4 w-4 text-purple-500" />;
                    ...
                    >
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>
                              {shipment.status === 'delivered' ? '已送达' :
                               ...
                               shipment.status === 'customs' ? '清关中' :
                               shipment.status === 'pending' ? '待发运' : '延误'}
                            </span>
                          </div>
                        </Badge>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>预计: {new Date(shipment.estimatedArrival).toLocaleDateString('zh-CN')}</span>
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        {selectedShipment.vesselName && (
                          <div>
                            ...
                            <div className="font-medium">{selectedShipment.vesselName}</div>
                          </div>
                        )}
                        {selectedShipment.containerNumber && (
                          <div>
                            <span className="text-gray-500">集装箱号:</span>
                            <div className="font-medium">{selectedShipment.containerNumber}</div>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {new Date(selectedShipment.estimatedArrival).toLocaleString('zh-CN')}
                          </span>
                        </div>
                        {selectedShipment.actualArrival && (
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-green-600">
                              {new Date(selectedShipment.actualArrival).toLocaleString('zh-CN')}
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    {/* 物流事件 */}
                    <div>
                      <h3 className="font-medium text-gray-900 mb-3">物流轨迹</h3>
                      <div className="space-y-4">
                        {selectedShipment.events.map((event, index) => (
                          <div key={event.id} className="flex gap-3">
                            ...
                            <div className="flex-1 pb-2">
                              ...
                              <div className="text-sm text-gray-500">
                                {event.location} • {new Date(event.timestamp).toLocaleString('zh-CN')}
                              </div>
                              <div className="text-xs text-gray-400 mt-1">{event.status}</div>
                            </div>
                          </div>
                        ))}
                      </div>
