...
export default function AchievementCenterPage() {
    ...
    setTimeout(() => {
      setAchievements([
        ...
        },
        {
          id: 'ach_010',
          title: '忠实粉丝',
          description: '成为VIP会员满1年',
          icon: <Award className="w-6 h-6" />,
          points: 800,
          category: 'advanced',
          status: 'locked',
          progress: 0,
          requirement: 'VIP会员满1年',
  ...
  const getCategoryCounts = () => {
    return {
      ...
      intermediate: achievements.filter(a => a.category === 'intermediate').length,
  ...
  const getCompletedCounts = () => {
    return {
      ...
      basic: achievements.filter(a => a.category === 'basic' && a.status === 'completed').length,
      intermediate: achievements.filter(a => a.category === 'intermediate' && a.status === 'completed').length,
      advanced: achievements.filter(a => a.category === 'advanced' && a.status === 'completed').length,
      special: achievements.filter(a => a.category === 'special' && a.status === 'completed').length
    }
  }
  ...
  const categories = [
    { value: 'all', label: '全部成就', count: categoryCounts.all, completed: completedCounts.total },
    { value: 'basic', label: '基础成就', count: categoryCounts.basic, completed: completedCounts.basic },
    { value: 'intermediate', label: '中级成就', count: categoryCounts.intermediate, completed: completedCounts.intermediate },
    { value: 'advanced', label: '高级成就', count: categoryCounts.advanced, completed: completedCounts.advanced },
    { value: 'special', label: '特殊成就', count: categoryCounts.special, completed: completedCounts.special }
  ]
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-2xl font-bold text-gray-900">
                  {Math.round((completedCounts.total / categoryCounts.all) * 100)}%
                </p>
            ...
            >
              ...
              <CardContent className="p-6">
                ...
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <Badge 
                    variant={
                      achievement.category === 'special' ? 'default' :
                      achievement.category === 'advanced' ? 'destructive' :
                      achievement.category === 'intermediate' ? 'secondary' : 'outline'
                    }
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {isCompleted && achievement.completedDate && (
                    <div className="flex items-center text-green-600">
                      ...
                      <span>完成于: {formatDate(achievement.completedDate)}</span>
                    </div>
                  )}
                </div>
      ...
      {filteredAchievements.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">暂无此类成就</h3>
