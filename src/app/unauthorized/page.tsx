...
export default function UnauthorizedPage() {
  ...
  useEffect(() => {
    ...
    // 记录错误到控制台（实际项目中应该发送到错误监控服务?    console.warn(`Unauthorized access attempt - Trace ID: ${id}`);
  }, []);

  const requestedPath = searchParams.get('from') || '/';
  const reason = searchParams.get('reason') || '访问被拒?;
  ...
  const handleContactSupport = () => {
    // 实际项目中这里应该打开联系支持的对话框或页?    alert(`请联系系统管理员，提供追踪ID: ${traceId}`);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-50 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="bg-red-500 px-6 py-8 text-center">
          ...
          <h2 className="text-xl text-red-100">访问被拒?/h2>
        </div>
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            ...
            <ul className="space-y-2 text-gray-600">
              <li className="flex items-start">
                ...
                <span>确认您已使用正确的账户登?/span>
              </li>
              <li className="flex items-start">
                <span className="text-green-500 mr-2">�?/span>
                <span>联系系统管理员申请相应权?/span>
              </li>
              <li className="flex items-start">
                <span className="text-green-500 mr-2">�?/span>
                <span>如有疑问，请提供上方的追踪ID给技术支?/span>
              </li>
            </ul>
          </div>
        </div>
