...
export default function ApprovalsPage() {
  ...
  const approvals: ApprovalItem[] = [
    {
      ...
      type: 'order',
      title: '大额采购订单审批',
      description: '向Samsung Electronics采购Galaxy S24 Ultra手机500台，总金额350万元',
      requester: '采购部-张经理',
      department: '采购部',
      priority: 'high',
      status: 'pending',
    ...
    },
    {
      ...
      type: 'contract',
      title: '出口销售合同审批',
      description: '与TechGlobal Ltd.签署华为Mate 60 Pro出口合同，1000台，总金额850万元',
      requester: '销售部-李总监',
      department: '销售部',
      priority: 'high',
      status: 'in_review',
  ...
  const handleApprove = (approvalId: string) => {
    console.log('批准申请:', approvalId)
    // TODO: 实现批准逻辑
  }

  const handleReject = (approvalId: string) => {
    console.log('拒绝申请:', approvalId)
    // TODO: 实现拒绝逻辑
  }
            ...
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center space-x-3">
                <Badge variant="secondary">
                  待处理: {approvals.filter(a => a.status === 'pending').length}项
                </Badge>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-4">
                {filteredApprovals.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">暂无审批事项</h3>
                  ...
                  filteredApprovals.map((approval) => (
                    <div key={approval.id} className="border border-gray-200 rounded-lg p-6 hover:bg-gray-50">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                            ...
                            <span>提交时间: {new Date(approval.submittedAt).toLocaleString('zh-CN')}</span>
                            {approval.deadline && (
                              <span>截止时间: {new Date(approval.deadline).toLocaleString('zh-CN')}</span>
                            )}
                            {approval.amount && (
                              <span>金额: ¥{approval.amount.toLocaleString()}</span>
                            )}
                          </div>
                        </div>
                        
                        <div className="flex flex-col gap-2 ml-4">
                          <Button size="sm" onClick={() => handleViewDetails(approval.id)}>
                            查看详情
                          </Button>
