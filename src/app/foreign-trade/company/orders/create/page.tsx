...
export default function CreateOrderPage() {
  ...
  const [orderItems, setOrderItems] = useState<OrderItem[]>([
    { id: '1', productName: '', sku: '', quantity: 1, unitPrice: 0, totalPrice: 0 }
  ])
  ...
  // 合作伙伴选项
  const partners = formData.type === 'import' 
    ? [
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4 border-t">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <Badge 
                    variant={formData.priority === 'high' ? 'destructive' : 
                           formData.priority === 'medium' ? 'default' : 'secondary'}
                  >
                    {formData.priority === 'high' ? '高' : 
                     formData.priority === 'medium' ? '中' : '低'}优先级
                  </Badge>
                </div>
                
                <div className="text-sm text-gray-600 space-y-1">
                  <p>• 订单类型: {formData.type === 'import' ? '进口采购' : '出口销售'}</p>
