...
export default function ForeignTradeCompanyPage() {
  ...
  const loadTradeData = () => {
    // 模拟加载贸易数据
    const mockOrders: TradeOrder[] = activeRole === 'importer' ? [
      {
        id: 'PO-2026-001',
        type: 'import',
            ...
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center">
                <h1 className="text-xl font-bold text-gray-900">
                  {activeRole === 'importer' ? '进口商业务中心' : '出口商业务中心'}
                </h1>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">
                  {activeRole === 'importer' ? '进口商业务模式' : '出口商业务模式'}
                </h2>
              ...
              <Badge variant={activeRole === 'importer' ? 'default' : 'secondary'}>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-blue-600">{activeRole === 'importer' ? stats.importOrders : stats.exportOrders}</p>
                  </div>
                  <div className="p-3 bg-blue-100 rounded-lg">
                    {activeRole === 'importer' ? 
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {activeRole === 'importer' ? '采购订单跟踪' : '销售订单管理'}
                    </CardTitle>
                    <Button onClick={() => router.push('/foreign-trade/company/orders/create')}>
                    ...
                    {orders.map((order) => (
                      <div 
                        ...
                        onClick={(e) => {
                          ...
                          router.push(`/foreign-trade/company/order/${order.id}`);
                        }}
                        onKeyDown={(e) => {
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            ...
                            <p className="font-medium">
                              {new Date(order.expectedDelivery).toLocaleDateString('zh-CN')}
                            </p>
                          </div>
                        </div>
                      </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {[
                      { name: 'Samsung Electronics', country: '韩国', type: '供应商', status: 'active' },
                      { name: 'Apple Inc.', country: '美国', type: '供应商', status: 'active' },
                      { name: 'TechGlobal Ltd.', country: '美国', type: '客户', status: 'active' },
                      { name: 'Sony Corporation', country: '日本', type: '供应商', status: 'pending' }
                    ].map((partner, index) => (
                      <div key={index} className="border border-gray-200 rounded-lg p-4">
                        ...
                        <p className="text-sm text-gray-600 mb-1">{partner.country}</p>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="text-center p-6 bg-blue-50 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-blue-600">¥{(stats.totalAmount / 10000).toFixed(0)}万</p>
                      <p className="text-sm text-gray-600">总交易额</p>
                    </div>
                    <div className="text-center p-6 bg-green-50 rounded-lg">
                      <CheckCircle className="h-8 w-8 text-green-600 mx-auto mb-2" />
                      <p className="text-2xl font-bold text-green-600">{stats.totalOrders - stats.pendingApprovals}</p>
