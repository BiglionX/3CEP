...
export default function ProcurementDashboardPage() {
  ...
  // 状态标签映射
  const statusBadges: Record<string, { text: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
    pending: { text: '待审批', variant: 'secondary' },
    approved: { text: '已批准', variant: 'default' },
    processing: { text: '处理中', variant: 'default' },
    completed: { text: '已完成', variant: 'default' },
    cancelled: { text: '已取消', variant: 'destructive' }
  }

  // 优先级标签映射
  const priorityBadges: Record<string, { text: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
    low: { text: '低', variant: 'outline' },
    medium: { text: '中', variant: 'secondary' },
    high: { text: '高', variant: 'default' },
    urgent: { text: '紧急', variant: 'destructive' }
  }
  ...
  return (
    <EnterpriseLayout title="采购仪表板">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">合作供应商</CardTitle>
              ...
              <TableBody>
                {filteredOrders.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell>{order.items} 项</TableCell>
                    <TableCell>
