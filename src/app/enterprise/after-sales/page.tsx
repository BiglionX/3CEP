...
产品服务官智能体
        ...
        <div className="border-b border-gray-200 mb-8">
          <nav className="-mb-px flex space-x-8">
            {['dashboard', 'tickets', 'qr-management', 'knowledge-base', 'content-management'].map((tab) => (
              <button
                key={tab}
              ...
              >
                {tab === 'dashboard' && '仪表板'}
                {tab === 'tickets' && '工单管理'}
                {tab === 'qr-management' && '二维码管理'}
                {tab === 'knowledge-base' && '知识库'}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">平均响应时间: 2.5小时</p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">解决率: 94.2%</p>
                </CardContent>
              </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <h3 className="font-semibold text-blue-900 mb-2">设备绑定</h3>
                      <p className="text-blue-700 text-sm">
                        为每台设备生成唯一二维码，便于快速识别和信息查询
                      </p>
                    </div>
                    <Button className="w-full">批量生成二维码</Button>
                  </div>
                  
                  <div className="space-y-4">
                    <div className="bg-green-50 p-4 rounded-lg">
                      <h3 className="font-semibold text-green-900 mb-2">服务记录</h3>
                      <p className="text-green-700 text-sm">
                        扫码查看设备完整服务历史和维护记录
                      </p>
                    </div>
                    <Button variant="outline" className="w-full">查看服务记录</Button>
                  </div>
                  
                  <div className="space-y-4">
                    <div className="bg-purple-50 p-4 rounded-lg">
                      <h3 className="font-semibold text-purple-900 mb-2">保修查询</h3>
                      <p className="text-purple-700 text-sm">
                        快速查询设备保修状态和剩余期限
                      </p>
                    </div>
                    <Button variant="outline" className="w-full">保修查询</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
...
function EnterpriseContentManagement() {
  ...
  const subTabs = [
    ...
    { id: 'manuals', name: '说明书', icon: '📖' },
    { id: 'repair-tips', name: '维修技巧', icon: '🛠️' },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 配件管理 */}
      {activeSubTab === 'parts' && (
        <EnterprisePartsManagement />
      )}

      {/* 说明书管理 */}
      {activeSubTab === 'manuals' && (
        <EnterpriseManualsManagement />
      )}

      {/* 维修技巧管理 */}
      {activeSubTab === 'repair-tips' && (
        <EnterpriseRepairTipsManagement />
      )}

      {/* 软件升级管理 */}
      {activeSubTab === 'software' && (
        <EnterpriseSoftwareUpdatesManagement />
      )}

      {/* 有奖问答管理 */}
      {activeSubTab === 'quiz' && (
        <EnterpriseRewardQuizManagement />
      )}

      {/* 新品众筹管理 */}
      {activeSubTab === 'crowdfunding' && (
        <EnterpriseCrowdfundingManagement />
      )}

      {/* 企业资料管理 */}
      {activeSubTab === 'documents' && (
        <EnterpriseDocumentsManagement />
      )}
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">发布和管理企业配件信息</p>
        </div>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">暂无配件</h3>
              <p className="text-gray-500">开始发布您的第一个配件</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">配件信息</th>
                    ...
                    <tr key={part.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(part.status)}`}>
                          {part.status === 'published' ? '已发布' :
                           part.status === 'pending_review' ? '审核中' :
                           part.status === 'rejected' ? '已拒绝' : '草稿'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button className="text-blue-600 hover:text-blue-900">编辑</button>
                          <button className="text-green-600 hover:text-green-900">查看详情</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-900">发布新配件</h3>
                <button 
                  onClick={() => {
                    setShowCreateModal(false);
                    resetForm();
...
// 说明书管理组件
function EnterpriseManualsManagement() {
  ...
  const fetchManuals = async () => {
    try {
      ...
      if (searchTerm) params.append('search', searchTerm);
      if (statusFilter !== 'all') params.append('status', statusFilter);
      if (productModelFilter !== 'all') params.append('product_model', productModelFilter);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">产品说明书管理</h2>
          <p className="text-gray-600 mt-1">发布和管理多语言产品说明书</p>
        </div>
        <Button onClick={() => setShowCreateModal(true)}>
          <Plus className="w-4 h-4 mr-2" />
          上传说明书
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {manuals.map((manual) => (
                    <tr key={manual.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          <div className="text-sm font-medium text-gray-900">{manual.product_name}</div>
                          <div className="text-sm text-gray-500">{manual.product_model}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {manual.title?.zh || manual.title?.en || '未命名'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex space-x-1">
                          {manual.language_codes?.map((lang: string) => (
                            <span key={lang} className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800">
                              {lang.toUpperCase()}
                            </span>
                          ))}
                        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button className="text-blue-600 hover:text-blue-900">编辑</button>
                          <button className="text-green-600 hover:text-green-900">预览</button>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-900">上传新产品说明书</h3>
                <button 
                  onClick={() => {
                    setShowCreateModal(false);
                    resetForm();
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({...formData, title: {...formData.title, zh: e.target.value}})}
                    placeholder="请输入中文标题"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setFormData({...formData, content: {...formData.content, zh: e.target.value}})}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const codes = e.target.checked 
                            ...
                            : formData.language_codes.filter(code => code !== 'zh');
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const codes = e.target.checked 
                            ...
                            : formData.language_codes.filter(code => code !== 'en');
                          setFormData({...formData, language_codes: codes});
                        }}
                        className="mr-2"
                      />
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const codes = e.target.checked 
                            ...
                            : formData.language_codes.filter(code => code !== 'ja');
                          setFormData({...formData, language_codes: codes});
                        }}
                        className="mr-2"
                      />
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const codes = e.target.checked 
                            ...
                            : formData.language_codes.filter(code => code !== 'ko');
                          setFormData({...formData, language_codes: codes});
                        }}
                        className="mr-2"
                      />
...
// 维修技巧管理组件
function EnterpriseRepairTipsManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">维修技巧管理</h2>
          <p className="text-gray-600 mt-1">发布和管理图文/视频维修技巧</p>
        </div>
        <Button onClick={() => setShowCreateModal(true)}>
          <Plus className="w-4 h-4 mr-2" />
          发布技巧
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">暂无维修技巧</h3>
              <p className="text-gray-500">开始发布您的第一条维修技巧</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">技巧信息</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">类型</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">适用设备</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">难度</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">预估时间</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">状态</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">操作</th>
                  </tr>
                </thead>
                    ...
                    <tr key={tip.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{tip.description?.substring(0, 50)}...</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <span className="inline-flex items-center">
                          <span className="mr-2">{getContentTypeIcon(tip.content_type)}</span>
                          {tip.content_type === 'article' ? '图文' : 
                           tip.content_type === 'video' ? '视频' : '图集'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex flex-wrap gap-1">
                          {tip.device_models?.slice(0, 2).map((model: string, index: number) => (
                            <span key={index} className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800">
                              {model}
                            </span>
                          ))}
                          {tip.device_models?.length > 2 && (
                            <span className="text-xs text-gray-500">+{tip.device_models.length - 2}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {tip.estimated_time ? `${tip.estimated_time}分钟` : '未设定'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(tip.status)}`}>
                          {tip.status === 'published' ? '已发布' :
                           tip.status === 'pending_review' ? '审核中' :
                           tip.status === 'rejected' ? '已拒绝' : '草稿'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button className="text-blue-600 hover:text-blue-900">编辑</button>
                          <button className="text-green-600 hover:text-green-900">预览</button>
                          <button className="text-purple-600 hover:text-purple-900">分享</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-900">发布新维修技巧</h3>
                <button 
                  onClick={() => {
                    setShowCreateModal(false);
                    resetForm();
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <Label htmlFor="title">技巧标题 *</Label>
                    <Input
                      id="title"
                ...
                <div>
                  <Label htmlFor="description">技巧描述</Label>
                  <Textarea
                    id="description"
                    value={formData.description}
                    onChange={(e) => setFormData({...formData, description: e.target.value})}
                    placeholder="请输入技巧详细描述"
                    rows={3}
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({...formData, device_models: e.target.value.split(',').map(item => item.trim()).filter(Boolean)})}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({...formData, fault_types: e.target.value.split(',').map(item => item.trim()).filter(Boolean)})}
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <Button onClick={createRepairTip}>
                    发布技巧
                  </Button>
                </div>
              </div>
...
function EnterpriseSoftwareUpdatesManagement() {
  ...
  const fetchSoftwareUpdates = async () => {
    try {
      ...
      if (searchTerm) params.append('search', searchTerm);
      if (statusFilter !== 'all') params.append('status', statusFilter);
      if (updateTypeFilter !== 'all') params.append('update_type', updateTypeFilter);
      if (productModelFilter !== 'all') params.append('product_model', productModelFilter);
      
      const response = await fetch(`/api/enterprise/software-updates?${params}`);
      const result = await response.json();
      ...
      setError('获取软件升级包列表失败');
      console.error('获取软件升级包列表错误:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 创建软件升级包
  const createSoftwareUpdate = async () => {
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">软件升级包管理</h2>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">暂无软件升级包</h3>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">升级包信息</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">类型</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">版本信息</th>
                    ...
                    <tr key={update.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{update.product_name} {update.product_model}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getUpdateTypeBadge(update.update_type).color}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <div>
                          ...
                          {update.previous_version && (
                            <div className="text-gray-500">从 v{update.previous_version} 升级</div>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {update.file_size ? formatFileSize(update.file_size) : '未知'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {update.release_date || '未设定'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(update.status)}`}>
                          {update.status === 'published' ? '已发布' :
                           update.status === 'pending_review' ? '审核中' :
                           update.status === 'rejected' ? '已拒绝' : '草稿'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <button className="text-blue-600 hover:text-blue-900">编辑</button>
                          <button className="text-green-600 hover:text-green-900">预览</button>
                          <button className="text-purple-600 hover:text-purple-900">下载</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-900">上传新软件升级包</h3>
                <button 
                  onClick={() => {
                    setShowCreateModal(false);
                    resetForm();
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData({...formData, previous_version: e.target.value})}
                      placeholder="如: 1.2.2"
                    />
                  </div>
                ...
                <div>
                  <Label htmlFor="title">升级包标题 *</Label>
                  <Input
                    id="title"
                    value={formData.title}
                    onChange={(e) => setFormData({...formData, title: e.target.value})}
                ...
                <div>
                  <Label htmlFor="description">升级包描述</Label>
                  <Textarea
                    id="description"
                    value={formData.description}
                    onChange={(e) => setFormData({...formData, description: e.target.value})}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <Label htmlFor="file_hash">文件哈希值</Label>
                    <Input
                      id="file_hash"
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <Button onClick={createSoftwareUpdate}>
                    上传升级包
                  </Button>
                </div>
              </div>
...
function EnterpriseRewardQuizManagement() {
  const [questions, setQuestions] = useState<any[]>([
    {
      id: '1',
      title: '产品使用技巧问答',
      description: '关于产品日常使用的常见问题解答',
      question: '如何正确清洁设备表面？',
      answer_options: ['A. 使用酒精擦拭', 'B. 使用专用清洁剂', 'C. 直接用水冲洗', 'D. 不需要清洁'],
      correct_answer: 'B',
      reward_points: 100,
      reward_fc_amount: 50,
    ...
    },
    {
      ...
      answer_options: ['A. 高性价比', 'B. 创新技术', 'C. 完善售后', 'D. 以上都是'],
      correct_answer: 'D',
      reward_points: 150,
      reward_fc_amount: 75,
  ...
  const statusOptions = [
    ...
    { value: 'ended', label: '已结束', color: 'bg-blue-100 text-blue-800' },
    { value: 'closed', label: '已关闭', color: 'bg-red-100 text-red-800' }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">创建和管理有奖问答活动，提升用户参与度</p>
        </div>
        <Button onClick={() => setShowCreateModal(true)}>
          <Plus className="w-4 h-4 mr-2" />
          创建问答
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">总参与人次</CardTitle>
            <Users className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {questions.reduce((sum, q) => sum + q.reward_fc_amount * q.current_participants, 0)}
            </div>
                ...
                {questions.map((question) => (
                  <tr key={question.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-4 px-4 max-w-xs">
                      <div className="text-sm truncate" title={question.question}>
                        {question.question}
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <div className="text-sm">
                        ...
                        <div>FC币: {question.reward_fc_amount}</div>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <div>
                        ...
                        <div className="w-24 bg-gray-200 rounded-full h-2 mt-1">
                          <div 
                            ...
                            style={{ width: `${calculateProgress(question.current_participants, question.max_participants)}%` }}
                          ></div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      <div className="text-sm">
                        ...
                        <div className="text-gray-500">至 {formatDate(question.end_time)}</div>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <div className="flex space-x-2">
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <Label htmlFor="reward_fc_amount">奖励FC币</Label>
                    <Input
                      id="reward_fc_amount"
                      type="number"
                      value={formData.reward_fc_amount}
                      onChange={(e) => setFormData({...formData, reward_fc_amount: parseInt(e.target.value)})}
                      placeholder="FC币数量"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm">参与率</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="text-2xl font-bold text-blue-600">
                        {calculateProgress(selectedQuestion.current_participants, selectedQuestion.max_participants)}%
                      </div>
                      <div className="text-sm text-gray-500">
                        {selectedQuestion.current_participants} / {selectedQuestion.max_participants}
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm">浏览量</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="text-2xl font-bold text-green-600">
                        {selectedQuestion.view_count}
                      </div>
                      <div className="text-sm text-gray-500">次浏览</div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-purple-600">
                        {selectedQuestion.reward_fc_amount * selectedQuestion.current_participants}
                      </div>
                      <div className="text-sm text-gray-500">FC币</div>
                    </CardContent>
                  </Card>
                </div>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {selectedQuestion.answer_options.map((option: string, index: number) => (
                            <div 
                              key={index}
                              className={`p-2 rounded ${selectedQuestion.correct_answer === ['A','B','C','D'][index] ? 'bg-green-100 border border-green-300' : 'bg-gray-50'}`}
                            >
                              <span className="font-medium mr-2">{['A','B','C','D'][index]}.</span>
                              {option}
                              {selectedQuestion.correct_answer === ['A','B','C','D'][index] && (
