...
export default function EnterpriseDemoPage() {
  ...
  const notifications = {
    ...
    showError: (title: string, message: string) => toast.error(title, message),
  ...
  // 模拟表格数据
  const [tableData, _setTableData] = useState<DemoData[]>([
    {
      id: '1',
      name: '智能客服机器人',
      status: 'active',
  ...
  const tableActions = [
    ...
    },
    {
      label: '删除',
      onClick: (record: DemoData) => {
        showInfo('删除确认', `确定要删除 ${record.name} 吗？`);
      },
      condition: (record: DemoData) => record.status === 'inactive',
    },
  ];
  ...
  // 处理表单提交示例
  const _handleFormSubmit = async (_data: any) => {
    try {
      // 这里可以调用实际的API
      // console.log('表单数据:', _data);
      ...
      {activeTab === 'services' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>管理您的企业AI服务和应用</CardDescription>
          </CardHeader>
          <CardContent>
            <DataTable
              data={tableData}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>创建和管理企业的采购订单</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>点击按钮测试不同类型的通知</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-wrap gap-3">
              <Button
                onClick={() => showSuccess('操作成功', '数据已保存到数据库')}
              >
              ...
              <Button
                variant="destructive"
                onClick={() => showError('操作失败', '请检查输入数据')}
              >
              ...
              <Button
                variant="outline"
                onClick={() => showWarning('注意提醒', '某些字段需要特别注意')}
              >
                ⚠️ 警告通知
              </Button>
              <Button
                variant="secondary"
                onClick={() => useToast().info('系统信息', '这是系统提示信息')}
              >
                ℹ️ 信息通知
              </Button>
            </CardContent>
