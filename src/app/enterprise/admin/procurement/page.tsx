...
export default function EnterpriseProcurementPage() {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'pending': return { label: '待审批', color: 'bg-yellow-100 text-yellow-800', icon: Clock };
      case 'approved': return { label: '已批准', color: 'bg-blue-100 text-blue-800', icon: CheckCircle };
      case 'processing': return { label: '处理中', color: 'bg-purple-100 text-purple-800', icon: Package };
      case 'shipped': return { label: '已发货', color: 'bg-indigo-100 text-indigo-800', icon: Truck };
      case 'delivered': return { label: '已送达', color: 'bg-green-100 text-green-800', icon: CheckCircle };
      case 'cancelled': return { label: '已取消', color: 'bg-red-100 text-red-800', icon: XCircle };
  ...
  const getPriorityConfig = (priority: string) => {
    switch (priority) {
      case 'low': return { label: '低', color: 'bg-gray-100 text-gray-800' };
      case 'medium': return { label: '中', color: 'bg-blue-100 text-blue-800' };
      case 'high': return { label: '高', color: 'bg-orange-100 text-orange-800' };
      case 'urgent': return { label: '紧急', color: 'bg-red-100 text-red-800' };
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          <Card>
            <CardContent className="pt-4 text-center">
              ...
              <p className="text-xs text-gray-600">待审批</p>
            </CardContent>
          </Card>
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                            <div>
                              ...
                              <span className="ml-2 font-medium">{order.items} 项</span>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <span className="ml-2">{order.createdAt}</span>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          {filteredOrders.length === 0 && (
            <Card>
              <CardContent className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">未找到匹配的采购订单</h3>
                <p className="text-gray-500 mb-4">尝试调整搜索条件或创建新的采购订单</p>
                <Button>
                  <Plus className="w-4 h-4 mr-2" />
                  新建采购单
                </Button>
