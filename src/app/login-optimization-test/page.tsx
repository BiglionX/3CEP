...
export default function LoginOptimizationTest() {
  ...
  const runComprehensiveTest = async () => {
    ...
    try {
      ...
      const targetPath = Object.keys(targetDescriptions).find(path =>
        redirect.startsWith(path)
      );
      const targetDescription = targetPath
        ? targetDescriptions[targetPath as keyof typeof targetDescriptions]
        : '未知页面';
      ...
      setTestResults(prev => [
        ...prev,
        {
          ...
          details: `识别为目? ${targetDescription} (${targetPath || '通用页面'})`,
        },
      ]);
      ...
      setTestResults(prev => [
        ...prev,
        {
          ...
          details: loginSuccess
            ? `登录成功 - 用户: ${loginData?.email}, 管理? ${loginData?.is_admin}`
            : `登录失败 - ${loginData.error || '未知错误'}`,
        },
      ]);

      // 测试4: 倒计时功能模?      setCurrentTest('测试倒计时功?);
      await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      setTestResults(prev => [
        ...prev,
        {
          feature: '倒计时功?,
          status: 'success',
          details: '3秒倒计时机制已在UnifiedLogin组件中实?,
        },
      ]);
  ...
  // 快速跳转测?  const testQuickJump = () => {
    router.push(`/login?redirect=${redirect}`);
  };

  // 手动触发倒计时测?  const testCountdownManually = () => {
    let countdown = 3;
    const interval = setInterval(() => {
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`倒计? ${countdown}秒`)countdown--;
      if (countdown < 0) {
        clearInterval(interval);
        // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('倒计时结束，准备跳转')}
    }, 1000);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12">
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">全面测试登录体验的各项优化功?/p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>执行各项功能测试并查看结?/CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>需要验证的核心功能特?/CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {[
                  ...
                  { name: '智能目标识别', desc: '自动识别并显示目标页面类? },
                  {
                    name: '登录成功倒计?,
                    desc: '3秒可视化倒计时，用户可主动控?,
                  },
        ...
        {testResults.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>各项功能的验证结?/CardDescription>
            </CardHeader>
            <CardContent>
