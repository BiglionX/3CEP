...
export interface UnifiedDevice {
  ...
  // 技术规?  specifications?: DeviceSpecifications; // 规格参数
  screen_size?: string; // 屏幕尺寸
  storage_options?: string[]; // 存储容量选项
  color_options?: string[]; // 颜色选项

  // 状态信?  status: 'active' | 'inactive' | 'discontinued'; // 设备状?  availability: 'available' | 'limited' | 'unavailable'; // 可获得?
  // 时间?  created_at: string; // 创建时间
...
export interface DeviceSpecifications {
  cpu?: string; // 处理?  gpu?: string; // 图形处理?  ram?: string; // 内存
  storage?: string; // 存储
  battery?: string; // 电池容量
  camera?: string; // 摄像头规?  os?: string; // 操作系统
...
export interface UnifiedPart {
  ...
  specifications?: PartSpecifications; // 技术规?  compatible_devices?: string[]; // 兼容设备列表
  image_url?: string; // 配件图片

  // 业务信息
  status: 'active' | 'inactive' | 'discontinued';
  stock_quantity?: number; // 库存数量
  min_stock?: number; // 最小库存预?  max_stock?: number; // 最大库?
  // 时间?  created_at: string;
  updated_at: string;
}
...
export interface PartSpecifications {
  material?: string; // 材质
  warranty?: string; // 保修?  origin?: string; // 产地
  quality_grade?: string; // 质量等级
  compatibility_level?: string; // 兼容性等?  installation_difficulty?: 'easy' | 'medium' | 'hard'; // 安装难度
  [key: string]: any; // 其他规格字段
}
...
export interface PriceInfo {
  id: string;
  part_id: string;
  platform: string; // 平台名称（淘宝、京东、亚马逊等?  price: number; // 价格
  currency: string; // 货币单位
  url?: string; // 商品链接
  seller?: string; // 商家名称
  rating?: number; // 评分
  sales_volume?: number; // 销?  last_updated: string; // 最后更新时?  created_at: string;
}
...
export interface RepairRecord {
  ...
  actual_start_time?: string; // 实际开始时?  actual_end_time?: string; // 实际结束时间
  created_at: string;
  updated_at: string;
}
...
// 数据血缘关系模?export interface DataLineage {
...
// 元数据管理模?export interface EntityMetadata {
  entity_id: string;
  entity_type: string;
...
export interface DataSourceMapping {
  ...
  confidence_score: number; // 映射置信?(0-1)
  created_at: string;
  validated: boolean;
}
