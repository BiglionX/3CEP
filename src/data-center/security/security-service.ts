...
export interface AccessControlRule {
  ...
  roles: string[]; // 允许的角?  rateLimit?: number; // 速率限制（请?分钟?}
...
// 安全服务?export class SecurityService {
  ...
  // 初始化默认访问控制规?  private initializeDefaultRules(): void {
    // 管理员完全访?    this.addAccessRule({
  ...
  // 初始化数据脱敏规?  private initializeMaskingRules(): void {
    // 手机号码脱敏
  ...
  addAccessRule(rule: AccessControlRule): void {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?添加访问规则: ${rule.id}`)}
  ...
  // 检查访问权?  checkAccess(
    ...
    // 检查账户锁?    const lockoutInfo = this.loginAttempts.get(userId);
    if (lockoutInfo && lockoutInfo.count >= this.config.maxLoginAttempts) {
      const lockoutEnd =
        lockoutInfo.lastAttempt + this.config.lockoutDuration * 1000;
    ...
    // 查找匹配的规?    for (const rule of this.accessRules.values()) {
      if (
  ...
  // 资源匹配检?  private matchesResource(
    ...
    // 支持通配符匹?    const rulePattern = ruleResource.replace(/\*/g, '.*');
    return new RegExp(`^${rulePattern}$`).test(requestedResource);
  }

  // 速率限制检?  private checkRateLimit(userId: string, limit: number): boolean {
    // 简化的速率限制实现
  ...
  logAudit(entry: Omit<AuditLogEntry, 'id' | 'timestamp'>): void {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
      `🔐 审计日志: ${entry.userId} ${entry.action} ${entry.resource} [${entry.status}]`
    )// 清理过期日志
    this.cleanupOldLogs();
  }
  ...
  // 批量数据脱敏
  maskDataArray<T extends Record<string, any>>(
    dataArray: T[],
    userRole: string
  ): T[] {
  ...
  authenticateUser(
    ...
    // 检查登录尝试次?    const attemptInfo = this.loginAttempts.get(username) || {
      count: 0,
      lastAttempt: 0,
    };
    ...
    if (attemptInfo.count >= this.config.maxLoginAttempts) {
      const lockoutEnd =
        attemptInfo.lastAttempt + this.config.lockoutDuration * 1000;
      if (Date.now() < lockoutEnd) {
        return {
          success: false,
    ...
    // 简化的认证逻辑（实际应该查询用户数据库?    if (username === 'admin' && password === 'admin123') {
      // 登录成功，重置失败计?      this.loginAttempts.delete(username);
      ...
      // 登录失败，增加失败计?      this.loginAttempts.set(username, {
  ...
  private generateJWT(userId: string, role: string): string {
    // 实际应用中应该使用jsonwebtoken�?    return `fake_jwt_token_for_${userId}_${role}_${Date.now()}`;
  }
  ...
  setUserPermissions(permission: UserPermission): void {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?设置用户权限: ${permission.userId} (${permission.role})`);
  }
  ...
  private cleanupOldLogs(): void {
    ...
    if (this.auditLogs.length !== oldLogCount) {
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
        `🧹 清理?${oldLogCount - this.auditLogs.length} 条过期日志`
      )}
  }
  ...
  getSecurityStats(): {
    ...
    const failedAttempts = Array.from(this.loginAttempts.values()).reduce(
    ...
    return {
      ...
      activeSessions: 0, // 简化实?      failedLoginAttempts: failedAttempts,
      lockedAccounts,
      recentLogs,
    };
  }
