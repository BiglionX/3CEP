...
export interface BacklogAlertConfig {
  topic: string;
  threshold: number; // 消息数量阈?  checkInterval: number; // 检查间隔（毫秒?  alertChannels: string[]; // 告警渠道
}

// 延迟监控配置
export interface LatencyAlertConfig {
  topic: string;
  thresholdMs: number; // 延迟阈值（毫秒?  windowSize: number; // 时间窗口大小（毫秒）
...
export class MonitoringAlertSystem {
    ...
    this.backlogConfigs = [
      {
        topic: 'price_update',
        threshold: 1000,
        checkInterval: 30000, // 30�?        alertChannels: ['email', 'slack'],
      },
      {
  ...
  private initializeHealthChecks(): void {
    this.healthChecks = [
      ...
      },
      {
        ...
        check: async () => {
          try {
            // 检查实时处理服务的状?            const processingStats =
              enhancedRealTimeService.getProcessingStats();

            // 检查是否有正常的事件处理活?            let totalProcessed = 0;
            let hasRecentActivity = false;

            for (const stat of processingStats.values()) {
              totalProcessed += stat.totalProcessed;
              // 检查最近是否有处理活动?4小时内）
              if (stat.lastProcessedAt) {
                const lastProcessedTime = new Date(
                  stat.lastProcessedAt
                ).getTime();
                const currentTime = Date.now();
                if (currentTime - lastProcessedTime < 24 * 60 * 60 * 1000) {
                  hasRecentActivity = true;
                }
              }
            }

            // 服务健康标准：至少有一定的处理记录且有近期活动
            return totalProcessed >= 0 && hasRecentActivity;
          } catch (error) {
            console.error('处理服务健康检查异?', error);
            return false;
          }
        },
        critical: true,
        description: '实时处理服务健康检?,
      },
      {
        name: 'system_memory',
        check: async () => {
          const usedMemory = process.memoryUsage().heapUsed / 1024 / 1024;
          return usedMemory < 1000; // 1GB阈?        },
        critical: false,
        description: '系统内存使用检?,
      },
      {
        ...
        check: async () => {
          ...
          return totalProcessed > 0; // 至少有处理记?        },
        critical: false,
        description: '事件处理速率检?,
      },
    ];
  ...
  addBacklogAlert(config: BacklogAlertConfig): void {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?添加堆积告警配置: ${config.topic}`)}

  // 添加延迟告警配置
  addLatencyAlert(config: LatencyAlertConfig): void {
    this.latencyConfigs.push(config);
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?添加延迟告警配置: ${config.topic}`)}

  // 移除告警配置
  removeBacklogAlert(topic: string): void {
    this.backlogConfigs = this.backlogConfigs.filter(
      config => config.topic !== topic
    );
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`🗑�?移除堆积告警配置: ${topic}`)}

  removeLatencyAlert(topic: string): void {
    this.latencyConfigs = this.latencyConfigs.filter(
      config => config.topic !== topic
    );
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`🗑�?移除延迟告警配置: ${topic}`)}

  // 启动监控
  start(): void {
    if (this.isRunning) {
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('⚠️ 监控系统已在运行')return;
    }

    this.isRunning = true;
    this.startMonitoringLoop();
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('�?监控告警系统已启?)}
  ...
  stop(): void {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('🛑 监控告警系统已停?)}
  ...
  private async performMonitoringChecks(): Promise<void> {
    // 执行健康检?    await this.performHealthChecks();

    // 检查消息堆?    await this.checkMessageBacklog();

    // 检查处理延?    await this.checkProcessingLatency();
      ...
      try {
        ...
        if (!isHealthy) {
          ...
          await this.createAlert({
            type: 'health',
            severity,
            title: `健康检查失? ${check.name}`,
            message: check.description,
            alertChannels: ['email', 'slack'],
          });
        }
      } catch (error) {
        console.error(`�?健康检查执行失?${check.name}:`, error);
        await this.createAlert({
          type: 'health',
          severity: check.critical ? 'critical' : 'error',
          title: `健康检查异? ${check.name}`,
          message: `检查执行异? ${error instanceof Error ? error.message : '未知错误'}`,
          alertChannels: ['email'],
        });
      }
    }
        ...
        if (stats && typeof stats.messageCount === 'number') {
          ...
          if (messageCount > config.threshold) {
            await this.createAlert({
              type: 'backlog',
              severity:
                messageCount > config.threshold * 2 ? 'critical' : 'warning',
              title: `消息堆积告警: ${config.topic}`,
              message: `队列 ${config.topic} 当前堆积 ${messageCount} 条消息，超过阈?${config.threshold}`,
              alertChannels: config.alertChannels,
            });
          }
        }
      } catch (error) {
        console.error(`�?堆积检查失?${config.topic}:`, error);
      }
    }
  }
    ...
    for (const config of this.latencyConfigs) {
      ...
      if (topicStats && topicStats.averageProcessingTime > config.thresholdMs) {
        await this.createAlert({
          type: 'latency',
          severity:
            topicStats.averageProcessingTime > config.thresholdMs * 2
              ? 'critical'
              : 'warning',
          title: `处理延迟告警: ${config.topic}`,
          message: `事件类型 ${config.topic} 平均处理时间 ${topicStats.averageProcessingTime}ms，超过阈?${config.thresholdMs}ms`,
          alertChannels: config.alertChannels,
        });
      }
    }
  ...
  private async collectPerformanceMetrics(): Promise<void> {
    ...
    // 收集汇总统?    for (const [eventType, stat] of stats.entries()) {
      totalProcessed += stat.totalProcessed;
      ...
      monitoringService.recordMetric(
        `event_processing_rate_${eventType}`,
        stat.totalProcessed
      );
      monitoringService.recordMetric(
        `event_error_rate_${eventType}`,
        stat.errorCount
      );
    }
  ...
  private async createAlert(options: {
    ...
    // 记录到监控系?    monitoringService.recordMetric('alerts_created', 1, {
      type: alert.type,
      severity: alert.severity,
    });

    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`🚨 创建告警: ${alert.title} (${alert.severity})`);
  }
  ...
  private async sendAlertNotification(
    ...
    // 这里应该集成实际的通知服务
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`📢 发送告警通知:`, {
      title: alert.title,
  ...
  async resolveAlert(alertId: string): Promise<void> {
    ...
    if (alert && !alert.resolved) {
      ...
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?告警已解? ${alert.title}`)}
  }
  ...
  // 获取系统健康状?  async getSystemHealth(): Promise<Record<string, boolean>> {
...
// 性能分析?export class PerformanceAnalyzer {
  private latencySamples: Map<string, number[]> = new Map();
  private sampleWindowSize: number = 100; // 保留最?00个样?
  ...
  getAllLatencyStats(): Record<
    string,
    ReturnType<PerformanceAnalyzer['getLatencyStats']>
  > {
