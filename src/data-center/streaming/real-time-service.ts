...
// 实时处理器接?export interface RealTimeProcessor {
...
export class RealTimeDataService extends EventEmitter {
  ...
  private processingStats: Map<RealTimeEventType, EventProcessingStats> =
    new Map();
  ...
  private reportPerformanceMetrics(): void {
    ...
    // 记录总体指标
    monitoringService.recordMetric(
      'realtime_total_events_processed',
      totalEvents
    );
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
      `📊 实时处理统计: 总事?${totalEvents}, 错误=${totalErrors}, 错误?${errorRate.toFixed(2)}%`
    );
  }
  ...
  private setupRedisEvents(): void {
    this.redisClient.on('connect', () => {
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('�?Redis Stream连接成功')});
    ...
    this.redisClient.on('ready', () => {
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('�?Redis Stream准备就绪')});
  }

  // 注册实时处理?  registerProcessor(processor: RealTimeProcessor): void {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?注册处理? ${eventType}`)}

  // 创建消费者组
  async createConsumerGroup(config: ConsumerGroupConfig): Promise<void> {
    try {
      // 创建Stream（如果不存在?      await this.redisClient.xadd(config.streamKey, '*', 'init', 'true');
      ...
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?创建消费者组: ${config.groupName}`)} catch (error) {
      if (!(error as Error).message.includes('BUSYGROUP')) {
        throw error;
      }
      // 消费者组已存在，继续使用
      this.consumerGroups.set(config.groupName, config);
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`�?使用现有消费者组: ${config.groupName}`)}
  }
  ...
  async publishEvent(event: RealTimeEvent): Promise<string> {
    try {
      ...
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`📤 发布事件: ${event.type} (${eventId})`);
  ...
  // 启动消费?  async startConsumers(): Promise<void> {
    ...
    this.isRunning = true;
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('🚀 启动实时数据消费?..')for (const [groupName, config] of this.consumerGroups.entries()) {
      this.startConsumerLoop(config);
    }
  }

  // 消费者循?  private async startConsumerLoop(config: ConsumerGroupConfig): Promise<void> {
    while (this.isRunning) {
      try {
        ...
        console.error(`�?消费?${config.consumerName} 处理错误:`, error);
        await this.sleep(1000); // 错误后短暂休?      }
    }
  }
  ...
  private async processMessage(
    ...
    try {
      ...
      if (processors.length === 0) {
        ...
        // 确认消息处理完成（即使没有处理器也要确认?        await this.redisClient.xack(
          config.streamKey,
          config.groupName,
          messageId
        );
      ...
      // 并行处理
      const processingPromises = processors.map(
        processor =>
          this.executeProcessorWithTimeout(processor, eventData, 5000) // 5秒超?      );
      ...
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
        `�?处理完成: ${eventData.type} (${messageId})耗时: ${processingTime}ms`
      );
      ...
      const processingTime = Date.now() - startTime;
      this.updateStats(
        this.parseEventData(messageData).type,
        false,
        processingTime
      );
      ...
      // 记录到监控系?      monitoringService.recordMetric('realtime_processing_failures', 1, {
  ...
  // 带超时的处理器执?  private async executeProcessorWithTimeout(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`处理器执行超? ${timeoutMs}ms`));
      }, timeoutMs);
  ...
  private updateStats(
    ...
    if (success) {
      ...
      const newAvg =
        (currentAvg * (stats.successCount - 1) + processingTime) /
        stats.successCount;
  ...
  private async handleDeadLetterMessage(
    ...
    try {
      ...
      // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(`💀 消息已移至死信队? ${messageId} (${eventData.type})`);
    } catch (dlqError) {
  ...
  // 获取特定事件类型的统?  getEventTypeStats(
    eventType: RealTimeEventType
  ): EventProcessingStats | undefined {
  ...
  async stop(): Promise<void> {
    ...
    // 报告最终统?    this.reportPerformanceMetrics();

    await this.redisClient.quit();
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log('🛑 实时数据服务已停?)}
...
// 价格更新处理?export class PriceUpdateProcessor implements RealTimeProcessor {
  getType(): RealTimeEventType {
    return 'price_update';
  }
  ...
  async process(event: RealTimeEvent): Promise<void> {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
      `💰 处理价格更新: ${priceData.partId} -> ${priceData.newPrice}`
    )// 这里可以实现具体的业务逻辑?    // 1. 更新缓存
    // 2. 触发通知
    // 3. 记录价格历史
    // 4. 检查价格异?
    // 示例：简单的阈值检?    if (priceData.changePercent && Math.abs(priceData.changePercent) > 10) {
      console.warn(
        `⚠️ 价格大幅波动警告: ${priceData.partId} 变化 ${priceData.changePercent}%`
      );
    }
  }
}

// 库存变更处理?export class InventoryChangeProcessor implements RealTimeProcessor {
  getType(): RealTimeEventType {
  ...
  async process(event: RealTimeEvent): Promise<void> {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
      `📦 处理库存变更: ${inventoryData.partId} 数量 ${inventoryData.newQuantity}`
    )// 库存预警逻辑
...
// 用户行为处理?export class UserActionProcessor implements RealTimeProcessor {
  getType(): RealTimeEventType {
    return 'user_action';
  }
  ...
  async process(event: RealTimeEvent): Promise<void> {
    ...
    // TODO: 移除调试日志 - // TODO: 移除调试日志 - console.log(
      `👤 处理用户行为: ${actionData.userId} -> ${actionData.actionType}`
    )// 可以实现实时推荐、行为分析等逻辑
  }
}
...
// 注册默认处理?realTimeDataService.registerProcessor(new PriceUpdateProcessor());
