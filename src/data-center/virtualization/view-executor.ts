...
export class VirtualViewExecutor {
  ...
  private defaultTimeoutMs: number = 30000; // 30秒默认超时
  ...
  async executeView(
    ...
    if (!view) {
      throw new Error(`未找到视图: ${viewName}`);
    }
    ...
    if (options.useCache !== false && view.cacheEnabled && !options.forceRefresh) {
  ...
  async getViewMetadata(viewName: string) {
    ...
    return {
      ...
      columnCount: 0, // 需要实际查询才能知道
      rowCount: 0     // 需要实际查询才能知道
    };
  }

  // 预热视图缓存
  async warmupViews(viewNames?: string[]): Promise<void> {
    const viewsToWarm = viewNames || 
      this.viewManager.getAllViews()
        .filter(view => view.cacheEnabled)
        .map(view => view.name);

    console.log(`🔥 预热视图缓存: ${viewsToWarm.join(', ')}`);
    ...
    const successCount = Object.values(results).filter(r => !r.hasOwnProperty('error')).length;
    console.log(`✅ 预热完成: ${successCount}/${viewsToWarm.length} 个视图`);
  }
  ...
  // 构建带参数的查询
  private buildParameterizedQuery(sql: string, parameters?: Record<string, any>): string {
    ...
    for (const [key, value] of Object.entries(parameters)) {
      ...
      parameterizedSql = parameterizedSql.replace(new RegExp(placeholder, 'g'), replacement);
    }
  ...
  // 带超时的查询执行
  private async executeQueryWithTimeout(query: string, timeoutMs: number): Promise<any> {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`查询超时 (${timeoutMs}ms)`));
      }, timeoutMs);

      trinoClientInstance.executeQuery(query)
        .then(result => {
  ...
  // 生成缓存键
  private generateCacheKey(viewName: string, parameters?: Record<string, any>): string {
    if (!parameters) {
  ...
  // 获取缓存结果
  private async getCachedResult(cacheKey: string): Promise<ViewExecutionResult | null> {
    try {
