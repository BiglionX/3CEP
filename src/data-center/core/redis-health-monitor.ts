...
export class RedisHealthMonitor {
  ...
  private recordMetrics(status: RedisHealthStatus): void {
    if (status.isConnected) {
      ...
      monitoringService.recordMetric('redis_connected_clients', status.clients.connectedClients);
      monitoringService.recordMetric('redis_commands_per_second', status.stats.instantaneousOpsPerSec);
      
      // 计算缓存命中率
      const totalAccesses = status.stats.keyspaceHits + status.stats.keyspaceMisses;
      if (totalAccesses > 0) {
  ...
  public startMonitoring(intervalMs: number = 30000): void {
    ...
    console.log(`📊 Redis健康监控已启动，检查间隔: ${intervalMs}ms`);
  }
  ...
  public stopMonitoring(): void {
    if (this.monitoringInterval) {
      ...
      console.log('🛑 Redis健康监控已停止');
    }
  }

  // 获取连接池状态
  public async getConnectionPoolStatus(): Promise<any> {
    try {
      // 这里可以扩展为检查连接池中每个连接的状态
      const poolInfo = {
        poolSize: 3,
        activeConnections: 3, // 简化实现
        queueLength: 0,
        timestamp: new Date().toISOString()
      };
  ...
  // 获取慢查询日志
  public async getSlowLog(count: number = 10): Promise<any[]> {
    try {
