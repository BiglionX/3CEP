...
export class DataMapperService {
  ...
  mapPriceData(lionfixPrices: any[], fixcyclePrices: any[]): PriceInfo[] {
    ...
    lionfixPrices.forEach(price => {
      unifiedPrices.push(DataModelFactory.createPriceInfo({
        ...price,
  ...
  // 确定设备状态
  private determineDeviceStatus(lionfixDevice: any, fixcycleDevice?: any): 'active' | 'inactive' | 'discontinued' {
    // 优先使用lionfix状态
    if (lionfixDevice.status) {
      return lionfixDevice.status;
    }
    
    // 如果lionfix没有状态，使用fixcycle状态
    if (fixcycleDevice?.status) {
  ...
  // 确定设备可用性
  private determineAvailability(lionfixDevice: any, fixcycleDevice?: any): 'available' | 'limited' | 'unavailable' {
    // 检查是否有库存信息
    ...
    // 基于状态判断
    const status = this.determineDeviceStatus(lionfixDevice, fixcycleDevice);
    if (status === 'active') {
      return 'available';
  ...
  // 确定配件状态
  private determinePartStatus(lionfixPart: any, fixcyclePart?: any): 'active' | 'inactive' | 'discontinued' {
  ...
  cleanupExpiredMappings(expiryDays: number = 30): number {
    ...
    for (const [key, mappings] of this.mappings.entries()) {
      const filteredMappings = mappings.filter(mapping => 
        mapping.created_at > cutoffISOString
      );
...
export class DataTransformer {
  ...
  // 标准化字段名称
  static standardizeFieldNames(data: Record<string, any>): Record<string, any> {
