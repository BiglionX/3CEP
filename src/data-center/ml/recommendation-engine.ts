...
export interface UserProfile {
  ...
  similarityScore?: number;    // 与其他用户的相似度
}
...
export class CollaborativeFilterRecommender implements Recommender {
  ...
  async recommend(userId: string, count: number = 10): Promise<RecommendationItem[]> {
    ...
    // 基于用户相似度的推荐
    const userBasedRecs = await this.getUserBasedRecommendations(userProfile, count);
    
    // 基于物品相似度的推荐
    const itemBasedRecs = await this.getItemBasedRecommendations(userProfile, count);
    
    // 合并和排序推荐结果
    const mergedRecs = this.mergeRecommendations(userBasedRecs, itemBasedRecs, count);
  ...
  // 提取用户偏好
  private extractPreferences(behaviors: UserBehavior[]): UserProfile['preferences'] {
  ...
  private parseItemId(itemId: string): { category?: string; brand?: string } {
    // 简化的解析逻辑，实际应该查询数据库
    if (itemId.includes('iphone')) return { category: 'smartphone', brand: 'Apple' };
    if (itemId.includes('galaxy')) return { category: 'smartphone', brand: 'Samsung' };
    ...
    for (let i = 0; i < users.length; i++) {
      for (let j = i + 1; j < users.length; j++) {
        ...
        if (similarity > this.config.similarityThreshold) {
          // 存储相似度（简化存储）
          console.log(`👥 用户 ${users[i]} 和 ${users[j]} 相似度: ${similarity.toFixed(3)}`);
        }
      }
    }
  }
  ...
  private calculateUserSimilarity(userId1: string, userId2: string): number {
    ...
    const categorySimilarity = commonCategories / Math.max(
      profile1.preferences.categories.length,
      profile2.preferences.categories.length,
      1
    );
    
    const brandSimilarity = commonBrands / Math.max(
  ...
  private async getUserBasedRecommendations(
    ...
    // 找到相似用户喜欢的物品
    const similarUsers = this.findSimilarUsers(userProfile.userId);
    
    // 收集相似用户的行为但目标用户未接触过的物品
    const userItems = new Set(userProfile.behaviorHistory.map(b => b.itemId));
      ...
      if (similarProfile) {
        similarProfile.behaviorHistory.forEach(behavior => {
          if (!userItems.has(behavior.itemId)) {
            ...
            candidateItems.set(behavior.itemId, currentScore + behavior.score * similarUser.similarity);
          }
        });
      }
    });
    ...
    recentItems.forEach(itemId => {
      ...
      similarItems.forEach(similarItem => {
        recommendations.push({
          ...
          reason: `与您浏览的 ${this.getItemDisplayName(itemId)} 相似`,
  ...
  // 查找相似物品（简化实现）
  private findSimilarItems(itemId: string): Array<{itemId: string, itemType: string, similarity: number}> {
    // 简化实现，返回同类别的一些物品
    const itemInfo = this.parseItemId(itemId);
    return [
      { itemId: `${itemId}_variant1`, itemType: this.getItemType(itemId) as 'device' | 'part' | 'accessory', similarity: 0.8 },
      { itemId: `${itemId}_variant2`, itemType: this.getItemType(itemId) as 'device' | 'part' | 'accessory', similarity: 0.7 },
      { itemId: `${itemId}_related1`, itemType: this.getItemType(itemId) as 'device' | 'part' | 'accessory', similarity: 0.6 }
    ];
  }
  ...
  private getDefaultRecommendations(count: number): RecommendationItem[] {
    // 返回热门物品作为默认推荐
    return [
      { itemId: 'popular_item_1', itemType: 'device', score: 90, reason: '热门推荐', confidence: 0.9 },
      { itemId: 'popular_item_2', itemType: 'part', score: 85, reason: '热门推荐', confidence: 0.85 },
      { itemId: 'popular_item_3', itemType: 'accessory', score: 80, reason: '热门推荐', confidence: 0.8 }
    ].slice(0, count);
  }
  ...
  private getItemType(itemId: string): 'device' | 'part' | 'accessory' {
    ...
    if (itemId.includes('part') || itemId.includes('screen') || itemId.includes('battery')) return 'part';
    return 'accessory';
  }
...
export class HybridRecommender {
  ...
  async recommend(userId: string, count: number = 10): Promise<RecommendationItem[]> {
    ...
    const recommendationSets = await Promise.all(
      this.recommenders.map(async recommender => ({
        ...
        recommendations: await recommender.recommend(userId, count * 2) // 获取更多候选
      }))
    );

    // 合并和加权推荐结果
    const mergedRecommendations = this.mergeWeightedRecommendations(recommendationSets, count);
    
    return mergedRecommendations;
  }

  private mergeWeightedRecommendations(
    recommendationSets: Array<{type: string, recommendations: RecommendationItem[]}>,
    count: number
  ): RecommendationItem[] {
    ...
    itemScores.forEach((scoreInfo, itemId) => {
      // 找到该物品的最佳推荐信息
      const bestRec = recommendationSets
  ...
  // 添加新的推荐器
  addRecommender(recommender: Recommender, weight: number): void {
