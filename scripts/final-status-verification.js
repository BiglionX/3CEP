  ...
  for (const test of criticalTests) {
    try {
      ...
      if (status === test.expected) {
        ...
        console.log(`   ⚠️  ${test.name} - 状态码 ${status} (预期: ${test.expected})`);
        testResults.push({ 
          name: test.name, 
          status: 'WARN',
        ...
        console.log(`   ❌ ${test.name} - 状态码 ${status} (预期: ${test.expected})`);
        testResults.push({ 
          name: test.name, 
          status: 'FAIL',
          details: `状态码: ${status}, 预期: ${test.expected}`
  ...
  for (const api of mobileApis) {
    try {
      ...
      if (status === api.expected) {
        ...
        console.log(`   ⚠️  ${api.name} - 状态码 ${status} (预期: ${api.expected})`);
        testResults.push({ 
          name: `${api.name} API`, 
          status: 'WARN',
...
if (require.main === module) {
  finalSystemStatusVerification().then(result => {
    console.log('\n📊 验证完成！');
