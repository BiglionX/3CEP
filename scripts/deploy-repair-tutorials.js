...
async function deployRepairTutorials() {
  ...
  try {
    ...
    const migrationPath = path.join(
      ...
      'supabase',
      'migrations',
      '022_create_repair_tutorials_table.sql'
    );
    ...
    const verifyResponse = await fetch(
      `${supabaseUrl}/rest/v1/repair_tutorials?select=*,limit=1`,
      {
        headers: {
    ...
    if (verifyResponse.ok) {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        data.slice(0, 3).forEach((row, index) => {
          console.log(
            `   ${index + 1}. ${row.title} (${row.device_model} - ${row.fault_type}) [${row.status}]`
          );
        });
      }
    } else {
