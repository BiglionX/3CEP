...
if (fs.existsSync(loginPagePath)) {
  ...
  // 显示实际的跳转逻辑
  console.log('\n实际跳转逻辑代码:');
  const jumpLogic = content.match(/if \(response\.ok\) \{[\s\S]*?router\.push\(targetRedirect\);[\s\S]*?\}/);
  if (jumpLogic) {
...
const apiRoutePath = path.join(process.cwd(), 'src', 'app', 'api', 'auth', 'login', 'route.ts');
...
console.log('2. taskkill /F /PID $(netstat -ano | findstr :3001 | findstr LISTENING | awk \'{print $5}\')');
