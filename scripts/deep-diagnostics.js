  ...
  try {
    ...
    try {
      ...
      if (freshSession.success) {
        // 测试管理后台访问
        const dashboardTest = await fetch('http://localhost:3001/admin/dashboard', {
          method: 'GET',
          headers: {
        ...
        console.log('重定向地址:', dashboardTest.headers.get('location') || '无重定向');
        
        const dashboardContent = await dashboardTest.text();
        console.log('页面内容长度:', dashboardContent.length);
        
        if (dashboardContent.includes('管理后台') || dashboardContent.includes('admin')) {
