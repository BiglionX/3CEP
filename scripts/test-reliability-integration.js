...
const { AgentOrchestrator } = require('../src/agents-orchestrator/orchestrator');
  ...
  try {
    ...
    try {
      ...
      if (response1.code === 200 && response2.code === 200) {
        if (idempotentCallCount === 1 && 
            response1.data.timestamp === response2.data.timestamp) {
          console.log(`   ✅ 通过 - 幂等性正确工作 (实际调用${idempotentCallCount}次)`);
          passedTests++;
        } else {
          console.log('   ⚠️  部分通过 - 响应成功但幂等性验证需要改进');
          console.log(`   实际调用次数: ${idempotentCallCount}`);
          console.log(`   时间戳匹配: ${response1.data.timestamp === response2.data.timestamp}`);
        }
      } else {
