  ...
  try {
    ...
    if (setCookieHeader) {
      ...
      cookieParts.forEach((part, index) => {
        if (part.includes('sb-')) {
          console.log(`Supabase Cookie ${index + 1}:`, part.substring(0, 100) + '...');
        }
      });
    }
    ...
    if (dashboardResponse.status === 200) {
      ...
      console.log('页面标题:', content.match(/<title>([^<]*)<\/title>/)?.[1] || '无标题');
    ...
    if (!userError && userData?.user) {
      console.log('用户元数据:', JSON.stringify(userData.user.user_metadata, null, 2));
      console.log('App元数据:', JSON.stringify(userData.user.app_metadata, null, 2));
    }
    ...
    for (const scenario of testScenarios) {
      ...
      const testResponse = await fetch('http://localhost:3001/admin/dashboard', {
        method: 'GET',
