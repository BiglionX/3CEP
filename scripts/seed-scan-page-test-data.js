...
console.log(`${colors.blue}${colors.bold}🌱 创建扫码落地页测试数据${colors.reset}\n`);
...
async function seedTestData() {
  ...
  const getProfileResponse = await makeRequest(`/api/devices/${TEST_CONFIG.testQrCodeId}/profile`, {
    method: 'GET'
  });
  ...
  if (getProfileResponse.success) {
    ...
    console.log(`${colors.red}❌ 设备档案验证失败: ${getProfileResponse.data.error}${colors.reset}`);
    return;
  }
  
  // 2. 获取生命周期事件
  console.log('\n2️⃣ 获取生命周期事件...');
  const eventsResponse = await makeRequest(`/api/devices/${TEST_CONFIG.testQrCodeId}/lifecycle`, {
    method: 'GET'
  });
  
  if (eventsResponse.success) {
    const events = eventsResponse.data.data || [];
    console.log(`${colors.green}✅ 成功获取 ${events.length} 个事件${colors.reset}`);
    
    // 显示最近的事件
    console.log('\n📅 最近的生命周期事件:');
    events.slice(0, 3).forEach((event, index) => {
      console.log(`   ${index + 1}. ${event.eventType} - ${new Date(event.eventTimestamp).toLocaleString('zh-CN')}`);
    });
  } else {
    console.log(`${colors.red}❌ 获取事件失败: ${eventsResponse.data.error}${colors.reset}`);
  }
  
  // 3. 验证数据
  console.log('\n3️⃣ 验证测试数据...');
  const profileResponse = await makeRequest(`/api/devices/${TEST_CONFIG.testQrCodeId}/profile`, {
    method: 'GET'
  });
  ...
  if (profileResponse.success) {
    ...
    console.log(`${colors.red}❌ 数据验证失败: ${profileResponse.data.error}${colors.reset}`);
  }
