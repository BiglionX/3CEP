...
const { enhancedRealTimeService, EventPriority } = require('../src/data-center/streaming/enhanced-realtime-service');
const { businessEventTrigger } = require('../src/data-center/streaming/business-event-trigger');
const { eventProcessingPipeline } = require('../src/data-center/streaming/notification-alert-system');
  ...
  try {
    ...
    for (const [eventType, stats] of processingStats.entries()) {
      if (stats.totalProcessed > 0) {
        console.log(`      ${eventType}: 处理${stats.totalProcessed}个, 成功${stats.successCount}个, 错误${stats.errorCount}个`);
      }
    }
    ...
    console.log(`      处理速率: ${(testEvents / (totalTime / 1000)).toFixed(2)} events/sec`);
    ...
    console.log(`   ⚡ 延迟验证: ${validationPassed ? '✅ 通过' : '❌ 未通过'} (<100ms)`);
    console.log(`   🎯 系统验证: ${validationPassed ? '✅ 系统运行正常' : '❌ 需要优化'}`);
