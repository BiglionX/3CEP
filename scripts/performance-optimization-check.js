...
const performanceChecks = {
        ...
        files.forEach(file => {
          ...
          if (stat.isDirectory() && !['node_modules', '.next', '.git'].includes(file)) {
            ...
            if (imageExtensions.includes(ext)) {
              imageCount++;
              if (stat.size > 100 * 1024) { // 大于100KB的图片
                largeImages.push({
  ...
  seoOptimization: () => {
        ...
        files.forEach(file => {
          ...
          if (stat.isDirectory()) {
            ...
            try {
              ...
              metaTags.push({
                path: filePath.replace(pagesDir, '').replace('/page.tsx', '').replace('/page.jsx', '') || '/',
                hasTitle,
                hasDescription
              });
            } catch (error) {
    ...
    const missingMeta = metaTags.filter(page => !page.hasTitle || !page.hasDescription);
