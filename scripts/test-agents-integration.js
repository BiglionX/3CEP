...
function parseAgentsInventory(markdownContent) {
  ...
  for (const line of lines) {
    ...
    if (line.includes('已部署') && line.includes('|')) {
      ...
      const columns = cleanLine.split('|').map(col => col.trim()).filter(col => col);
      ...
      res.on('end', () => {
        try {
          ...
          if (testResult.success) {
            console.log(`   ✅ 通过 - 状态码: ${res.statusCode}, 响应时间: ${testResult.responseTime}ms`);
          } else {
...
async function main() {
  try {
    // 读取智能体清单
    const inventoryPath = path.join(__dirname, '..', 'docs', 'technical-docs', 'agents-inventory.md');
