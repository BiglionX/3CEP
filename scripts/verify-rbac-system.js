...
if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
  ...
  console.error(
    '请确保设置了 NEXT_PUBLIC_SUPABASE_URL 和 SUPABASE_SERVICE_ROLE_KEY'
  );
  process.exit(1);
}
...
async function verifyRBACSystem() {
  ...
  try {
    ...
    try {
      ...
      // 验证超级管理员拥有最高权限
      if (
        roleHierarchy['admin'] === Math.max(...Object.values(roleHierarchy))
      ) {
    ...
    console.log(
      '• ✅ 用户角色管理(admin/content_reviewer/shop_reviewer/finance/viewer)'
    );
