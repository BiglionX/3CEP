...
async function deployAdminSystem() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '..', 'supabase', 'migrations', '003_add_admin_roles.sql')
      ...
      if (statement) {
        try {
          ...
          if (statement.includes('create type')) {
            console.log(`🔧 执行语句 ${i + 1}/${statements.length}: CREATE TYPE`)
    ...
    const validationQueries = [
      { name: '用户角色枚举', query: () => supabase.from('admin_users').select('role').limit(1) },
      { name: '权限表', query: () => supabase.from('permissions').select('id').limit(1) },
      { name: '管理员用户表', query: () => supabase.from('admin_users').select('id').limit(1) }
    ]
