...
function getVersionStatus(appliedMigrations, allMigrations) {
  ...
  const status = {
    current: appliedVersions.length > 0 ? appliedVersions[appliedVersions.length - 1] : '0.0.0',
    latest: allVersions.length > 0 ? allVersions[allVersions.length - 1] : '0.0.0',
    upToDate: appliedVersions.length === allVersions.length,
    pendingCount: 0,
    failedCount: 0
  };
  
  status.pendingCount = allMigrations.filter(m => 
...
async function main() {
  try {
    ...
    const { migrations: allMigrations, rollbacks: allRollbacks } = getAllMigrationFiles();
    ...
    if (appliedMigrations.length === 0) {
      ...
      appliedMigrations.forEach(migration => {
        ...
        const timeInfo = migration.execution_time ? `(${migration.execution_time}ms)` : '';
        console.log(`   ${statusIcon} V${migration.version} - ${migration.description} ${timeInfo}`);
        if (!migration.success && migration.error_message) {
          console.log(`      错误: ${migration.error_message.substring(0, 100)}...`);
        }
      });
    }
    ...
    if (allRollbacks.length > 0) {
      ...
      Object.keys(rollbackMap).sort(compareVersions).forEach(version => {
    ...
    if (status.pendingCount > 0) {
      ...
      if (status.pendingCount === 1) {
        console.log(`   或指定版本: npm run db:migrate -- --version=${pendingMigrations[0].version}`);
      }
    }
    
    if (appliedMigrations.length > 0) {
      const lastVersion = appliedMigrations[appliedMigrations.length - 1].version;
