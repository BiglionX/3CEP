...
console.log(`${colors.blue}${colors.bold}🔧 设备生命周期核心API功能验证${colors.reset}\n`);
...
async function runLifecycleAPITests() {
  ...
  logTestResult(
    '缺少参数验证',
    missingParamResponse.status === 400 && missingParamResponse.data.error.includes('qrcodeId'),
  ...
  logTestResult(
    '事件类型验证',
    invalidEventTypeResponse.status === 400 && invalidEventTypeResponse.data.error.includes('无效的事件类型'),
  ...
  // 测试6: 查询设备档案和事件
  const profileResponse = await makeRequest(`/api/lifecycle/profile?qrcodeId=${TEST_CONFIG.testQrCodeId}`, {
    method: 'GET'
  });
  logTestResult(
    '查询设备档案',
  ...
  if (profileResponse.success && profileResponse.data.data) {
    ...
    // 验证事件顺序（应该是倒序）
    if (events.length >= 2) {
      const isDescending = new Date(events[0].eventTimestamp) >= new Date(events[1].eventTimestamp);
      logTestResult(
        '事件时间排序',
        isDescending,
  ...
  if (manufacturedEventResponse.success) {
    ...
    const updatedProfileResponse = await makeRequest(`/api/lifecycle/profile?qrcodeId=${TEST_CONFIG.testQrCodeId}`, {
      method: 'GET'
    });
  ...
  // 测试10: 验证激活事件记录
  const activationEvents = profileResponse.data.data?.events?.filter(e => e.eventType === 'activated') || [];
  logTestResult(
    '激活事件记录',
  ...
  // 测试12: 查询不存在的设备
  const nonExistentResponse = await makeRequest('/api/lifecycle/profile?qrcodeId=non_existent_device', {
    method: 'GET'
  });
  logTestResult(
    '不存在设备查询',
  ...
  if (passedTests === totalTests) {
    ...
    console.log(`${colors.blue}\n🚀 核心功能清单:${colors.reset}`);
    console.log('• RESTful API端点: /api/lifecycle/events, /api/lifecycle/profile');
