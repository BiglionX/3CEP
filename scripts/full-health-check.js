...
async function checkEnvironmentConfiguration(environment) {
  ...
  console.log(`   ✅ 通过: ${result.passed}, ⚠️ 警告: ${result.warnings}, ❌ 失败: ${result.failed}`);
  return result;
}
...
async function checkServiceStatus(environment) {
  ...
  // 检查Docker服务状态
  try {
    const dockerOutput = execSync('docker ps --format "table {{.Names}}\t{{.Status}}"', { 
      stdio: 'pipe', 
      encoding: 'utf8' 
    });
    result.passed++;
...
async function generateHealthReport(results, environment, duration) {
  ...
  categories.forEach(category => {
    ...
    if (result) {
      ...
      console.log(`   ${category.name}: ✅${result.passed} ⚠️${result.warnings} ❌${result.failed}`);
    }
  });
