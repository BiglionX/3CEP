...
function calculateScoringFactors(distance, inventory, cost, warehouse) {
  ...
  // 服务得分（性能指标平均值）
  const serviceScore = (warehouse.performanceMetrics.accuracyRate + 
                       warehouse.performanceMetrics.onTimeRate) / 2;
...
function optimizeWarehouseSelection(request) {
  ...
  const warehouseScores = mockWarehouses.map(warehouse => {
    ...
    // 检查库存
    const inventoryAvailability = checkInventory(warehouse.id, request.orderItems);
    ...
    return {
      ...
      serviceLevel: {
        ...
        qualityScore: (warehouse.performanceMetrics.accuracyRate + 
                      warehouse.performanceMetrics.onTimeRate) / 2
      },
  ...
  const sortedWarehouses = warehouseScores.sort((a, b) => {
    // 首先确保有足够库存
    if (!a.inventoryAvailability.hasSufficientStock && 
        b.inventoryAvailability.hasSufficientStock) return 1;
    if (a.inventoryAvailability.hasSufficientStock && 
        !b.inventoryAvailability.hasSufficientStock) return -1;
  ...
  const improvementRate = ((randomCost - selectedWarehouse.totalCost) / randomCost) * 100;
  ...
  const response = {
    ...
    optimizationMetrics: {
      ...
      factorsConsidered: ['distance', 'inventory', 'cost', 'delivery_time', 'service_quality'],
    ...
    confidenceScore: selectedWarehouse.inventoryAvailability.hasSufficientStock ? 90 : 70
  };
...
function runTest() {
  ...
  console.log('📍 测试请求:');
  console.log(`   收货地址: ${testRequest.deliveryAddress.province}${testRequest.deliveryAddress.city}`);
  console.log(`   商品数量: ${testRequest.orderItems.length}种`);
  console.log(`   总价值: ¥${testRequest.orderItems.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0)}`);
  ...
  try {
    ...
    console.log(`⏱️  配送时间: ${result.selectedWarehouse.estimatedDeliveryTime.toFixed(1)} 小时`);
    console.log(`⭐ 优化得分: ${result.selectedWarehouse.optimizationScore.toFixed(1)}/100`);
    console.log(`📊 成本改善率: ${result.optimizationMetrics.improvementRate.toFixed(2)}%`);
    console.log(`💡 成本节省: ¥${result.costAnalysis.savings.absolute.toFixed(2)} (${result.costAnalysis.savings.percentage.toFixed(2)}%)`);
    ...
    if (result.alternativeOptions.length > 0) {
      ...
      result.alternativeOptions.forEach((alt, index) => {
        console.log(`   ${index + 1}. ${alt.warehouseName} - ¥${alt.totalCost.toFixed(2)} (${alt.estimatedDeliveryTime.toFixed(1)}h)`);
      });
    }
