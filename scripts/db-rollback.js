  ...
  try {
    ...
    if (!fs.existsSync(rollbackFilePath)) {
      ...
      for (const statement of statements) {
        ...
        if (error) {
          console.log(`   ⚠️  语句执行警告: ${error.message.substring(0, 100)}`);
        }
      }
    }
...
async function main() {
  try {
    ...
    // 确定要回滚的迁移
    const migrationsToRollback = findRollbackSteps(history, targetVersion, steps);
    ...
    migrationsToRollback.forEach(m => console.log(`   V${m.version} - ${m.description}`));
