...
class CronJobMonitor {
  ...
  async generateReport(config, routes, testResults, logFile) {
    ...
    console.log(
      `  模块测试通过率: ${passedTests}/${Object.keys(testResults).length}`
    );

    if (
      existingRoutes === totalRoutes &&
      passedTests === Object.keys(testResults).length
    ) {
      console.log('\n🎉 配置验证成功！');
    ...
    const reportFile = path.join(
      this.logsDir,
      `cron-monitor-report-${new Date().toISOString().split('T')[0]}.json`
    );
