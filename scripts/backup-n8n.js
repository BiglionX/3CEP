...
function backupPostgresDatabase() {
    ...
    const backupFile = path.join(CONFIG.backupDir, `n8n-db-${getCurrentTimestamp()}.sql`);
...
function backupN8nData() {
    ...
    const timestamp = getCurrentTimestamp();
    const backupFile = path.join(CONFIG.backupDir, `n8n-data-${timestamp}.tar.gz`);
    ...
    try {
        ...
        // 从容器中复制数据
        execSync(`docker cp ${CONFIG.containers.n8n}:/home/node/.n8n ${tempDir}/n8n-data`, { stdio: 'inherit' });
...
function backupDockerVolumes() {
    ...
    for (const volume of volumes) {
        try {
            const backupFile = path.join(CONFIG.backupDir, `${volume}-${getCurrentTimestamp()}.tar.gz`);
            
            // 创建卷备份
            execSync(`docker run --rm -v ${volume}:/volume -v ${CONFIG.backupDir}:/backup alpine tar czf /backup/${path.basename(backupFile)} -C /volume .`, { stdio: 'inherit' });
...
function cleanupOldBackups() {
    ...
    try {
        const files = fs.readdirSync(CONFIG.backupDir)
            .filter(file => file.startsWith('n8n-') && (file.endsWith('.tar.gz') || file.endsWith('.sql')))
            .map(file => ({
                name: file,
        ...
        console.log(`✅ 保留最新的 ${Math.min(files.length, CONFIG.keepLast)} 个备份`);
    } catch (error) {
...
function generateBackupReport(backupFiles, startTime) {
    ...
    const reportFile = path.join(CONFIG.backupDir, `backup-report-${getCurrentTimestamp()}.json`);
...
async function main() {
    ...
    try {
        ...
        try {
            execSync(`docker ps | grep -E "(${CONFIG.containers.postgres}|${CONFIG.containers.n8n})"`, { stdio: 'pipe' });
        ...
        console.log(`总大小: ${Math.round(backupFiles.reduce((total, file) => total + (fs.existsSync(file) ? fs.statSync(file).size : 0), 0) / 1024 / 1024)} MB`);
