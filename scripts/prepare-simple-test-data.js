...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY ||
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhyanF6YmhxdWVsZXN6a3Zuc2VuIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcyOTc1NjQ0MywiZXhwIjoyMDQ1MzMyNDQzfQ.YOUR_SERVICE_KEY_HERE'
);
  ...
  try {
    ...
    if (testPartIds.length > 0) {
      try {
        const priceResponse = await fetch(
          'http://localhost:3001/api/v1/parts/prices',
          {
            method: 'POST',
        ...
        console.log(
          `✅ 配件价格API测试: ${priceData.data?.length || 0} 条数据返回`
        );
      } catch (error) {
    ...
    try {
      const feedResponse = await fetch(
        'http://localhost:3001/api/v1/feed/hot?page=1&page_size=3'
      );
      const feedData = await feedResponse.json();
      console.log(
        `✅ 热点信息流API测试: ${feedData.data?.list?.length || 0} 条数据返回`
      );
    } catch (error) {
    ...
    try {
      const searchResponse = await fetch(
        'http://localhost:3001/api/v1/search?q=iPhone&page=1&page_size=3'
      );
      const searchData = await searchResponse.json();
      console.log(
        `✅ 搜索API测试: ${searchData.data?.list?.length || 0} 条数据返回`
      );
    } catch (error) {
