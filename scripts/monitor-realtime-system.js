      ...
      try {
          ...
          if (stat.totalProcessed > 0) {
            const errorRate =
              stat.totalProcessed > 0
                ? ((stat.errorCount / stat.totalProcessed) * 100).toFixed(2)
                : '0.00';
            console.log(`  ${eventType}:`);
            console.log(`    处理量: ${stat.totalProcessed}`);
            console.log(
              `    成功率: ${((stat.successCount / stat.totalProcessed) * 100).toFixed(2)}%`
            );
        ...
        if (alerts.length > 0) {
          ...
          alerts.slice(0, 5).forEach(alert => {
            ...
            console.log(
              `  ${severityIcon} [${alert.severity.toUpperCase()}] ${alert.title}`
            );
        ...
        console.log(
          `  内存总量: ${(memoryUsage.heapTotal / 1024 / 1024).toFixed(2)} MB`
        );
        console.log(
          `  CPU用户时间: ${(cpuUsage.user / 1000000).toFixed(2)} 秒`
        );
        console.log(
          `  CPU系统时间: ${(cpuUsage.system / 1000000).toFixed(2)} 秒\n`
        );
