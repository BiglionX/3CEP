  ...
  try {
    ...
    if (shop?.alliance_level) {
      switch (shop.alliance_level) {
        case 'silver': levelBonus = 1.1; break;
        case 'gold': levelBonus = 1.2; break;
        case 'diamond': levelBonus = 1.5; break;
        default: levelBonus = 1.0;
      }
    }
...
async function distributeReward(order, rewardAmount) {
  try {
    ...
    const { error: updateError } = await supabase
      .from('repair_shops')
      .update({ 
        fcx2_balance: supabase.rpc('coalesce', [{ fcx2_balance: 0 }, 0]) + rewardAmount
      })
      .eq('id', order.repair_shop_id);
  ...
  try {
    ...
    for (const order of orders) {
      try {
        ...
        log(`计算奖励: 基础${reward.baseReward}, 最终${reward.finalAmount} FCX2`);
        ...
        log(`❌ 工单 ${order.order_number} 处理失败: ${error.message}`, 'ERROR');
        failedCount++;
      }
    }
    ...
    log(`任务完成 - 总计: ${result.processed}, 成功: ${result.success}, 失败: ${result.failed}`);
    return result;
...
async function main() {
  try {
    ...
    // 输出汇总报告
    log('=== 任务执行汇总 ===');
    log(`奖励发放: 处理${rewardResult.processed}个工单，成功${rewardResult.success}个，失败${rewardResult.failed}个`);
