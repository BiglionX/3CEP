...
const templatePath = path.join(__dirname, '..', 'n8n-workflows', 'templates', 'agent-invoke-template.json');
const readmePath = path.join(__dirname, '..', 'n8n-workflows', 'templates', 'README.md');
...
try {
  ...
  console.log(`   - 文件大小: ${(templateContent.length / 1024).toFixed(2)} KB`);
  ...
  envVars.forEach(envVar => {
    const hasEnvVar = templateContent.includes(`$env.${envVar}`) || templateContent.includes(envVar);
