...
async function realUserLoginTest() {
  ...
  try {
    ...
    console.log('\n1️⃣ 访问登录页面...');
    await page.goto('http://localhost:3001/login?redirect=%2Fadmin%2Fdashboard');
    ...
    try {
      ...
      if (currentUrl.includes('/admin/dashboard')) {
        ...
        const hasAdminContent = pageContent.includes('管理后台') || 
        ...
        const errorMessage = await page.$eval('.text-red-700', el => el?.textContent || '').catch(() => '');
        if (errorMessage) {
    ...
    const supabaseCookies = cookies.filter(cookie => cookie.name.includes('sb-'));
