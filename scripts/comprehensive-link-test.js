  ...
  try {
    ...
    if (!poolError && hotLinkPool) {
      ...
      hotLinkPool.forEach((link, index) => {
        console.log(
          `   ${index + 1}. ${link.title} (${link.source}) - 状态: ${link.status}`
        );
      });
    }
    ...
    if (!queryError && queryResult) {
      ...
      queryResult.forEach((link, index) => {
        console.log(
          `   ${index + 1}. ${link.title} (优先级: ${link.priority})`
        );
      });
    } else {
    ...
    if (!priorityError && priorityLinks) {
      ...
      adjustedLinks.forEach(link => {
        console.log(
          `   ${link.title}: ${link.oldPriority} → ${link.newPriority}`
        );
      });
    }
    ...
    for (const tableName of tablesToCheck) {
      try {
        const { data } = await supabase.from(tableName).select('status');

        if (data) {
          stats[tableName] = {
            total: data.length,
            active: data.filter(
              item => item.status === 'active' || item.status === 'promoted'
            ).length,
            pending: data.filter(item => item.status === 'pending_review')
              .length,
          };
        }
      } catch (err) {
    ...
    Object.entries(stats).forEach(([table, stat]) => {
      console.log(
        `   ${table}: 总计${stat.total}条 (活跃${stat.active}条, 待审核${stat.pending}条)`
      );
    });
