...
try {
  ...
  console.log(`   包含Agent类型: ${[...new Set(testCases.map(tc => tc.agentName))].join(', ')}\n`);
} catch (error) {
...
try {
  ...
  console.log(`   Agent数量: ${Object.keys(metrics.agentPerformance).length}\n`);
} catch (error) {
...
console.log('   AGENTS_HOST=prod-server AGENTS_PORT=3001 npm run monitor:agents');
console.log('');
console.log('4. 查看帮助:');
