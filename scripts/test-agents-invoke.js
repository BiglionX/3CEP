    ...
    const req = http.request(config, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log(`   响应头: X-Mock-Response=${res.headers['x-mock-response'] || 'N/A'}, X-Execution-Time=${res.headers['x-execution-time'] || 'N/A'}`);
          ...
          if (isSuccess && !testCase.expectError) {
            ...
            if (result.data) {
              console.log(`   详细错误: ${JSON.stringify(result.data, null, 2)}`);
            }
          } else {
            console.log(`   ❌ 响应不符合预期`);
