...
async function deployLinkReviewTables() {
  ...
  try {
    ...
    const migrationPath = path.join(
      ...
      'supabase',
      'migrations',
      '005_create_link_review_tables.sql'
    );
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (statement.startsWith('SELECT') || statement.includes('status')) {
        console.log(
          `✅ 语句 ${i + 1}: 查询语句 - ${statement.substring(0, 50)}...`
        );
        successCount++;
        continue;
      }
      ...
      if (statement.startsWith('INSERT')) {
        console.log(
          `📋 语句 ${i + 1}: 插入数据 - ${statement.substring(0, 50)}...`
        );
        try {
          // 对于插入语句，我们需要特殊处理
          if (statement.includes('article_categories')) {
            await executeInsertStatement(
              supabase,
              'article_categories',
              statement
            );
      ...
      if (statement.startsWith('CREATE')) {
        console.log(
          `🔧 语句 ${i + 1}: 创建对象 - ${statement.substring(0, 50)}...`
        );
        try {
