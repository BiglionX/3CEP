...
function validateSqlSyntax(content, fileName) {
  ...
  // 检查禁止的语句
  VALIDATION_RULES.forbiddenStatements.patterns.forEach(({ regex, message }) => {
...
function validateVersionSequence(files) {
  ...
  const versions = files.map(file => {
    ...
    if (versionMatch) {
      ...
      return {
        ...
        numeric: parseInt(major) * 1000000 + parseInt(minor) * 1000 + parseInt(patch)
      };
    }
    return null;
  }).filter(Boolean);
  ...
  for (let i = 0; i < versions.length; i++) {
    ...
    if (i > 0) {
      ...
      if (versionDiff > 1001) { // 允许小版本和补丁版本的变化
