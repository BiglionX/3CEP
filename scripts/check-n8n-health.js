...
const HEALTH_CHECK_TIMEOUT =
  parseInt(process.env.N8N_HEALTH_CHECK_TIMEOUT) || 30;
...
const HEALTH_CHECKS = [
  {
    ...
    endpoint: '/healthz',
    validator: response =>
      response.status === 'ok' || response.message === 'n8n is running',
    required: true,
  },
  {
  ...
  },
  {
    ...
    validator: response => {
      ...
      // 检查最近执行是否有严重错误
      const recentErrors = response.results.filter(
...
async function runAllHealthChecks() {
  ...
  results.forEach((result, index) => {
    ...
    console.log(
      `${index + 1}. ${result.name}: ${statusIcon} ${result.message} ${requiredText}`
    );
  ...
  console.log('='.repeat(50));
  console.log(
    `必需检查: ${passedRequired}/${passedRequired + failedRequired} 通过`
  );
  console.log(
    `总体检查: ${results.filter(r => r.status === 'PASS').length}/${totalChecks} 通过`
  );
