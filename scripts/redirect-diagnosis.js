...
class RedirectDiagnosis {
  ...
  async checkMiddleware() {
    ...
    const checks = [
      ...
      },
      {
        ...
        found: content.match(/NextResponse\.redirect\(new URL\('\/login'/) !== null
      },
      {
        name: '未授权重定向',
        pattern: /NextResponse\.redirect\(new URL\('\/unauthorized'/,
        found: content.match(/NextResponse\.redirect\(new URL\('\/unauthorized'/) !== null
      }
    ];
  ...
  async analyzeRedirectChain() {
    ...
    console.log('2. 中间件检测未登录 -> 重定向到 /login?redirect=/admin/dashboard');
  ...
  generateReport() {
    ...
    const reportPath = path.join(process.cwd(), 'redirect-diagnosis-report.json');
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
    
    console.log(`📄 详细报告已保存至: ${path.relative(process.cwd(), reportPath)}`);
  }
}
