...
function testBuild() {
  ...
  if (buildResult.success) {
    ...
    if (hasBuild) {
      ...
      logTest(
        '构建大小检查',
        buildSize < 100 * 1024 * 1024 ? 'PASS' : 'WARN',
...
function checkSecurity() {
  ...
  if (fs.existsSync(nextConfigPath)) {
    ...
    const hasSecurityHeaders =
      content.includes('headers') || content.includes('security');
...
async function runDeploymentTests() {
  ...
  testResults.details.forEach(detail => {
    ...
    console.log(
      `  ${statusIcon} ${detail.name}${detail.message ? ` (${detail.message})` : ''}`
    );
  });
