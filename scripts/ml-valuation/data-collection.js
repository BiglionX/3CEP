...
class DeviceFeatureExtractor {
  ...
  static extractFeatures(deviceProfile) {
    ...
    // 时间特征
    features.age_months = this.calculateAgeInMonths(
      deviceProfile.manufacturingDate
    );
    ...
    features.cpu_score = this.getCpuPerformanceScore(
      specs.processor || specs.cpu
    );

    // 使用历史特征
    features.total_repair_count = deviceProfile.totalRepairCount || 0;
    features.part_replacement_count =
      deviceProfile.totalPartReplacementCount || 0;
...
class ConditionQuantifier {
  ...
  static quantifyCondition(lifecycleEvents) {
    ...
    // 分析维修和更换事件
    const repairEvents = lifecycleEvents.filter(
      e => e.eventType === 'repaired'
    );
    const partReplaceEvents = lifecycleEvents.filter(
      e => e.eventType === 'part_replaced'
    );

    // 根据维修历史调整成色
    condition.functionality_score = Math.max(
      0.1,
      1.0 - repairEvents.length * 0.1
    );
    ...
    partReplaceEvents.forEach(event => {
      ...
      if (subtype.includes('screen') || notes.includes('屏幕')) {
        condition.screen_condition = Math.max(
          0.3,
          condition.screen_condition - 0.3
        );
      }

      if (subtype.includes('battery') || notes.includes('电池')) {
        condition.battery_health = Math.max(
          0.2,
          condition.battery_health - 0.4
        );
      }

      if (notes.includes('外壳') || notes.includes('机身')) {
        condition.body_condition = Math.max(
          0.2,
          condition.body_condition - 0.2
        );
      }
    });
...
async function collectTrainingData() {
  ...
  try {
    ...
    for (const transaction of transactionData) {
      const deviceQrcodeId =
        transaction.device_qrcode_id || transaction.qrcodeId;
...
function generateDataReport(data) {
  ...
  console.log(
    `平均价格: ¥${(prices.reduce((a, b) => a + b, 0) / prices.length).toFixed(2)}`
  );
