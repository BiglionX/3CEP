...
class PageNavigationChecker {
  ...
  async checkPageAccessibility(page) {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        // 检查关键元素
        const hasExpectedElements = this.checkExpectedElements(response.body, page.category);
  ...
  checkNavigationElements(html) {
    const navIndicators = [
      ...
      'href=', 'link', 'button', 'click'
    ];
    return navIndicators.some(indicator => html.toLowerCase().includes(indicator));
  }

  // 检查按钮元素
  checkButtonElements(html) {
    const buttonIndicators = [
      '<button', 'btn', 'submit', 'onclick',
      'data-testid', 'role="button"'
    ];
    return buttonIndicators.some(indicator => html.toLowerCase().includes(indicator));
  }
  ...
  printSummary() {
    ...
    if (failedTests.length > 0) {
      ...
      failedTests.forEach(test => {
        console.log(`   • ${test.category} - ${test.page}: ${test.error || `HTTP ${test.statusCode}`}`);
      });
    }
    
    // 显示部分通过详情
    const partialTests = this.results.details.filter(d => d.status === 'partial');
    if (partialTests.length > 0) {
      ...
      partialTests.forEach(test => {
        ...
        console.log(`   • ${test.category} - ${test.page}: ${issues.join(', ')}`);
      });
    }
  }
