// Redis缓存性能测试脚本
const { cacheManager, generateCacheKey } = require('../src/utils/cache-manager');
...
async function testRedisCache() {
  ...
  try {
    ...
    const batchSuccess = batchResults.filter(r => r !== null).length === batchSize;
    ...
    setTimeout(async () => {
      ...
      console.log(`   过期后读取: ${expiredResult ? '❌ 仍有数据' : '✅ 已过期'}`);
    }, 2500);
    ...
    const successRate = ((successfulOperations / benchmarkIterations) * 100).toFixed(1);
    ...
    // 7. 清理测试数据
    console.log('\n7️⃣ 清理测试数据...');
    const cleanupPatterns = ['performance_test*', 'batch_test_*', 'benchmark_*'];
    let totalCleaned = 0;
