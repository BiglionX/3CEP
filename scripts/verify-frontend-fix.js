...
async function verifyFrontendFix() {
  ...
  try {
    ...
    // 等待页面加载完成
    await page.waitForSelector('[data-testid="auth-status"]', { timeout: 5000 });
    ...
    const adminPermission = await page.evaluate(() => {
      const adminEls = Array.from(document.querySelectorAll('*')).filter(el => 
        el.textContent && (el.textContent.includes('管理员权限') || el.textContent.includes('admin'))
      );
