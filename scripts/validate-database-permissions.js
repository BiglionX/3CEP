...
class DatabasePermissionValidator {
  ...
  async validateTenantIsolation() {
    try {
      ...
      if (tenantError) {
        this.logResult('租户隔离检查', 'FAIL', '租户表不存在或无法访问', tenantError)
        return false
      }
      ...
      if (userTenantError) {
        this.logResult('租户隔离检查', 'FAIL', '用户租户关联表不存在或无法访问', userTenantError)
        return false
      }
      
      // 检查关键业务表是否包含租户字段
      const tablesWithTenant = ['parts', 'uploaded_content', 'appointments']
      let missingTenantFields = []
  ...
  async validateSecurityViews() {
    ...
    for (const viewName of securityViews) {
      try {
        ...
        this.logResult(`安全视图-${viewName}`, 'FAIL', `视图检查异常`, error.message)
      }
    }
  }
  ...
  async validatePermissionFunctions() {
    ...
    for (const functionName of functions) {
      try {
        ...
        if (error && error.code !== 'P0001') { // P0001是预期的测试错误
          this.logResult(`权限函数-${functionName}`, 'FAIL', `函数调用失败`, error)
        } else {
          this.logResult(`权限函数-${functionName}`, 'PASS', '函数可用')
        }
      } catch (error) {
        this.logResult(`权限函数-${functionName}`, 'FAIL', `函数检查异常`, error.message)
      }
    }
  }
