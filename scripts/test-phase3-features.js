...
async function testPhase3Features() {
  ...
  try {
    const response = await fetch(`${baseUrl}/api/v1/recommend/personalized?user_id=test_user_123&strategy=hybrid&page=1&page_size=5`);
  ...
  console.log('\n2️⃣ 测试积分余额查询API...');
  try {
    const response = await fetch(`${baseUrl}/api/v1/points?user_id=test_user_123&action=get_balance`);
  ...
  console.log('\n3️⃣ 测试积分规则查询API...');
  try {
    const response = await fetch(`${baseUrl}/api/v1/points?user_id=anonymous&action=get_rules`);
  ...
  console.log('\n4️⃣ 测试积分历史查询API (无认证)...');
  try {
    const response = await fetch(`${baseUrl}/api/v1/points?user_id=test_user_123&action=get_history&page=1&page_size=10`);
  ...
  try {
    // 第一次请求（应该缓存未命中）
    const firstResponse = await fetch(`${baseUrl}/api/v1/points?user_id=test_user_123&action=get_rules`);
    const firstData = await firstResponse.json();
    const firstFromCache = firstData.from_cache || false;
    
    // 第二次请求（应该缓存命中）
    const secondResponse = await fetch(`${baseUrl}/api/v1/points?user_id=test_user_123&action=get_rules`);
    const secondData = await secondResponse.json();
    const secondFromCache = secondData.from_cache || false;
    
    console.log(`   第一次请求缓存状态: ${firstFromCache ? '命中' : '未命中'}`);
    console.log(`   第二次请求缓存状态: ${secondFromCache ? '命中' : '未命中'}`);
...
if (require.main === module) {
  testPhase3Features().then(result => {
    console.log('\n测试完成！');
