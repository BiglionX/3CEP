...
requiredVars.forEach(varName => {
  if (envVars[varName]) {
    console.log(
      `✅ ${varName}: ${envVars[varName].substring(0, 20)}${envVars[varName].length > 20 ? '...' : ''}`
    );
  } else {
...
if (iframeConfigValid) {
  ...
  if (envVars.N8N_SESSION_COOKIE_SAMESITE !== 'none') {
    console.warn(
      '⚠️  警告: N8N_SESSION_COOKIE_SAMESITE 应该设置为 none 以支持跨域'
    );
  }

  if (
    envVars.N8N_PROTOCOL === 'https' &&
    envVars.N8N_SESSION_COOKIE_SECURE !== 'true'
  ) {
    console.warn(
      '⚠️  警告: 使用 HTTPS 时 N8N_SESSION_COOKIE_SECURE 应该设置为 true'
    );
  }
} else {
...
// 检查必要目录
console.log('\n📁 检查必要目录结构:');
const requiredDirs = ['n8n', 'n8n/custom', 'n8n/logs'];
...
if (hasErrors) {
  ...
  console.log(
    '2. 或手动部署: docker-compose -f docker-compose.n8n.yml --env-file .env.n8n up -d'
  );
