...
async function initializeDatabase() {
  ...
  try {
    ...
    const { error: tableError } = await supabase.rpc('execute_sql', { sql: warehouseTableSQL })
    if (tableError) {
    ...
    const { error: operationError } = await supabase.rpc('execute_sql', { sql: operationTableSQL })
    if (operationError) {
    ...
    const { error: indexError } = await supabase.rpc('execute_sql', { sql: indexesSQL })
    if (indexError) {
    ...
    for (const warehouse of sampleWarehouses) {
      ...
      if (insertError) {
        if (insertError.code === '23505') {
          ...
          console.log(`   ❌ 插入仓库 ${warehouse.code} 失败:`, insertError.message)
        }
      } else {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          const { error: operationInsertError } = await supabase
            ...
            .insert([{
              ...
              completed_at: new Date(Date.now() - Math.random() * 29 * 24 * 60 * 60 * 1000),
              processing_time: 15 + Math.floor(Math.random() * 45),
              item_count: 10 + Math.floor(Math.random() * 190),
              total_value: 1000 + Math.random() * 9000,
              pick_time: operationType === 'outbound' ? 5 + Math.floor(Math.random() * 20) : null,
              pack_time: operationType === 'outbound' ? 3 + Math.floor(Math.random() * 12) : null,
              ship_time: operationType === 'outbound' ? 10 + Math.floor(Math.random() * 30) : null,
              status: Math.random() > 0.95 ? 'exception' : 'completed',
              accuracy: Math.random() > 0.02
            }])

          if (operationInsertError) {
            if (operationInsertError.code !== '23505') { // 忽略重复键错误
              console.log(`   ❌ 插入操作数据失败:`, operationInsertError.message)
            }
          }
        }
      }
