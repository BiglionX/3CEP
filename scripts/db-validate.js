...
function validateVersionSequence(migrations) {
  ...
  for (let i = 1; i < versions.length; i++) {
    ...
    // 检查版本是否递增
    if (
      currVer.major < prevVer.major ||
      (currVer.major === prevVer.major && currVer.minor < prevVer.minor) ||
      (currVer.major === prevVer.major &&
        currVer.minor === prevVer.minor &&
        currVer.patch < prevVer.patch)
    ) {
...
function validateSqlSyntax(content, fileName) {
  ...
  const incompleteStatements = statements.filter(
    stmt =>
      stmt.trim().length > 0 &&
      !stmt.trim().endsWith(';') &&
...
function validateContentQuality(content, fileName) {
  ...
  // 检查RLS策略
  const hasRLS =
    content.toUpperCase().includes('ROW LEVEL SECURITY') ||
...
function analyzeMigrationComplexity(content, fileName) {
  ...
  analysis.dataModifications =
    (upperContent.match(/INSERT INTO/g) || []).length +
    (upperContent.match(/UPDATE.*SET/g) || []).length +
    (upperContent.match(/DELETE FROM/g) || []).length;

  // 计算复杂度
  const complexityScore =
    analysis.tableCreations * 3 +
...
async function main() {
  try {
    ...
    // 1. 命名规范验证
    const namingResult = validateNamingConvention([
      ...migrations,
      ...rollbacks,
    ]);
    ...
    [...migrations, ...rollbacks].forEach(fileObj => {
      ...
      if (complexity.complexity !== 'low') {
        console.log(
          `     📊 复杂度: ${complexity.complexity} (${complexity.statementCount} 语句)`
        );
      }
    });
    ...
    console.log(
      `📊 语法检查: ${syntaxIssues === 0 ? '通过' : `${syntaxIssues} 个问题`}`
    );
    console.log(
      `📊 内容质量: ${contentIssues === 0 ? '良好' : `${contentIssues} 个建议`}`
    );
    console.log(
      `📊 迁移配对: ${pairingResult.validPairs}/${migrations.length} 完整`
    );
    ...
    if (pairingResult.orphanedRollbacks > 0) {
      console.log(
        `   ⚠️  存在 ${pairingResult.orphanedRollbacks} 个孤立回滚脚本`
      );
    }
