...
async function comprehensiveVerification() {
  ...
  try {
    ...
    console.log(`   结果: ${apiResult.success ? '✅' : '❌'} ${apiResult.message}`);
    
    // 2. 验证会话检查API
    console.log('\n2️⃣ 验证会话检查API...');
    const sessionResult = await testApiEndpoint('/api/auth/check-session');
    console.log(`   结果: ${sessionResult.success ? '✅' : '❌'} ${sessionResult.message}`);
    ...
    // 4. 总结
    console.log('\n📋 修复验证总结:');
    const allPassed = apiResult.success && sessionResult.success && fileCheck.every(c => c.status === '✅');
    ...
    if (allPassed) {
      ...
      console.log('3. 访问 http://localhost:3001/unified-auth-test 验证前端显示');
      ...
      if (!fileCheck.every(c => c.status === '✅')) console.log('   - 文件修复问题');
    }
  ...
  const checks = [
    {
      ...
      check: (content) => content.includes('await supabaseAdmin') && content.includes('isAdminUser')
    },
    {
