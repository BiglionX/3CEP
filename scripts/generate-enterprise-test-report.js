...
const {
  createTestReporter,
} = require('../tests/e2e/enterprise/utils/test-reporter');
...
async function collectPerformanceData(reporter) {
  ...
  try {
    ...
    if (perfData) {
      ...
      console.log(
        `     平均响应时间: ${perfData.avgResponseTime.toFixed(2)}ms`
      );
      console.log(
        `     最大响应时间: ${perfData.maxResponseTime.toFixed(2)}ms`
      );
    }
  } catch (error) {
    ...
    data.suites.forEach(suite => {
      suite.tests.forEach(test => {
        results.push({
          ...
          error:
            test.results && test.results[0]?.error
...
async function gatherSecurityFindings() {
  ...
  return {
    ...
    passedChecks: ['SQL注入防护', 'CSRF防护', '会话安全', '输入验证'],
  };
}
...
function printSummary(report) {
  ...
  report.suites.forEach(suite => {
    const passRate =
      suite.totalTests > 0
        ...
        : 0;
    console.log(
      `   ${suite.suiteName}: ${suite.passedTests}/${suite.totalTests} 通过 (${passRate}%)`
    );
  });
  ...
  if (report.performance) {
    console.log('\n⚡ 性能指标:');
    console.log(
      `   平均响应时间: ${report.performance.avgResponseTime.toFixed(2)}ms`
    );
    console.log(
      `   最大响应时间: ${report.performance.maxResponseTime.toFixed(2)}ms`
    );
    console.log(
      `   95%分位响应时间: ${report.performance.percentile95.toFixed(2)}ms`
    );
  }
...
function checkQualityGates(report) {
  ...
  // 安全漏洞检查 (高危漏洞=0)
  if (report.security) {
    const highVulns = report.security.vulnerabilities.filter(
      v => v.severity === 'high'
    ).length;
    if (highVulns > 0) {
  ...
  if (report.performance && report.performance.avgResponseTime > 3000) {
    failures.push(
      `平均响应时间 ${report.performance.avgResponseTime.toFixed(2)}ms 超过 3000ms 的基准`
    );
  }
