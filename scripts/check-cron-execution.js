...
class CronExecutionChecker {
  ...
  async checkExecutionStatus() {
    ...
    try {
      ...
      const cronLogs = logFiles.filter(file =>
        file.startsWith('cron-execution-')
      );
        ...
        executionEntries.slice(-10).forEach(entry => {
          const status = entry.content.includes('执行完成')
            ...
            : entry.content.includes('执行失败')
              ...
              : '⏳';
          console.log(
            `  ${status} [${entry.timestamp}] ${entry.content.replace(/\[.*?\]\s*/, '')}`
          );
        });
      } else {
      ...
      const now = new Date();
      const lastExecTime = lastExecution
        ? new Date(lastExecution.timestamp)
        : null;
  ...
  async manualTestExecution() {
    ...
    try {
      ...
      const hourlyTaskPath = path.join(
        __dirname,
        '..',
        'src',
        'app',
  ...
  async generateStatusReport() {
    ...
    try {
      ...
      if (cronLogs.length > 0) {
        ...
        const executionLines = lines.filter(
          line => line.includes('执行完成') || line.includes('执行失败')
        );
        ...
        if (executionLines.length > 0) {
          ...
          if (timestampMatch) {
            ...
            report.systemStatus = lastLine.includes('执行完成')
              ? 'healthy'
              : 'degraded';
          }
        }
    ...
    if (report.lastExecution) {
      ...
      console.log(
        `执行结果: ${report.lastExecution.success ? '成功' : '失败'}`
      );
    }
