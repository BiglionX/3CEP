...
async function runTestSuite(suiteKey, suiteConfig) {
  ...
  try {
    ...
    if (suiteConfig.command) {
      // 执行命令
      console.log(
        `🚀 执行: ${suiteConfig.command} ${suiteConfig.args.join(' ')}`
      );
    ...
    console.log(
      `📊 结果: ${suiteResult.status.toUpperCase()} (${Math.round(duration / 1000)}秒)`
    );
...
function getSelectedSuites() {
  ...
  if (options.suite) {
    ...
    if (TEST_CONFIG.suites[options.suite]) {
      suitesToRun = [
        { key: options.suite, config: TEST_CONFIG.suites[options.suite] },
      ];
    }
...
<testsuites name="FixCycle Unified Test Suite" 
  ...
  fs.writeFileSync(path.join(reportBaseDir, 'junit-report.xml'), junitContent);
}
...
function printSummary(report) {
  ...
  Object.entries(report.suites).forEach(([suiteKey, result]) => {
    ...
    console.log(
      `  ${statusIcons[result.status] || '❓'} ${suiteKey} [${result.status.toUpperCase()}] - ${Math.round(result.duration / 1000)}秒`
    );
  });

  console.log(`\n📄 报告文件位置:`);
  console.log(
    `   JSON报告: ${path.join(reportBaseDir, 'unified-test-report.json')}`
  );
  console.log(
    `   HTML报告: ${path.join(reportBaseDir, 'unified-test-report.html')}`
  );
