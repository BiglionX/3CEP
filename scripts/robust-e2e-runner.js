...
class RobustE2ETestRunner {
  ...
  async executeModule(moduleName, testFile) {
    ...
    try {
      ...
      console.log('🔍 执行环境检查...');
      execSync(
        'npx playwright test tests/e2e/basic-functionality.spec.ts --project=chromium-desktop',
        {
          stdio: 'pipe',
          timeout: 30000,
        }
      ...
      const result = execSync(
        `npx playwright test ${testFile} --project=chromium-desktop --workers=1 --timeout=120000`,
        {
          stdio: 'pipe',
      ...
      console.log(
        `📊 模块结果: ${passedCount} 通过, ${failedCount} 失败, 耗时 ${(duration / 1000).toFixed(1)}s`
      );
      ...
      console.log(
        `❌ 模块 ${moduleName} 执行失败: ${error.message.substring(0, 100)}`
      );
      return false;
    }
  }
  ...
  generateFinalReport() {
    ...
    console.log(
      `🕒 执行时间: ${(this.results.summary.totalTime / 1000).toFixed(1)}s`
    );
    ...
    const reportPath = path.join(
      process.cwd(),
      'test-results',
      'robust-e2e-execution-report.json'
    );
...
## 🎯 执行结果
${Object.entries(this.results.modules)
  .map(
    ([name, result]) =>
