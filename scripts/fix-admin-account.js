...
async function fixAdminAccount() {
  ...
  try {
    ...
    if (!targetUser) {
      ...
      console.log('当前邮箱确认状态:', targetUser.email_confirmed_at ? '已确认' : '未确认');
    ...
    const verifiedUser = finalUsers.users.find(user => user.email === adminEmail);
    ...
    if (verifiedUser) {
      ...
      console.log(`   邮箱确认: ${verifiedUser.email_confirmed_at ? '✅ 已确认' : '❌ 未确认'}`);
      console.log(`   角色: ${verifiedUser.user_metadata?.role || '未设置'}`);
      console.log(`   管理员: ${verifiedUser.user_metadata?.isAdmin ? '✅ 是' : '❌ 否'}`);
