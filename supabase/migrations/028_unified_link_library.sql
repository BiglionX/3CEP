-- 统一链接库表结构（整合hot_links和hot_link_pool）
-- Migration: 028_unified_link_library.sql
-- 创建时间: 2026-02-22
-- 版本: 1.0.0
...
CREATE INDEX IF NOT EXISTS idx_unified_link_library_search ON unified_link_library USING gin(
  (to_tsvector('chinese_zh', coalesce(title, '')) || 
   to_tsvector('chinese_zh', coalesce(description, '')))
);
