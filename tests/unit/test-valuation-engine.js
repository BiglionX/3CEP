...
} = require('../../src/lib/valuation/valuation-engine.service');
...
async function runValuationTests() {
  ...
  try {
    ...
    const result = await valuationService.calculateBaseValue(
      iphoneProfile,
      condition,
      6000
    );
    ...
    console.log(
      `   ⚙️  部件评分: ${(result.breakdown.componentAdjustment * 100).toFixed(1)}%`
    );
    console.log(
      `   🔧 成色乘数: ${(result.breakdown.conditionAdjustment * 100).toFixed(1)}%`
    );
    ...
    if (
      result.finalValue > 0 &&
      result.finalValue < result.breakdown.originalPrice
    ) {
  ...
  try {
    ...
    console.log(
      `   📉 折旧率: ${((result.breakdown.depreciation / result.breakdown.originalPrice) * 100).toFixed(1)}%`
    );
    console.log(
      `   🔧 维修历史影响: ${(result.breakdown.repairAdjustment * 100).toFixed(1)}%`
    );
  ...
  try {
    ...
    console.log(
      `   🏷️  品牌乘数: ${(result.breakdown.brandAdjustment * 100).toFixed(1)}%`
    );
    console.log(
      `   📱 类别乘数: ${(result.breakdown.categoryAdjustment * 100).toFixed(1)}%`
    );
