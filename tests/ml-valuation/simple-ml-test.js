...
async function runSimpleMLTests() {
  ...
  try {
    ...
    // 纯规则引擎估值
    const ruleResult = await valuationService.calculateBaseValue(
      testDevice,
      condition,
      6000
    );
    ...
    // 混合估值
    const hybridResult = await valuationService.calculateHybridValue(
      testDevice,
      condition,
      6000,
      true
  ...
  try {
    ...
    // 禁用ML的混合估值
    const disabledML = await valuationService.calculateHybridValue(
      device,
      undefined,
      4000,
      false
