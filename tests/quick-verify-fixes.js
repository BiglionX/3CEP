...
async function quickVerify() {
  ...
  try {
    ...
    const hasAnalytics = await page.isVisible('text=业务分析', {
      timeout: 3000,
    });
    ...
    const hasApprovals = await page.isVisible('text=审批管理', {
      timeout: 3000,
    });
    ...
    await page.goto(
      'http://localhost:3001/foreign-trade/company/order/PO-2026-001'
    );
    const hasOrderDetail = await page.isVisible('text=PO-2026-001', {
      timeout: 3000,
    });
    ...
    await page.goto(
      'http://localhost:3001/foreign-trade/company/orders/create'
    );
