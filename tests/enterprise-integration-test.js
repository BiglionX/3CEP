...
const ENTERPRISE_PAGES = [
  ...
  { name: '智能体定制服务', path: '/enterprise/agents/customize', category: '服务页面' },
  { name: 'B2B采购服务', path: '/enterprise/procurement', category: '服务页面' },
  
  // 企业管理后台
  { name: '企业认证页面', path: '/enterprise/admin/auth', category: '管理页面' },
  { name: '企业管理仪表板', path: '/enterprise/admin/dashboard', category: '管理页面' },
  { name: '智能体管理页面', path: '/enterprise/admin/agents', category: '管理页面' },
  { name: '采购管理页面', path: '/enterprise/admin/procurement', category: '管理页面' }
];

// 企业服务API端点
const ENTERPRISE_APIS = [
  { name: '企业注册接口', path: '/api/enterprise/register', method: 'POST', category: '认证API' },
  { name: '企业登录接口', path: '/api/enterprise/login', method: 'POST', category: '认证API' },
  { name: '智能体列表接口', path: '/api/enterprise/agents', method: 'GET', category: '业务API' },
  { name: '采购订单接口', path: '/api/enterprise/procurement/orders', method: 'GET', category: '业务API' }
];
...
async function testPage(page) {
  ...
  try {
    ...
    const hasHtmlContent = response.data.includes('<html') || response.data.includes('<div');
...
async function runIntegrationTest() {
  ...
  for (const api of ENTERPRISE_APIS) {
    ...
    if (result.isJson && result.responseData) {
      if (result.responseData.success !== undefined) {
        console.log(`   响应: ${result.responseData.success ? '成功' : '失败'}`);
      }
    }
    if (result.errorMessage) {
      console.log(`   错误: ${result.errorMessage}`);
  ...
  const passedTests = [...pageResults, ...apiResults].filter(r => r.status === 'PASS').length;
  const failedTests = [...pageResults, ...apiResults].filter(r => r.status === 'FAIL').length;
  const errorTests = [...pageResults, ...apiResults].filter(r => r.status === 'ERROR').length;
  ...
  console.log(`⚠️  错误: ${errorTests} (${((errorTests/totalTests)*100).toFixed(1)}%)`);
  
  // 分类统计
  console.log('\n📈 分类统计:');
  
  const categories = [...new Set([...pageResults, ...apiResults].map(r => r.category))];
  categories.forEach(category => {
    const categoryTests = [...pageResults, ...apiResults].filter(r => r.category === category);
    const categoryPassed = categoryTests.filter(r => r.status === 'PASS').length;
    const categoryTotal = categoryTests.length;
    console.log(`${category}: ${categoryPassed}/${categoryTotal} 通过 (${((categoryPassed/categoryTotal)*100).toFixed(1)}%)`);
  });
  
  // 性能统计
  console.log('\n⚡ 性能统计:');
  const allResponseTimes = [...pageResults, ...apiResults].map(r => r.responseTime);
