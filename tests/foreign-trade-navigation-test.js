...
async function testForeignTradeNavigation() {
  ...
  try {
    ...
    const hasStatsGrid = await page.isVisible('.grid.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-4');
    const statsCardCount = await page.locator('.grid.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-4 > div').count();
    ...
    // 2. 测试统计卡片点击跳转
    console.log('\n📋 测试 2: 统计卡片点击跳转...');
    const statCards = await page.locator('.grid.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-4 > div').all();
    let cardNavigationTests = [];
    
    for (let i = 0; i < Math.min(statCards.length, 4); i++) {
      const card = statCards[i];
      const cardText = await card.textContent();
      const cardTitle = await card.locator('.text-sm.font-medium').textContent();
      ...
      try {
        ...
        const navigated = currentUrl !== 'http://localhost:3001/foreign-trade/company';
    ...
    testResults.push({
      test: '统计卡片跳转功能',
      status: cardNavigationTests.some(test => test.点击后跳转) ? '✅ 部分通过' : '⚠️ 未发现跳转',
    ...
    // 测试特定按钮
    const createOrderBtn = await page.locator('button:has-text("创建订单")').first();
    const exportBtn = await page.locator('button:has-text("导出数据")').first();
    const viewDetailsBtn = await page.locator('button:has-text("查看详情")').first();
    ...
    // 4. 测试订单详情跳转
    console.log('\n📋 测试 4: 订单详情跳转...');
    const orderItems = await page.locator('.border.border-gray-200.rounded-lg.p-4.cursor-pointer').all();
    let orderDetailTests = [];
    ...
    testResults.push({
      test: '订单详情跳转',
      status: orderDetailTests.length > 0 ? 
        (orderDetailTests[0].成功跳转 ? '✅ 通过' : '❌ 失败') : 
        '⚠️ 未找到可点击的订单项',
