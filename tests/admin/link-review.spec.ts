...
test.describe('管理后台热点审核测试 (E2E-ADMIN-01/02/03)', () => {
  ...
  test('E2E-ADMIN-01: 管理员登录与热点审核页访问', async () => {
    ...
    await TestHelpers.waitForElement(
      adminPage,
      '[data-testid="pending-links-list"]'
    );
    ...
    await adminPage.screenshot({
      path: `test-results/admin-link-review-access-${Date.now()}.png`,
    });
  });
  ...
  test('E2E-ADMIN-02: 热点链接审核通过流程', async () => {
    ...
    // 2. 找到第一条待审核链接
    const firstPendingLink = await TestHelpers.waitForElement(
      adminPage,
      '.pending-link-item:first-child'
    );

    // 3. 记录审核前的状态
    const initialStatus = await firstPendingLink.$(
    ...
    // 8. 验证状态变更为已发布
    const updatedStatus = await firstPendingLink.$(
      '[data-testid="link-status"]'
    );
    ...
    await TestHelpers.waitForElement(
      adminPage,
      '[data-testid="published-articles-list"]'
    );
    ...
    await adminPage.screenshot({
      path: `test-results/admin-link-approved-${Date.now()}.png`,
    });
  });
  ...
  test('E2E-ADMIN-03: 热点链接驳回操作', async () => {
    ...
    // 2. 找到一条待审核链接
    const pendingLink = await TestHelpers.waitForElement(
      adminPage,
      '.pending-link-item:nth-child(2)'
    );
    ...
    await adminPage.screenshot({
      path: `test-results/admin-link-rejected-${Date.now()}.png`,
    });
  });
});
