...
async function verifyFixes() {
  ...
  try {
    ...
    if (statCards.length > 0) {
      ...
      const navigated =
        afterClickUrl.includes('/orders') && afterClickUrl !== initialUrl;
    ...
    const createOrderBtn = await page.$(
      'button:has-text("创建采购订单"), button:has-text("创建销售订单")'
    );
    ...
    if (createOrderBtn) {
      ...
      const afterClickUrl = page.url();
      const navigated =
        afterClickUrl.includes('/orders/create') &&
        afterClickUrl !== initialUrl;
