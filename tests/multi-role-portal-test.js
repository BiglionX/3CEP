...
const testConfig = {
  portalPages: [
    {
      path: '/',
      name: '主门户页面',
      expectedElements: ['角色选择', '维修店', '进口商', '出口商'],
    },
    {
      path: '/repair-shop/login',
      name: '维修店登录页',
      expectedElements: ['维修师登录', '用户名', '密码'],
    },
    {
      path: '/repair-shop/dashboard',
      name: '维修店仪表板',
      expectedElements: ['工单管理', '设备诊断', '今日工单'],
    },
    {
      path: '/repair-shop/work-orders',
      name: '工单管理页',
      expectedElements: ['新建工单', '工单列表', '筛选'],
    },
    {
      path: '/repair-shop/diagnostics',
      name: '设备诊断页',
      expectedElements: ['设备类型', '开始诊断', '测试结果'],
    },
    {
      path: '/importer/login',
      name: '进口商登录页',
      expectedElements: ['进口商登录', '公司名称', '用户名'],
    },
    {
      path: '/importer/dashboard',
      name: '进口商仪表板',
      expectedElements: ['采购订单', '供应商管理', '本月采购额'],
    },
    {
      path: '/importer/procurement',
      name: '采购管理页',
      expectedElements: ['采购订单', '新建订单', '供应商'],
    },
    {
      path: '/importer/logistics',
      name: '物流跟踪页',
      expectedElements: ['物流查询', '运输列表', '跟踪详情'],
    },
    {
      path: '/exporter/login',
      name: '出口商登录页',
      expectedElements: ['出口商登录', '公司名称', '用户名'],
    },
    {
      path: '/exporter/trading',
      name: '销售订单页',
      expectedElements: ['销售订单', '新建订单', '客户管理'],
    },
  ],
  ...
  systemComponents: [
    ...
    {
      name: '门户导航组件',
      path: 'src/components/PortalNavigation.tsx',
      required: false,
    },
    {
      name: '权限控制Hook',
  ...
  testConfig.systemComponents.forEach(component => {
    ...
    if (exists) {
      ...
      if (component.required) {
        console.log(
          `❌ ${component.name} - 缺失 (${component.path}) ⚠️ 必需文件`
        );
        allFilesExist = false;
      } else {
...
function generateReport(validationResults) {
  ...
  const reportPath = path.join(
    process.cwd(),
    'test-results',
    'multi-role-portal-test-report.json'
  );
...
async function runMultiRoleTests() {
  try {
    ...
    console.log(
      `   整体状态: ${report.summary.overallStatus === 'PASS' ? '✅ 通过' : '❌ 失败'}`
    );
    ...
    console.log('\n📋 详细报告已生成:');
    console.log(
      `   ${path.join(process.cwd(), 'test-results', 'multi-role-portal-test-report.json')}`
    );
