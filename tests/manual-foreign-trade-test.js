...
async function manualForeignTradeTest() {
  ...
  try {
    ...
    await page.screenshot({
      path: 'test-results/foreign-trade-homepage.png',
      fullPage: true,
    });
    ...
    const pageElements = {
      ...
      订单标签页: await page
        .$eval('button[data-state]', el => el.textContent.trim())
        .catch(() => '未找到'),
      角色指示器: await page
        .$eval('.flex.items-center.justify-between .text-sm', el =>
          el.textContent.trim()
        )
        .catch(() => '未找到'),
    };
    ...
    roleElements.slice(0, 5).forEach((el, index) => {
      console.log(
        `  ${index + 1}. ${el.tagName} - ${el.text.substring(0, 30)}...`
      );
    });
    ...
    try {
      await page
        .click('[data-testid="role-toggle"]', { timeout: 2000 })
        .catch(async () => {
    ...
    if (sidebar) {
      ...
      for (let i = 0; i < Math.min(sidebarItems.length, 3); i++) {
        try {
          const itemText = await sidebarItems[i].evaluate(el =>
            el.textContent?.trim()
          );
    ...
    // 保存测试报告
    fs.writeFileSync(
      'test-results/foreign-trade-manual-test-report.json',
      JSON.stringify(testSummary, null, 2)
    );

    console.log('\n🎉 外贸公司管理平台测试完成！');
    console.log(
      '📄 测试报告已保存到: test-results/foreign-trade-manual-test-report.json'
    );
