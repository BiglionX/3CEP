...
async function simpleForeignTradeCheck() {
  ...
  try {
    ...
    // 检查是否存在关键元素
    const hasImportHeader = await page.isVisible('text=进口商业务中心', { timeout: 3000 });
    const hasExportHeader = await page.isVisible('text=出口商业务中心', { timeout: 3000 });
    ...
    for (let i = 0; i < Math.min(buttons.length, 10); i++) {
      ...
      console.log(`  ${i + 1}. ${buttonText?.trim()} (${buttonClass?.includes('outline') ? '轮廓' : '实心'})`);
    }
    ...
    if (statCards.length > 0) {
      ...
      console.log('📄 第一个卡片内容:', firstCardText?.substring(0, 50) + '...');
    ...
    if (createOrderButtons.length > 0) {
      ...
      try {
        ...
        console.log('  点击前:', initialUrl);
        console.log('  点击后:', newUrl);
        console.log('  是否跳转:', newUrl !== initialUrl && newUrl.includes('create'));
      } catch (error) {
    ...
    if (orderItems.length > 0) {
      ...
      const hasPointer = await firstOrder.evaluate(el => el.classList.contains('cursor-pointer'));
