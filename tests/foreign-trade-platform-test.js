...
async function testForeignTradePlatform() {
  ...
  try {
    ...
    testResults.push({
      test: '访问外贸公司管理平台',
      status: hasHeader && hasStatsCards && hasCompanyList ? '✅ 通过' : '❌ 失败',
      details: {
        页面标题: pageTitle,
        平台头部显示: hasHeader,
    ...
    // 2. 测试角色切换功能
    console.log('\n📋 测试 2: 角色切换功能...');
    const initialRole = await page.locator('[data-testid="role-toggle"]').textContent();
    
    // 切换角色
    await page.click('[data-testid="role-toggle"]');
    await page.waitForTimeout(1000);
    
    const switchedRole = await page.locator('[data-testid="role-toggle"]').textContent();
    ...
    for (let i = 0; i < Math.min(statCards, 4); i++) {
      ...
      const navigated = currentUrl.includes('/foreign-trade/') && currentUrl !== 'http://localhost:3001/foreign-trade/company';
    ...
    testResults.push({
      test: '统计卡片跳转功能',
      status: navigationTests.every(test => test.跳转成功) ? '✅ 通过' : '⚠️ 部分通过',
    ...
    const hasSearchButton = await page.isVisible('[data-testid="search-button"]');
    
    testResults.push({
      test: '搜索功能',
      status: searchValue === '测试公司' && hasSearchButton ? '✅ 通过' : '❌ 失败',
      details: {
    ...
    const filterDropdownVisible = await page.isVisible('[data-testid="filter-dropdown"]');
    ...
    const companyItems = await page.locator('[data-testid="company-item"]').count();
    let companyItemTests = [];
    ...
    if (companyItems > 0) {
      ...
      // 测试删除功能
      const deleteButton = firstCompany.locator('[data-testid="delete-button"]');
      ...
      testResults.push({
        test: '公司列表项功能',
        status: hasViewButton && hasEditButton && hasDeleteButton ? '✅ 通过' : '⚠️ 部分通过',
    ...
    const sidebarItems = await page.locator('[data-testid="sidebar-item"]').count();
    let sidebarTests = [];
    ...
    const hasLoadingIndicator = await page.isVisible('[data-testid="loading-spinner"]', { timeout: 1000 }).catch(() => false);
    ...
    fs.writeFileSync('foreign-trade-platform-test-report.json', JSON.stringify(report, null, 2));
    fs.writeFileSync('foreign-trade-platform-test-report.md', generateMarkdownReport(report));
