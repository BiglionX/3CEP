...
async function runSimpleTests() {
  ...
  const result1 = await engine.calculateBaseValue(
    iphoneProfile,
    condition,
    6000
  );
  ...
  console.log(
    `   🔧 成色乘数: ${(result1.conditionMultiplier * 100).toFixed(1)}%\n`
  );
  ...
  const result2 = await engine.calculateBaseValue(
    oldIphoneProfile,
    poorCondition,
    4000
  );
  ...
  console.log(
    `   📉 折旧影响: ${((result2.breakdown.depreciation / result2.breakdown.originalPrice) * 100).toFixed(1)}%`
  );
  console.log(
    `   🔧 维修影响: ${(result2.breakdown.repairAdjustment * 100).toFixed(1)}%\n`
  );
  ...
  const result3 = await engine.calculateBaseValue(
    macbookProfile,
    excellentCondition,
    15000
  );
  ...
  console.log(
    `   🏷️  品牌乘数: ${(result3.breakdown.brandAdjustment * 100).toFixed(1)}%`
  );
  console.log(
    `   📱 类别乘数: ${(result3.breakdown.categoryAdjustment * 100).toFixed(1)}%\n`
  );
