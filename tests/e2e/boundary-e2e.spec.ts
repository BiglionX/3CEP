...
test.describe('边界情况端到端测试', () => {
  ...
  test('TC-001: 点赞3次触发草稿沉淀', async ({ page }) => {
    ...
    // 验证草稿创建提示
    await expect(
      page.locator('[data-testid="draft-success-toast"]')
  ...
  test('TC-002: 点赞超过3次不重复触发', async ({ page }) => {
    ...
    // 尝试第4次点赞
    const draftCountBefore = await page
      .locator('[data-testid="draft-count"]')
      .count();
    await page.click('[data-testid="like-button"]');
    await page.waitForTimeout(500);

    // 验证草稿数量没有增加
    const draftCountAfter = await page
      .locator('[data-testid="draft-count"]')
      .count();
  ...
  test('TC-004: 重复URL上传拒绝', async ({ page }) => {
    ...
    await page.fill(
      ...
      'https://example.com/existing-content'
    );
  ...
  test('TC-010: 预约时间冲突处理', async ({ page }) => {
    ...
    // 验证确认按钮被禁用
    await expect(
      page.locator('[data-testid="confirm-appointment"]')
    ).toBeDisabled();
  });
  ...
  test('TC-012: 连续时间段预约', async ({ page }) => {
    ...
    await expect(
      page.locator(
        '[data-testid="slot-1100-1200"] [data-lucide="check-circle"]'
      )
    ).toBeVisible();
