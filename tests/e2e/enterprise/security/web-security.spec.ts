...
test.describe('输入验证安全测试', () => {
  ...
  test('特殊字符输入验证', async ({ page }) => {
    ...
    for (const char of specialCharacters) {
      ...
      // 检查是否有错误提示
      const hasValidationError = await page.getByText(/无效字符|invalid character|格式错误|format error/i).isVisible();
      
      // 或者检查输入是否被清理
      const isSanitized = !inputValue.includes(char) && inputValue !== `test${char}@example.com`;
  ...
  test('文件上传安全测试', async ({ page }) => {
    ...
    for (const fileName of maliciousFiles) {
      ...
      if (await fileInput.isVisible()) {
        ...
        // 验证文件类型限制
        const hasFileTypeError = await page.getByText(/不支持的文件类型|unsupported file type|只允许上传|only allow/i).isVisible();
...
test.describe('XSS攻击防护测试', () => {
  test('反射型XSS防护', async ({ page, request }) => {
    ...
    for (const payload of SECURITY_TEST_CONFIG.xssPayloads) {
      ...
      await enterpriseUtils.navigateTo(`/enterprise/search?q=${encodedPayload}`);
  ...
  test('存储型XSS防护', async ({ page, request }) => {
    ...
    for (const payload of SECURITY_TEST_CONFIG.xssPayloads) {
      ...
      if (await bioInput.isVisible()) {
        ...
        // 验证载荷被正确处理
        const hasError = await page.getByText(/包含非法字符|contains illegal characters|输入无效|invalid input/i).isVisible();
        expect(hasError).toBeTruthy();
      }
    }
  });
...
test.describe('敏感信息保护测试', () => {
  test('错误信息泄露测试', async ({ request }) => {
    ...
    // 验证不泄露系统信息
    expect(JSON.stringify(result1)).not.toContain(/stack|trace|exception|database|table/i);
    ...
    // 验证不泄露数据库错误详情
    expect(JSON.stringify(result2)).not.toContain(/sql|mysql|postgresql|sqlite/i);
  });
