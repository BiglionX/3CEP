...
test.describe('企业管理后台功能测试', () => {
  test('企业仪表板访问和数据显示', async ({ page, request }) => {
    ...
    // 验证仪表板页面加载
    await expect(
      page.getByRole('heading', { name: /仪表板|Dashboard/ })
    ).toBeVisible();
  ...
  test('用户管理功能测试', async ({ page, request }) => {
    ...
    // 验证用户管理页面
    await expect(
      page.getByRole('heading', { name: /用户管理|User Management/ })
    ).toBeVisible();
    ...
    // 验证操作按钮
    await expect(
      page.getByRole('button', { name: /添加用户|Add User/ })
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: /导出|Export/ })
    ).toBeVisible();
  });
  ...
  test('用户详情查看功能', async ({ page, request }) => {
    ...
    // 验证用户详情弹窗或页面
    await expect(
      page.getByRole('dialog').or(page.getByTestId('user-detail-panel'))
    ).toBeVisible();
  ...
  test('权限配置功能测试', async ({ page, request }) => {
    ...
    // 验证权限管理页面
    await expect(
      page.getByRole('heading', { name: /权限管理|Permission Management/ })
    ).toBeVisible();
    ...
    // 验证权限勾选框显示
    const permissionCheckboxes = permissionsPanel.locator(
      'input[type="checkbox"]'
    );
  ...
  test('数据统计和报表功能', async ({ page, request }) => {
    ...
    // 验证报表页面
    await expect(
      page.getByRole('heading', { name: /数据报表|Reports/ })
    ).toBeVisible();
    ...
    for (const reportType of reportTypes) {
      await expect(
        page.getByRole('option', { name: reportType })
      ).toBeVisible();
    }

    // 生成报表测试
    await page
      .getByRole('button', { name: /生成报表|Generate Report/ })
      .click();
    ...
    // 验证报表下载按钮
    await expect(
      page.getByRole('button', { name: /下载|Download/ })
    ).toBeVisible();
  });
  ...
  test('通知中心功能测试', async ({ page, request }) => {
    ...
    // 验证通知中心页面
    await expect(
      page.getByRole('heading', { name: /通知中心|Notifications/ })
    ).toBeVisible();
    ...
    // 验证通知标记为已读功能
    const unreadNotifications = notificationsList.getByTestId(
      'unread-notification'
    );
    if ((await unreadNotifications.count()) > 0) {
      const firstUnread = unreadNotifications.first();
      await firstUnread
        .getByRole('button', { name: /标记为已读|Mark as read/ })
        .click();
  ...
  test('审计日志功能测试', async ({ page, request }) => {
    ...
    // 验证审计日志页面
    await expect(
      page.getByRole('heading', { name: /审计日志|Audit Log/ })
    ).toBeVisible();
    ...
    // 验证日志导出功能
    await expect(
      page.getByRole('button', { name: /导出日志|Export Logs/ })
    ).toBeVisible();
...
test.describe('管理员专属功能测试', () => {
  ...
  test('系统健康状态监控', async ({ page, request }) => {
    ...
    // 验证系统状态面板
    await expect(
      page.getByRole('heading', { name: /系统状态|System Status/ })
    ).toBeVisible();
    ...
    for (const service of services) {
      await expect(
        page.getByTestId(`service-status-${service.toLowerCase()}`)
      ).toBeVisible();
    }
