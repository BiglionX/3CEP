...
test.describe('企业用户认证流程测试', () => {
  test('企业用户注册流程测试', async ({ page, request }) => {
    ...
    // 验证注册页面元素
    await expect(
      page.getByRole('heading', { name: /注册|Register/ })
    ).toBeVisible();
    ...
    await page.waitForURL(
      `http://localhost:3003${ENTERPRISE_ROUTES.dashboard}`
    );
  ...
  test('企业用户登录流程测试', async ({ page, request }) => {
    ...
    // 验证登录页面元素
    await expect(
      page.getByRole('heading', { name: /登录|Login/ })
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('button', { name: /退出|Logout/ })
    ).toBeVisible();
  });
  ...
  test('企业用户登出功能测试', async ({ page, request }) => {
    ...
    // 验证已登录状态
    await expect(
      page.getByText(TEST_ENTERPRISE_USERS.regularUser.companyName)
    ).toBeVisible();
  ...
  test('忘记密码功能测试', async ({ page, request }) => {
    ...
    // 验证忘记密码页面
    await expect(
      page.getByRole('heading', { name: /重置密码|Reset Password/ })
    ).toBeVisible();
    await expect(page.getByPlaceholder(/邮箱|Email/)).toBeVisible();

    // 填写邮箱
    await page
      .getByPlaceholder(/邮箱|Email/)
      .fill(TEST_ENTERPRISE_USERS.regularUser.email);
  ...
  test('注册表单验证测试', async ({ page, request }) => {
    ...
    // 验证密码强度提示
    await expect(
      page.getByText(/密码长度至少8位|at least 8 characters/i)
    ).toBeVisible();
  });
  ...
  test('多设备登录测试', async ({ browser, request }) => {
    ...
    // 验证两个会话都可以正常使用
    await expect(
      secondPage.getByText(TEST_ENTERPRISE_USERS.regularUser.companyName)
    ).toBeVisible();
