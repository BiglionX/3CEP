...
test.describe('企业服务门户功能测试', () => {
  ...
  test('企业服务介绍页面功能验证', async ({ page }) => {
    ...
    // 验证CTA按钮存在
    await expect(page.getByRole('button', { name: /立即咨询|联系我们|获取报价/ })).toBeVisible();
  });
  ...
  test('企业联系方式页面功能验证', async ({ page }) => {
    ...
    // 验证表单字段
    await expect(contactForm.getByPlaceholder(/公司名称|企业名称/)).toBeVisible();
  ...
  test('企业门户导航功能测试', async ({ page }) => {
    ...
    const navLinks = [
      ...
      { selector: 'a[href="/enterprise/services"]', expectedPath: '/enterprise/services' },
      { selector: 'a[href="/enterprise/contact"]', expectedPath: '/enterprise/contact' }
    ];
