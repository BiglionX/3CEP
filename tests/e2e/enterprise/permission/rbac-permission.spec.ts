...
test.describe('角色权限验证测试', () => {
  test('企业管理员权限验证', async ({ page, request }) => {
    ...
    await expect(
      page.getByRole('link', { name: /用户管理|User Management/ })
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: /系统设置|System Settings/ })
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: /权限管理|Permission Management/ })
    ).toBeVisible();
  });
  ...
  test('采购经理权限验证', async ({ page, request }) => {
    ...
    await enterpriseUtils.navigateTo(ENTERPRISE_ROUTES.dashboard);
    await expect(
      page.getByRole('link', { name: /采购订单|Purchase Orders/ })
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: /供应商管理|Supplier Management/ })
    ).toBeVisible();
  });
  ...
  test('智能体操作员权限验证', async ({ page, request }) => {
    ...
    await enterpriseUtils.navigateTo(ENTERPRISE_ROUTES.dashboard);
    await expect(
      page.getByRole('link', { name: /我的智能体|My Agents/ })
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: /工作流|Workflows/ })
    ).toBeVisible();
  });
  ...
  test('普通用户权限验证', async ({ page, request }) => {
    ...
    // 测试基础访问权限
    const basicRoutes = [ENTERPRISE_ROUTES.dashboard, '/enterprise/profile'];
...
test.describe('细粒度权限控制测试', () => {
  ...
  test('按钮操作权限控制', async ({ page, request }) => {
    ...
    await expect(
      page.getByRole('button', { name: /编辑|Edit/ }).first()
    ).toBeVisible();
    await expect(
      page.getByRole('button', { name: /删除|Delete/ }).first()
    ).toBeVisible();
...
test.describe('API权限验证', () => {
  test('API端点权限控制', async ({ request }) => {
    ...
    // 管理员访问
    const adminToken = await getTestUserToken(
      request,
