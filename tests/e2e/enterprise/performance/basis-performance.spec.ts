...
import { ENTERPRISE_ROUTES, PERFORMANCE_BENCHMARKS } from '../enterprise.config';
...
test.describe('页面加载性能测试', () => {
  test('企业首页加载性能', async ({ page }) => {
    ...
    console.log(`企业首页加载时间: ${loadTime}ms (阈值: ${PERFORMANCE_BENCHMARKS.pageLoadThreshold}ms)`);
  ...
  test('移动端页面加载性能', async ({ page }) => {
    ...
    // 移动端可能稍微宽松一些的阈值
    expect(loadTime).toBeLessThan(PERFORMANCE_BENCHMARKS.pageLoadThreshold * 1.5);
...
test.describe('API响应性能测试', () => {
  ...
  test('认证API响应时间', async ({ request }) => {
    ...
    expect(responseTime).toBeLessThan(PERFORMANCE_BENCHMARKS.apiResponseThreshold);
...
test.describe('并发性能测试', () => {
  test('多用户并发登录测试', async ({ browser }) => {
    ...
    for (let i = 0; i < concurrentUsers; i++) {
      ...
      const loginPromise = enterpriseUtils.loginAs('regularUser')
        .then(() => {
  ...
  test('静态资源加载时间', async ({ page }) => {
    ...
    Object.entries(resourceStats).forEach(([type, resources]) => {
      if (resources.length > 0) {
        const avgTime = resources.reduce((sum, r) => sum + (r.timing.responseEnd - r.timing.requestStart), 0) / resources.length;
        console.log(`${type}: ${resources.length}个, 平均加载时间: ${avgTime.toFixed(2)}ms`);
      }
    });
...
test.afterAll(async () => {
  console.log('\n=== 性能测试基准报告 ===');
  console.log(`页面加载时间阈值: ${PERFORMANCE_BENCHMARKS.pageLoadThreshold}ms`);
  console.log(`API响应时间阈值: ${PERFORMANCE_BENCHMARKS.apiResponseThreshold}ms`);
