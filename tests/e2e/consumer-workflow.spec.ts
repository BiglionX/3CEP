...
test.describe('消费者核心流程测试 (E2E-CON-*)', () => {
  ...
  test('E2E-CON-01: 消费者登录与内容浏览', async () => {
    ...
    // 3. 等待登录完成
    await consumerPage.waitForURL(TEST_CONFIG.BASE_URL + '/', { timeout: TEST_CONFIG.timeouts.pageLoad });
    
    // 4. 验证首页内容展示
    await TestHelpers.waitForElement(consumerPage, '[data-testid="content-feed"]');
    ...
    // 7. 验证简单难度文章优先展示
    const easyDifficultyTags = await consumerPage.$$('.difficulty-tag:has-text("简单")');
    expect(easyDifficultyTags.length).toBeGreaterThan(0);
    
    await consumerPage.screenshot({ path: `test-results/consumer-login-feed-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-02: 故障相关搜索', async () => {
    ...
    // 3. 等待搜索结果
    await TestHelpers.waitForElement(consumerPage, '[data-testid="search-results"]');
    ...
    // 5. 验证相关文章优先显示
    const firstResultTitle = await consumerPage.$('.search-result-item:first-child .result-title');
    const titleText = await firstResultTitle.textContent();
    expect(titleText.toLowerCase()).toContain('电池');
    
    await consumerPage.screenshot({ path: `test-results/consumer-search-battery-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-03: 文章详情与附近店铺查看', async () => {
    ...
    // 2. 点击第一篇文章进入详情页
    const firstArticle = await TestHelpers.waitForElement(consumerPage, '.article-card:first-child');
    await firstArticle.click();
    
    // 3. 等待详情页加载
    await TestHelpers.waitForElement(consumerPage, '[data-testid="article-detail"]');
    
    // 4. 验证文章内容显示
    const articleContent = await consumerPage.$('[data-testid="article-content"]');
    expect(articleContent).toBeTruthy();
    
    // 5. 滚动到底部查看附近店铺
    await consumerPage.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    ...
    await consumerPage.screenshot({ path: `test-results/consumer-article-detail-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-04: 维修估价工具', async () => {
    ...
    // 3. 等待估价页面加载
    await TestHelpers.waitForElement(consumerPage, '[data-testid="estimate-form"]');
    
    // 4. 选择设备和故障类型
    await consumerPage.selectOption('[data-testid="device-select"]', 'iPhone 12');
    ...
    // 6. 等待结果展示
    await TestHelpers.waitForElement(consumerPage, '[data-testid="estimate-results"]');
    ...
    await consumerPage.screenshot({ path: `test-results/consumer-estimate-tool-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-05: 附近店铺列表模式', async () => {
    ...
    // 3. 确保在列表模式
    const listViewButton = await consumerPage.$('[data-testid="list-view-button"]');
    ...
    await consumerPage.screenshot({ path: `test-results/consumer-shop-list-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-06: 地图模式切换', async () => {
    ...
    await consumerPage.screenshot({ path: `test-results/consumer-shop-map-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-07: 店铺详情查看', async () => {
    ...
    // 2. 点击第一个店铺进入详情页
    const firstShopCard = await TestHelpers.waitForElement(consumerPage, '.shop-card:first-child');
    await firstShopCard.click();
    
    // 3. 等待详情页加载
    await TestHelpers.waitForElement(consumerPage, '[data-testid="shop-detail"]');
    ...
    // 5. 验证服务报价展示
    const servicePrices = await consumerPage.$('[data-testid="service-prices"]');
    ...
    await consumerPage.screenshot({ path: `test-results/consumer-shop-detail-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-08: 预约维修流程', async () => {
    ...
    // 3. 等待预约表单加载
    await TestHelpers.waitForElement(consumerPage, '[data-testid="appointment-form"]');
    ...
    // 7. 等待预约确认
    await TestHelpers.waitForElement(consumerPage, '[data-testid="appointment-success"]');
    
    // 8. 验证预约成功提示
    const successMessage = await consumerPage.$('[data-testid="appointment-success"]');
    expect(successMessage).toBeTruthy();
    
    // 9. 验证预约状态为待确认
    const appointmentStatus = await consumerPage.$('[data-testid="appointment-status"]');
    const statusText = await appointmentStatus.textContent();
    expect(statusText).toContain('待确认');
    
    await consumerPage.screenshot({ path: `test-results/consumer-appointment-success-${Date.now()}.png` });
  });
  ...
  test('E2E-CON-09: 我的预约查看', async () => {
    ...
    // 4. 等待预约列表加载
    await TestHelpers.waitForElement(consumerPage, '[data-testid="appointments-list"]');
    ...
    await consumerPage.screenshot({ path: `test-results/consumer-my-appointments-${Date.now()}.png` });
  });
});