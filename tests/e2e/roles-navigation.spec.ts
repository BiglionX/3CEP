...
test.describe('G1. 角色导航权限测试 (ROLES-NAVIGATION)', () => {
  ...
  test('G1-01: 管理员访问管理页面', async () => {
    ...
    await TestHelpers.waitForElement(
      page,
      '[data-testid="permissions-management"]'
    );

    await page.screenshot({
      path: `test-results/g1-admin-access-success-${Date.now()}.png`,
    });
  });
  ...
  test('G1-02: 工程师访问管理页面被拒绝', async () => {
    ...
    if (response?.status() === 403) {
      ...
      const forbiddenMessage =
        (await page.$('[data-testid="access-forbidden"]')) ||
    ...
    await page.screenshot({
      path: `test-results/g1-engineer-access-denied-${Date.now()}.png`,
    });
  });
  ...
  test('G1-03: 消费者访问专业功能被拒绝', async () => {
    ...
    // 2. 尝试访问工程师专属页面
    const engResponse = await page.goto(
      `${TEST_CONFIG.BASE_URL}/engineer/tools`
    );
    ...
    if (adminResponse?.status() === 403) {
      ...
      const redirectElements =
        (await page.$$('text=权限不足')) || (await page.$$('text=访问被拒绝'));
      expect(redirectElements.length).toBeGreaterThan(0);
    }

    await page.screenshot({
      path: `test-results/g1-consumer-access-restricted-${Date.now()}.png`,
    });
  });
  ...
  test('G1-04: 店铺主访问权限范围验证', async () => {
    ...
    await page.screenshot({
      path: `test-results/g1-shopowner-permission-scope-${Date.now()}.png`,
    });
  });
  ...
  test('G1-05: 未授权用户访问受保护路由', async () => {
    ...
    const apiResponse = await page.request.get(
      `${TEST_CONFIG.BASE_URL}/api/admin/users`
    );
    expect(apiResponse.status()).toBe(401); // 未认证

    await page.screenshot({
      path: `test-results/g1-unauthorized-access-${Date.now()}.png`,
    });
  });
  ...
  test('G1-06: 角色菜单权限显示', async () => {
    ...
    // 验证工程师看到专业工具菜单
    const engineerToolsMenu = await page.$(
      '[data-testid="engineer-tools-menu"]'
    );
    ...
    await page.screenshot({
      path: `test-results/g1-role-menu-display-${Date.now()}.png`,
    });
  });
});
