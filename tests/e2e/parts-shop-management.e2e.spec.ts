...
test.describe('E2E-PARTS-SHOP: 配件管理与店铺管理端到端测试', () => {
  ...
  test('E2E-PARTS-01: 配件智能搜索与价格比较', async ({ page }) => {
    ...
    executor.addStep('配件详情查看', async () => {
      ...
      await helpers.clickAndWait(
        `[data-testid="view-details-${testPart.sku}"]`
      );
  ...
  test('E2E-SHOP-01: 店铺入驻申请到审核通过', async ({ page }) => {
    ...
    executor.addStep('管理员审核', async () => {
      ...
      const applicationItem = page.locator(
        `[data-testid="application-${newShopOwner.shopData.name}"]`
      );
      await expect(applicationItem).toBeVisible();

      await helpers.clickAndWait(
        `[data-testid="review-${newShopOwner.shopData.name}"]`
      );
  ...
  test('E2E-PARTS-SHOP-01: 配件采购与供应链协同', async ({ page }) => {
    ...
    // 模拟库存不足触发预警
    const lowStockItem = page
      .locator('[data-testid^="inventory-item-"]')
      .filter({ hasText: /紧张|短缺/ })
      .first();
