...
test.describe('外贸公司管理平台完整测试', () => {
  ...
  test('统计卡片跳转功能测试', async ({ page }) => {
    ...
    for (let i = 0; i < Math.min(cardCount, 2); i++) {
      ...
      console.log(
        `📊 统计卡片 ${i + 1} "${cardText?.substring(0, 20)}..." 跳转测试: ${isForeignTradePage ? '成功' : '未跳转'}`
      );
  ...
  test('订单详情跳转测试', async ({ page }) => {
    ...
    if (orderCount > 0) {
      ...
      console.log(
        `📦 订单详情跳转测试: "${orderText?.substring(0, 30)}..." -> ${isInOrderDetail ? '成功' : '失败'}`
      );
  ...
  test('创建新订单功能测试', async ({ page }) => {
    ...
    if (await createButton.isVisible()) {
      ...
      // 检查是否打开了模态框或跳转到创建页面
      const modalOrNewPage = await page
        .locator('[role="dialog"], form')
        .first()
        .isVisible();
      console.log(
        `➕ 创建订单功能测试: ${modalOrNewPage ? '成功触发' : '未响应'}`
      );
