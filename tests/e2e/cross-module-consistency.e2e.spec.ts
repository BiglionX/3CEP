...
test.describe('E2E-CROSS: 跨模块数据一致性验证', () => {
  ...
  test('E2E-CROSS-01: 用户数据跨模块一致性验证', async ({ page }) => {
    ...
    if (result.success) {
      ...
      DependencyChecker.markDependencyFailed('E2E-CROSS-01', result.failedSteps.join(', '));
  ...
  test('E2E-CROSS-02: 设备数据流转一致性验证', async ({ page }) => {
    ...
    executor.addStep('技师端数据验证', async () => {
      ...
      // 查找对应设备的工单
      const workOrder = page.locator(`[data-testid="work-order-${testDevice.id}"]`);
    ...
    if (result.success) {
      ...
      DependencyChecker.markDependencyFailed('E2E-CROSS-02', result.failedSteps.join(', '));
  ...
  test('E2E-CROSS-03: 配件库存数据一致性验证', async ({ page }) => {
    ...
    executor.addStep('配件搜索数据验证', async () => {
      ...
      const searchResult = page.locator(`[data-testid="part-result-${testPart.sku}"]`);
    ...
    executor.addStep('采购模块数据验证', async () => {
      ...
      // 验证采购搜索结果
      const poSearchResult = page.locator(`[data-testid="po-part-${testPart.sku}"]`);
    ...
    executor.addStep('维修模块配件调用验证', async () => {
      ...
      const repairPart = page.locator(`[data-testid="repair-part-${testPart.sku}"]`);
    ...
    if (result.success) {
      ...
      DependencyChecker.markDependencyFailed('E2E-CROSS-03', result.failedSteps.join(', '));
  ...
  test('E2E-CROSS-04: 订单状态跨模块同步验证', async ({ page }) => {
    ...
    const allSuccess = userResult.success && engineerResult.success && 
                      adminResult.success && finalUserResult.success;

    if (allSuccess) {
      DependencyChecker.markDependencyCompleted('E2E-CROSS-04');
      await helpers.takeScreenshot('order-status-sync-success');
    } else {
      DependencyChecker.markDependencyFailed('E2E-CROSS-04', '订单状态同步失败');
  ...
  test('E2E-CROSS-05: 财务数据一致性验证', async ({ page }) => {
    ...
    executor.addStep('财务报表数据验证', async () => {
      ...
      // 验证交易记录
      const transactionRecord = page.locator(`[data-testid="transaction-${testAmount}"]`);
    ...
    if (result.success) {
      ...
      DependencyChecker.markDependencyFailed('E2E-CROSS-05', result.failedSteps.join(', '));
