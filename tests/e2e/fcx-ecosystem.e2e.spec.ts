...
test.describe('E2E-FCX: FCX生态系统集成端到端测试', () => {
  ...
  test('E2E-FCX-01: FCX通证奖励与消费流程', async ({ page }) => {
    ...
    executor.addStep('维修质量奖励', async () => {
      ...
      await helpers.clickAndWait(
        '[data-testid="complete-high-quality-repair"]'
      );
      ...
      await helpers.waitForElement(
        '[data-testid="quality-reward-notification"]'
      );
  ...
  test('E2E-FCX-05: FCX生态应用集成', async ({ page }) => {
    ...
    // 步骤3: 使用FCX兑换应用服务
    const premiumApp = page
      .locator('[data-testid="app-card"]')
      .filter({ hasText: '高级维修诊断' })
      .first();
