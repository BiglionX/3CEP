...
test.describe('工程师核心流程测试 (E2E-ENG-*)', () => {
  ...
  test('E2E-ENG-01: 工程师登录与信息流查看', async () => {
    ...
    await engineerPage.screenshot({
      path: `test-results/engineer-login-feed-${Date.now()}.png`,
    });
  });
  ...
  test('E2E-ENG-02: 热点链接点赞功能', async () => {
    ...
    // 2. 找到第一个可点赞的热点链接
    const firstLikeButton = await TestHelpers.waitForElement(
      engineerPage,
      '[data-testid="like-button"]:not([disabled])'
    );
    ...
    // 5. 验证点赞数增加
    const updatedLikeCountElement = await engineerPage.$(
    ...
    // 6. 验证按钮状态变化
    const updatedLikeButton = await engineerPage.$(
    ...
    await engineerPage.screenshot({
      path: `test-results/engineer-like-success-${Date.now()}.png`,
    });
  });
  ...
  test('E2E-ENG-03: 取消点赞功能', async () => {
    ...
    // 2. 找到已点赞的内容
    const unlikeButton = await TestHelpers.waitForElement(
      engineerPage,
    ...
    await engineerPage.screenshot({
      path: `test-results/engineer-unlike-success-${Date.now()}.png`,
    });
  });
  ...
  test('E2E-ENG-04: 24小时内重复点赞限制', async () => {
    ...
    // 2. 找到一个已点赞的内容
    const likedButton = await TestHelpers.waitForElement(
      engineerPage,
    ...
    // 6. 验证错误提示显示
    const errorMessage = await engineerPage.$(
      '[data-testid="like-limit-error"]'
    );
    expect(errorMessage).toBeTruthy();

    await engineerPage.screenshot({
      path: `test-results/engineer-like-limit-${Date.now()}.png`,
    });
  });
  ...
  test('E2E-ENG-05: 热点沉淀机制测试', async () => {
    ...
    const targetLink = await TestHelpers.waitForElement(
      engineerPage,
      '[data-testid="hot-link-with-2-likes"]'
    );
    ...
    // 4. 验证草稿创建提示
    const draftSuccessToast = await engineerPage.$(
      '[data-testid="draft-success-toast"]'
    );
    expect(draftSuccessToast).toBeTruthy();

    // 5. 验证草稿指示器显示
    const draftIndicator = await engineerPage.$(
    ...
    await engineerPage.screenshot({
      path: `test-results/engineer-draft-trigger-${Date.now()}.png`,
    });
  });
  ...
  test('E2E-ENG-06: 全局搜索功能', async () => {
    ...
    const uniqueTypes = new Set(
      await Promise.all(resultTypes.map((el: any) => el.textContent()))
    );
    ...
    await engineerPage.screenshot({
      path: `test-results/engineer-search-results-${Date.now()}.png`,
    });
  });
});
