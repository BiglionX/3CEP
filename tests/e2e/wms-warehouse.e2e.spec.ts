...
test.describe('E2E-WMS: WMS仓储管理端到端测试', () => {
  ...
  test('E2E-WMS-01: 仓储管理系统集成测试', async ({ page }) => {
    ...
    executor.addStep('连接WMS系统', async () => {
      ...
      await helpers.verifyElementExists('[data-testid="connection-status-connected"]');
    });
    ...
    executor.addStep('库存查询与同步', async () => {
      ...
      // 查找特定配件的库存信息
      const partInventory = page.locator(`[data-testid="inventory-item-${testPart.sku}"]`);
    ...
    executor.addStep('订单自动分配', async () => {
      ...
      // 验证系统自动分配仓库
      await helpers.waitForElement('[data-testid="warehouse-allocation-result"]');
    ...
    executor.addStep('出库与物流跟踪', async () => {
      ...
      const pendingOrders = page.locator('[data-testid="outbound-order-pending"]');
    ...
    executor.addStep('库存预警与补货', async () => {
      ...
      if (alertCount > 0) {
        ...
        // 发起补货申请
        await helpers.clickAndWait('[data-testid="create-replenishment-button"]');
        await helpers.fillForm({
          '[data-testid="replenish-quantity"]': '20',
          '[data-testid="supplier-select"]': '苹果官方供应商'
        });
        
        await helpers.clickAndWait('[data-testid="submit-replenishment-button"]');
    ...
    if (result.success) {
      ...
      DependencyChecker.markDependencyFailed('E2E-WMS-01', result.failedSteps.join(', '));
  ...
  test('E2E-WMS-02: 多仓库协同管理', async ({ page }) => {
    ...
    // 验证仓库间连接线
    const connectionLines = page.locator('[data-testid="warehouse-connection"]');
    await expect(connectionLines).toHaveCount(nodeCount * (nodeCount - 1) / 2);
    ...
    const pendingTransfer = page.locator('[data-testid="transfer-request"]').first();
  ...
  test('E2E-WMS-03: 智能仓储优化', async ({ page }) => {
    ...
    for (const item of pickingItems) {
      ...
      await page.fill('[data-testid="picking-quantity"]', item.quantity.toString());
    ...
    for (const category of ['A', 'B', 'C']) {
      const categoryItems = page.locator(`[data-testid="abc-items-${category}"]`);
  ...
  test('E2E-WMS-04: 仓储数据分析与报表', async ({ page }) => {
    ...
    await helpers.verifyElementExists('[data-testid="anomaly-resolution-suggestions"]');
