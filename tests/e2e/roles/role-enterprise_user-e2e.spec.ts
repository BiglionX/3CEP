...
test.describe('企业用户权限测试 (ENTERPRISE_USER)', () => {
  ...
  test('enterprise_user-login-001: 企业用户登录测试', async () => {
    // 访问登录页面
    await page.goto('http://localhost:3003/enterprise/login?redirect=/enterprise/dashboard');
    
    // 输入登录凭据
    await page.fill('[data-testid="email-input"]', 'enterprise-user@fixcycle.com');
