...
test.describe('采购经理权限测试 (PROCUREMENT_MANAGER)', () => {
  ...
  test('procurement_manager-login-001: 采购经理登录测试', async () => {
    // 访问登录页面
    await page.goto('http://localhost:3003/enterprise/login?redirect=/enterprise/procurement/dashboard');
    
    // 输入登录凭据
    await page.fill('[data-testid="email-input"]', 'procurement-manager@fixcycle.com');
