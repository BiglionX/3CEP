...
test.describe('G2. 角色操作权限测试 (ROLES-ACTIONS)', () => {
  ...
  test('G2-01: 管理员工作流回放操作', async () => {
    ...
    // 3. 选择一个工作流进行回放
    const workflowRow = await TestHelpers.waitForElement(page, '[data-testid="workflow-row"]:first-child');
    ...
    for (const entry of auditEntries) {
      ...
      if (actionText?.includes('workflow_replay') && actionText?.includes('admin')) {
    ...
    await page.screenshot({ path: `test-results/g2-admin-replay-success-${Date.now()}.png` });
  });
  ...
  test('G2-02: 工程师工具执行权限', async () => {
    ...
    await page.selectOption('[data-testid="fault-type-select"]', 'screen_broken');
    await page.click('[data-testid="run-diagnostic"]');

    // 4. 验证工具执行成功
    await TestHelpers.waitForElement(page, '[data-testid="diagnostic-results"]');
    ...
    await page.screenshot({ path: `test-results/g2-engineer-tool-execution-${Date.now()}.png` });
  });
  ...
  test('G2-03: 消费者受限操作验证', async () => {
    ...
    const toolResponse = await page.goto(TEST_CONFIG.BASE_URL + '/tools/advanced');
    if (toolResponse?.status() === 403) {
      expect(toolResponse.status()).toBe(403);
    } else {
      await page.waitForTimeout(2000);
      const toolErrorMsg = await page.$('text=该工具需要专业资格') ||
                          await page.$('text=请联系专业技术人员');
      expect(toolErrorMsg).toBeTruthy();
    }

    await page.screenshot({ path: `test-results/g2-consumer-restricted-actions-${Date.now()}.png` });
  });
  ...
  test('G2-04: 店铺主设置修改权限', async () => {
    ...
    await TestHelpers.waitForElement(page, '[data-testid="shop-settings-form"]');
    ...
    // 4. 验证保存成功
    await TestHelpers.waitForElement(page, '[data-testid="settings-save-success"]');
    ...
    if (sysSettingsResponse?.status() === 403) {
      ...
      await page.waitForTimeout(2000);
      const sysErrorMsg = await page.$('text=系统设置仅限管理员访问') ||
                         await page.$('text=需要超级管理员权限');
      expect(sysErrorMsg).toBeTruthy();
    }

    await page.screenshot({ path: `test-results/g2-shopowner-settings-modify-${Date.now()}.png` });
  });
  ...
  test('G2-05: 操作审计日志完整性', async () => {
    ...
    await page.screenshot({ path: `test-results/g2-audit-log-integrity-${Date.now()}.png` });
  });
  ...
  test('G2-06: 多角色权限继承验证', async () => {
    ...
    const engineerTools = await page.$('[data-testid="engineer-tools-section"]');
    expect(engineerTools).toBeTruthy();

    // 3. 验证可以看到店铺管理
    await page.goto(TEST_CONFIG.BASE_URL + '/shop');
    const shopManagement = await page.$('[data-testid="shop-management-section"]');
    ...
    await page.screenshot({ path: `test-results/g2-multirole-permissions-${Date.now()}.png` });
  });
});