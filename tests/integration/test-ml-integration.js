...
async function runIntegrationTests() {
  ...
  try {
    ...
    console.log(
      `   ${healthStatus.status === 'healthy' ? '✅' : '❌'} ML服务状态: ${healthStatus.status}`
    );
    ...
    console.log(
      `   ${predictionResult.success ? '✅' : '❌'} 预测请求: ${predictionResult.success ? '成功' : '失败'}`
    );

    if (predictionResult.success) {
      console.log(
        `      预测价格: ¥${predictionResult.data.predictedPrice.toFixed(2)}`
      );
      console.log(
        `      置信度: ${(predictionResult.data.confidence * 100).toFixed(1)}%`
      );
    ...
    console.log(
      `   ${batchResult.success ? '✅' : '❌'} 批量预测: ${batchResult.success ? '成功' : '失败'}`
    );

    if (batchResult.success) {
      console.log(`      处理设备数: ${batchResult.data.count}`);
      console.log(
        `      总处理时间: ${batchResult.data.totalProcessingTime.toFixed(3)}秒`
      );
      console.log(
        `      平均每个设备: ${((batchResult.data.totalProcessingTime / batchResult.data.count) * 1000).toFixed(1)}毫秒`
      );
    }
    ...
    if (predictionResult.success) {
      ...
      console.log(`   ✅ 置信度评估完成`);
      console.log(
        `      综合置信度: ${(confidenceResult.confidence * 100).toFixed(1)}%`
      );
      console.log(`      置信级别: ${confidenceResult.confidenceLevel}`);
      console.log(
        `      建议数量: ${confidenceResult.recommendations.length}条`
      );

      // 显示详细因子
      console.log('      置信因子详情:');
      Object.entries(confidenceResult.confidenceFactors).forEach(
        ([factor, value]) => {
    ...
    console.log(
      `   ${modelInfo.success ? '✅' : '❌'} 模型信息获取: ${modelInfo.success ? '成功' : '失败'}`
    );
    ...
    if (modelInfo.success) {
      ...
      console.log(
        `      加载时间: ${new Date(modelInfo.data.loaded_at).toLocaleString()}`
      );
    }
    ...
    console.log(
      `   ${!errorResult.success ? '✅' : '❌'} 无效数据处理: ${!errorResult.success ? '正确拦截' : '未拦截'}`
    );
    ...
    console.log(
      `   ${failoverResult.status === 'unhealthy' ? '✅' : '❌'} 故障转移: ${failoverResult.status}`
    );
