...
async function testDIYTutorialModule() {
  ...
  try {
    ...
    const publicResponse = await fetch(
      `${baseUrl}/api/tutorials?page=1&pageSize=5`
    );
    const publicData = await publicResponse.json();
    recordTest(
      '公共API访问',
      publicResponse.ok,
      `状态码: ${publicResponse.status}`
    );

    if (publicResponse.ok) {
      recordTest(
        '返回教程数据',
        Array.isArray(publicData.tutorials),
        `教程数量: ${publicData.tutorials?.length || 0}`
      );
      recordTest('分页信息完整', publicData.pagination !== undefined, '');
      recordTest(
        '教程数据结构正确',
        publicData.tutorials &&
          publicData.tutorials.length > 0 &&
          publicData.tutorials[0].title,
    ...
    const searchResponse = await fetch(
      `${baseUrl}/api/tutorials?search=iPhone`
    );
    const searchData = await searchResponse.json();
    recordTest(
      '搜索功能',
      searchResponse.ok,
      `搜索"iPhone"结果: ${searchData.tutorials?.length || 0}条`
    );

    // 3. 测试设备型号筛选
    console.log('\n=== 测试3: 设备型号筛选 ===');
    const deviceResponse = await fetch(
      `${baseUrl}/api/tutorials?deviceModel=iPhone 14 Pro`
    );
    const deviceData = await deviceResponse.json();
    recordTest(
      '设备筛选',
      deviceResponse.ok,
      `iPhone 14 Pro教程: ${deviceData.tutorials?.length || 0}条`
    );

    // 4. 测试故障类型筛选
    console.log('\n=== 测试4: 故障类型筛选 ===');
    const faultResponse = await fetch(
      `${baseUrl}/api/tutorials?faultType=screen_broken`
    );
    const faultData = await faultResponse.json();
    recordTest(
      '故障筛选',
      faultResponse.ok,
      `屏幕损坏教程: ${faultData.tutorials?.length || 0}条`
    );
    ...
    if (publicData.tutorials && publicData.tutorials.length > 0) {
      ...
      const detailResponse = await fetch(
        `${baseUrl}/api/tutorials/${firstTutorialId}`
      );
      const detailData = await detailResponse.json();
      recordTest(
        '教程详情访问',
        detailResponse.ok,
        `教程ID: ${firstTutorialId}`
      );

      if (detailResponse.ok) {
        recordTest(
          '详情数据完整',
          detailData.tutorial !== undefined,
          `标题: ${detailData.tutorial?.title}`
        );
        recordTest(
          '浏览次数更新',
          detailData.tutorial?.view_count > 0,
          `浏览数: ${detailData.tutorial?.view_count}`
        );
      }
    } else {
    ...
    try {
      ...
      recordTest(
        '教程列表页面',
        tutorialsPageResponse.status === 200,
        `状态码: ${tutorialsPageResponse.status}`
      );
    } catch (error) {
    ...
    if (publicData.tutorials && publicData.tutorials.length > 0) {
      ...
      recordTest(
        ...
        sampleTutorial.title &&
          sampleTutorial.device_model &&
          sampleTutorial.fault_type,
      ...
      recordTest(
        '工具和配件数组',
        Array.isArray(sampleTutorial.tools) &&
          Array.isArray(sampleTutorial.parts),
    ...
    const errorResponse = await fetch(
      `${baseUrl}/api/tutorials/nonexistent-id`
    );
    recordTest(
      '404错误处理',
    ...
    console.log(
      `   通过率: ${Math.round((testResults.passed / testResults.total) * 100)}%`
    );
    ...
    console.log('\n📋 验收标准检查:');
    console.log(
      `✅ 能够成功添加新的维修教程: ${(await canCreateTutorial()) ? '通过' : '待验证'}`
    );
    console.log(
      `✅ 支持编辑现有教程的所有字段: ${(await canEditTutorial()) ? '通过' : '待验证'}`
    );
    console.log(
      `✅ 可以删除不需要的教程: ${(await canDeleteTutorial()) ? '通过' : '待验证'}`
    );
    console.log(
      `✅ 通过API能够正确获取教程数据: ${testResults.passed > 5 ? '通过' : '部分通过'}`
    );
    console.log(
      `✅ 管理后台界面操作流畅: ${(await canAccessAdminInterface()) ? '通过' : '待验证'}`
    );
    console.log(
      `✅ 数据验证和错误处理完善: ${testResults.failed === 0 ? '通过' : '部分通过'}`
    );
  } catch (error) {
