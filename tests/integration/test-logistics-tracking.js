...
} = require('../dist/supply-chain/services/logistics-tracking.service');
const {
  LogisticsTrackingConfig,
  LogisticsCarrier,
} = require('../dist/supply-chain/models/logistics.model');
...
async function runTest() {
  ...
  const recognitionTests = [
    {
      number: '123456789012',
      expected: LogisticsCarrier.SF_EXPRESS,
      desc: '12位数字',
    },
    {
      number: 'YT123456789012',
      expected: LogisticsCarrier.YTO,
      desc: 'YT开头',
    },
    {
      number: 'ZT123456789012',
      expected: LogisticsCarrier.ZTO,
      desc: 'ZT开头',
    },
    {
      number: 'ST123456789012',
      expected: LogisticsCarrier.STO,
      desc: 'ST开头',
    },
  ];
  ...
  for (const test of recognitionTests) {
    ...
    if (result.carrier === test.expected) {
      ...
      console.log(
        `   ❌ ${test.desc}: ${test.number} -> 期望${test.expected}, 实际${result.carrier}`
      );
    }
  }
  ...
  if (recognitionPassed === recognitionTests.length) {
    console.log(
      `   ✅ 识别功能测试通过 (${recognitionPassed}/${recognitionTests.length})`
    );
    passedTests++;
  } else {
    console.log(
      `   ⚠️  识别功能部分通过 (${recognitionPassed}/${recognitionTests.length})`
    );
  }
  ...
  const reportPath = path.join(
    ...
    '..',
    'logs',
    'logistics-tracking-test-report.json'
  );
