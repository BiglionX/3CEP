...
async function runSingleTest(testCase) {
  ...
  try {
    ...
    console.log(
      `   ✓ 返回 ${diagnosisResult.recommendedParts.length} 个推荐配件`
    );
    console.log(
      `   ✓ 关键词匹配: ${matchedKeywords.length}/${testCase.expectedKeywords.length}`
    );
...
async function testSessionManagement() {
  ...
  try {
    ...
    const infoResponse = await fetch(
      `${BASE_URL}/api/diagnosis/analyze?sessionId=${sessionId}`
    );
...
async function runAllTests() {
  try {
    ...
    console.log(`📊 性能测试结果:`);
    console.log(
      `   - 并发处理能力: ${perfResults.successfulRequests}/${perfResults.concurrentRequests}`
    );
