        ...
        if (result.data) {
          if (result.data.success !== undefined) {
            ...
            if (result.data.tracking) {
              console.log(`   📦 运单号: ${result.data.tracking.trackingNumber}`);
              console.log(`   🚛 物流商: ${result.data.tracking.carrierName}`);
              console.log(`   📍 状态: ${result.data.tracking.status}`);
              console.log(`   📅 轨迹节点: ${result.data.tracking.timeline?.length || 0}个`);
            }
            
            if (result.data.summary) {
              console.log(`   📊 批量统计: ${result.data.summary.successCount}/${result.data.summary.totalCount} 成功`);
            }
          }
        }
