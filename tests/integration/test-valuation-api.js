...
async function runApiTests() {
  ...
  const healthChecks = [
    ...
    },
    {
      ...
      check: async () => {
        try {
          const response = await fetch(`${BASE_URL}/api/lifecycle/profile`, {
            method: 'OPTIONS',
          });
          return response.ok;
        } catch {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(
        `   📱 设备型号: ${result.data.deviceInfo?.productModel || 'N/A'}`
      );
  ...
  try {
    ...
    const response = await fetch(`${BASE_URL}/api/valuation/estimate`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.VALUATION_API_KEY || process.env.LIFECYCLE_API_KEY || 'dev-key'}`,
        'Content-Type': 'application/json',
    ...
    if (result.success) {
      ...
      console.log(
        `   📱 设备: ${result.data.deviceInfo?.productModel || 'N/A'}`
      );
      console.log(`   💰 最终估值: ¥${result.data.finalValue}`);
      console.log(`   📊 详细分解:`);
      console.log(
        `      - 原始价格: ¥${result.data.breakdown?.originalPrice || 'N/A'}`
      );
      console.log(
        `      - 折旧金额: ¥${result.data.breakdown?.depreciation?.toFixed(2) || 'N/A'}`
      );
      console.log(
        `      - 部件评分: ${(result.data.breakdown?.componentAdjustment * 100 || 0).toFixed(1)}%`
      );
      console.log(
        `      - 成色乘数: ${(result.data.breakdown?.conditionAdjustment * 100 || 0).toFixed(1)}%`
      );
    } else {
