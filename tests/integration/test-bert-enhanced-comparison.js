...
async function testEnhancedVersion(testCase) {
  try {
    const response = await fetch(
      'http://localhost:3001/api/b2b-procurement/parse-demand-enhanced',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
function compareResults(traditional, enhanced) {
  ...
  if (enhConf > tradConf) {
    console.log(
      `     📈 置信度提升: +${enhConf - tradConf}个百分点 (${tradConf}% → ${enhConf}%)`
    );
  } else if (enhConf < tradConf) {
    console.log(
      `     📉 置信度下降: ${enhConf - tradConf}个百分点 (${tradConf}% → ${enhConf}%)`
    );
  } else {
  ...
  if (enhTime < tradTime) {
    console.log(
      `     🚀 处理加速: 快${tradTime - enhTime}ms (${tradTime}ms → ${enhTime}ms)`
    );
  } else if (enhTime > tradTime) {
    console.log(
      `     🐢 处理变慢: 慢${enhTime - tradTime}ms (${tradTime}ms → ${enhTime}ms)`
    );
  } else {
  ...
  if (enhItems > tradItems) {
    console.log(
      `     📦 识别物品增加: +${enhItems - tradItems}个 (${tradItems} → ${enhItems})`
    );
  } else if (enhItems < tradItems) {
    console.log(
      `     📦 识别物品减少: ${enhItems - tradItems}个 (${tradItems} → ${enhItems})`
    );
  } else {
