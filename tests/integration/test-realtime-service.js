...
async function testRealTimeService() {
  ...
  try {
    ...
    const { extendedProcessors } = await import('../src/data-center/streaming/extended-processors');

    // 1. 测试服务初始化
    console.log('1️⃣ 测试服务初始化...');
    console.log('   ✅ 实时数据服务实例已创建');
    console.log(`   📊 已注册消费者组数: ${realTimeDataService.consumerGroups.size}`);
    ...
    console.log(`   📊 总处理器数: ${Array.from(realTimeDataService.processors.values()).flat().length}`);
    ...
    for (const groupConfig of testGroups) {
      try {
        ...
        if (error.message.includes('BUSYGROUP')) {
          ...
          console.log(`   ⚠️ 创建消费者组失败: ${groupConfig.groupName} - ${error.message}`);
        }
      }
    }
    ...
    for (const [eventType, stat] of stats.entries()) {
      if (stat.totalProcessed > 0) {
        console.log(`     ${eventType}: 处理${stat.totalProcessed}个, 成功${stat.successCount}个, 错误${stat.errorCount}个, 平均耗时${stat.averageProcessingTime}ms`);
      }
    }
    ...
    const orderStats = realTimeDataService.getEventTypeStats('order_status_change');
    if (orderStats) {
    ...
    // 7. 测试服务状态查询
    console.log('\n7️⃣ 测试服务状态...');
    console.log(`   🏃‍♂️ 服务运行状态: ${realTimeDataService.isRunning ? '运行中' : '已停止'}`);
    console.log(`   📋 消费者组数量: ${realTimeDataService.consumerGroups.size}`);
...
if (require.main === module) {
  testRealTimeService().then(success => {
