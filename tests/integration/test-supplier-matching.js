...
async function runIntegrationTest() {
  ...
  try {
    ...
    for (const [index, dataset] of TEST_DATASETS.entries()) {
      ...
      console.log(
        `📊 测试结果: ${result.passed ? '✅ 通过' : '❌ 失败'} (准确率: ${result.accuracy.toFixed(1)}%)\n`
      );
    }
...
function calculateAccuracy(matches, expectedSuppliers) {
  ...
  const correctMatches = matchedSuppliers.filter(supplier =>
    expectedSuppliers.some(
      expected => supplier.includes(expected) || expected.includes(supplier)
    )
  );
  ...
  results.forEach(result => {
    ...
    if (result.topMatches.length > 0) {
      ...
      result.topMatches.forEach((match, index) => {
        console.log(
          `    ${index + 1}. ${match.supplierName} (${match.matchScore.toFixed(1)}分)`
        );
        console.log(
          `       品类匹配: ${match.categoryMatchScore.toFixed(1)}%, 价格竞争力: ${match.priceCompetitiveness.toFixed(1)}%`
        );
      });
    }
