...
console.log(
  `${colors.blue}${colors.bold}📱 扫码落地页功能测试${colors.reset}\n`
);
...
const testCases = [
  ...
  },
  {
    ...
    test: () => {
      const chineseManuals = testData.manuals.filter(
        m => m.is_published && m.language_codes.includes('zh')
      );
      const englishManuals = testData.manuals.filter(
        m => m.is_published && m.language_codes.includes('en')
      );
  ...
  },
  {
    ...
    test: () => {
      ...
      return {
        ...
        averageDistance: (
          shops.reduce((sum, s) => sum + s.distance, 0) / shops.length
        ).toFixed(1),
        averageRating: (
...
testCases.forEach((testCase, index) => {
  console.log(
    `${colors.cyan}测试 ${index + 1}/${totalTests}: ${testCase.name}${colors.reset}`
  );
  ...
  try {
    ...
    if (typeof result === 'boolean') {
      ...
      if (testCase.name === '多语言说明书支持') {
        console.log(
          `${colors.gray}    中文说明书: ${result.chineseAvailable ? '✅' : '❌'}${colors.reset}`
        );
        console.log(
          `${colors.gray}    英文说明书: ${result.englishAvailable ? '✅' : '❌'}${colors.reset}`
        );
        console.log(
          `${colors.gray}    总说明书数: ${result.totalManuals}${colors.reset}`
        );
      } else if (testCase.name === '设备自动识别') {
        Object.entries(result).forEach(([device, detection]) => {
          console.log(
            `${colors.gray}    ${device}: ${detection.brand} ${detection.deviceType} (${detection.confidence}%置信度)${colors.reset}`
          );
        });
      } else if (testCase.name === '附近维修店推荐') {
        console.log(
          `${colors.gray}    总店铺数: ${result.totalShops}${colors.reset}`
        );
        console.log(
          `${colors.gray}    认证店铺: ${result.verifiedShops}${colors.reset}`
        );
        console.log(
          `${colors.gray}    平均距离: ${result.averageDistance}km${colors.reset}`
        );
        console.log(
          `${colors.gray}    平均评分: ${result.averageRating}${colors.reset}`
        );
        console.log(
          `${colors.gray}    服务信息: ${result.hasServices ? '✅' : '❌'}${colors.reset}`
        );
      } else if (testCase.name === '功能按钮状态控制') {
        Object.entries(result).forEach(([scenario, state]) => {
          console.log(`${colors.gray}    ${scenario}:${colors.reset}`);
          console.log(
            `${colors.gray}      说明书按钮: ${state.buttons.manuals ? '启用' : '禁用'}${colors.reset}`
          );
          console.log(
            `${colors.gray}      诊断按钮: ${state.buttons.diagnosis ? '启用' : '禁用'}${colors.reset}`
          );
        });
      }
    }
