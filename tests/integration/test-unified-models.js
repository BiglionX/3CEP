...
function testDataModels() {
  ...
  console.log('\n🏗️ 验证模型结构:');
  try {
    const modelContent = fs.readFileSync('src/data-center/models/unified-models.ts', 'utf8');
  ...
  console.log('\n🔄 验证数据映射服务:');
  try {
    const mapperContent = fs.readFileSync('src/data-center/core/data-mapper.ts', 'utf8');
    ...
    requiredClasses.forEach(className => {
      const exists = mapperContent.includes(`class ${className}`) || 
  ...
  console.log('  输入数据:');
  console.log(`    Lionfix设备: ${sampleLionfixDevice.brand} ${sampleLionfixDevice.model}`);
  console.log(`    本地设备: 库存 ${sampleFixcycleDevice.stock_quantity}`);

  // 5. 检查配置文件
  console.log('\n⚙️ 检查配置:');
  const configExists = fs.existsSync('src/data-center/core/data-center-service.ts');
