...
async function testCrowdfundingModule() {
  ...
  try {
    ...
    Object.entries(serviceMethods).forEach(([file, methods]) => {
      console.log(`\n📄 检查 ${file}:`)
      const filePath = path.join(__dirname, '../src/services/crowdfunding', file)
    ...
    apiRoutes.forEach(route => {
      const routePath = path.join(__dirname, '../src/app', route.replace('[id]', '\[id\]'), 'route.ts')
    ...
    pages.forEach(page => {
      const pagePath = path.join(__dirname, '../src/app', page.path.replace('[id]', '\[id\]'), 'page.tsx')
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/019_create_crowdfunding_system.sql')
    if (fs.existsSync(migrationPath)) {
      ...
      dataModels.forEach(model => {
        if (migrationContent.includes(`CREATE TABLE ${model.file}`)) {
          ...
          console.log(`❌ 数据模型: ${model.name} (${model.file}) 未在迁移文件中找到`)
        }
      })
    } else {
