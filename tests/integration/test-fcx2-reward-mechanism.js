...
async function testFcx2RewardMechanism() {
  ...
  try {
    ...
    for (const testCase of testCases) {
      try {
        const result = await rewardService.calculateOrderReward(
          mockOrder,
          testCase.rating
        );
        const isSuccess = result.finalAmount >= testCase.expectedMin;
        console.log(
          `  评分${testCase.rating}: ${isSuccess ? '✅' : '❌'} ${result.finalAmount.toFixed(2)} FCX`
        );
      } catch (error) {
    ...
    try {
      ...
      const option = await optionService.grantOption(
        testShopId,
        optionAmount,
        'test-order-001'
      );
    ...
    try {
      ...
      if (calculateResponse.ok) {
        ...
        console.log(
          `  ✅ 奖励计算API: ${(result.data?.finalAmount || 0).toFixed(2)} FCX`
        );
      } else {
    ...
    try {
      const balanceResponse = await fetch(
        '/api/fcx/rewards?shopId=test-shop-001&action=balance'
      );

      if (balanceResponse.ok) {
        const result = await balanceResponse.json();
        console.log(
          `  ✅ 余额查询API: ${(result.data?.balance || 0).toFixed(2)} FCX`
        );
      } else {
