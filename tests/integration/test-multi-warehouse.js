...
async function runTest() {
  ...
  try {
    ...
    const domesticTest = await fetch('http://localhost:3001/api/supply-chain/warehouses', {
      method: 'POST',
      headers: {
    ...
    const overseasTest = await fetch('http://localhost:3001/api/supply-chain/warehouses', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    // 4. 测试仓库列表查询
    console.log('\n4️⃣ 测试仓库列表查询...');
    const listTest = await fetch('http://localhost:3001/api/supply-chain/warehouses?limit=10');
    ...
    if (domesticResult.success && domesticResult.data?.warehouseId) {
      ...
      const syncTest = await fetch('http://localhost:3001/api/supply-chain/warehouses/sync', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    console.log('\n6️⃣ 测试跨仓调拨...');
    if (domesticResult.success && domesticResult.data?.warehouseId && 
        overseasResult.success && overseasResult.data?.warehouseId) {
      ...
      const transferTest = await fetch('http://localhost:3001/api/supply-chain/warehouses/transfer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    console.log('\n7️⃣ 测试容量规划...');
    if (domesticResult.success && domesticResult.data?.warehouseId) {
      const capacityTest = await fetch(`http://localhost:3001/api/supply-chain/warehouses/${domesticResult.data.warehouseId}/capacity`);
    ...
    console.log('\n8️⃣ 测试绩效报告...');
    if (domesticResult.success && domesticResult.data?.warehouseId) {
      const performanceTest = await fetch(`http://localhost:3001/api/supply-chain/warehouses/${domesticResult.data.warehouseId}/performance`);
    ...
    // 测试缺少必要参数
    const invalidTest = await fetch('http://localhost:3001/api/supply-chain/warehouses', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    console.log('✅ 参数验证结果:', invalidResult.error === '缺少必要参数: location.country');
