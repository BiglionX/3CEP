...
class FcxExchangeIntegrationTest {
  ...
  async setupTestData() {
    ...
    try {
      ...
      if (shopError)
        throw new Error(`创建测试维修店失败: ${shopError.message}`);
      ...
      if (warehouseError)
        throw new Error(`获取测试仓库失败: ${warehouseError.message}`);
      ...
      if (partsError)
        throw new Error(`获取测试配件失败: ${partsError.message}`);
  ...
  async testUserBalanceQuery() {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/fcx/accounts/${this.testUserId}/balance`
      );
  ...
  async testPartsListing() {
    ...
    try {
      const response = await fetch(
        'http://localhost:3001/api/parts/available?includeFcxPrice=true&limit=10'
      );
      const result = await response.json();
  ...
  async testFcxPriceQuery() {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/fcx/prices?${queryParams}`
      );
      const result = await response.json();
  ...
  async testInventoryReservation() {
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:3001/api/inventory/reserve',
        {
          method: 'POST',
  ...
  async testOrderQuery() {
    ...
    try {
      // 查询用户订单
      const response = await fetch(
        `http://localhost:3001/api/fcx/exchange?userId=${this.testUserId}&limit=5`
      );
      const result = await response.json();
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        const testOrder = result.data.find(
          order => order.order_number === this.testOrderNumber
        );
        if (testOrder) {
