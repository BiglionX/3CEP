...
async function runTest() {
  ...
  try {
    ...
    const agentResponse = await fetch(
      'http://localhost:3001/api/b2b-procurement/smart-agent',
      {
        method: 'POST',
        headers: {
    ...
    if (agentResult.fcxEstimate) {
      ...
      console.log(
        '   可以承担:',
        agentResult.fcxEstimate.canAfford ? '是' : '否'
      );
    }
    ...
    if (agentResult.quotationPlan) {
      ...
      if (executeResult.success) {
        console.log(
          '   成功执行请求数:',
          executeResult.executedRequests.length
        );
    ...
    for (const scenario of testScenarios) {
      ...
      let totalCost =
        (baseCost + supplierCost + itemCountCost + urgentCost) * batchDiscount;
