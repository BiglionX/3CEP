...
async function testAffiliateLinks() {
  ...
  try {
    ...
    const dbTestResponse = await fetch(
      `${baseUrl}/api/admin/test-db-connection`
    );
    ...
    const linksResponse = await fetch(
      `${baseUrl}/api/affiliate/links?partName=iPhone电池&limit=3`
    );
    ...
    if (generateResponse.ok && generateData.success) {
      ...
      console.log(
        `   追踪参数: ${Object.keys(generateData.data.trackingParams).length} 个`
      );
    } else {
    ...
    const analyticsResponse = await fetch(
      `${baseUrl}/api/affiliate/analytics/clicks?groupBy=day&limit=5`
    );
    ...
    if (analyticsResponse.ok) {
      ...
      console.log(
        `   唯一访客: ${analyticsData.data?.stats?.totalUniqueVisitors || 0}`
      );
    } else {
    ...
    const tutorialResponse = await fetch(
      `${baseUrl}/api/tutorials?page=1&pageSize=1`
    );
    ...
    if (tutorialResponse.ok && tutorialData.tutorials?.length > 0) {
      ...
      // 检查教程页面是否可访问
      const tutorialPageResponse = await fetch(
        `${baseUrl}/tutorial/${sampleTutorial.id}`
      );
