...
async function runTest() {
  ...
  try {
    ...
    // 2. 测试Stripe支付方式
    console.log('\n2️⃣ 测试Stripe支付方式...');
    const stripeTest = await fetch('http://localhost:3001/api/fcx/purchase/enhanced', {
      method: 'POST',
      headers: {
    ...
    // 3. 测试PayPal支付方式
    console.log('\n3️⃣ 测试PayPal支付方式...');
    const paypalTest = await fetch('http://localhost:3001/api/fcx/purchase/enhanced', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    // 4. 测试支付宝支付方式
    console.log('\n4️⃣ 测试支付宝支付方式...');
    const alipayTest = await fetch('http://localhost:3001/api/fcx/purchase/enhanced', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    console.log('\n5️⃣ 测试账户详情查询...');
    // 首先创建一个账户用于测试
    const accountCreation = await fetch('http://localhost:3001/api/fcx/accounts', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (accountResult.success) {
      ...
      const detailsTest = await fetch(`http://localhost:3001/api/fcx/accounts/${accountId}/details?userId=test-user-details`);
    ...
    // 6. 测试支付历史查询
    console.log('\n6️⃣ 测试支付历史查询...');
    const historyTest = await fetch('http://localhost:3001/api/fcx/purchase/enhanced?userId=test-user-stripe&limit=5');
    ...
    // 测试金额为0的情况
    const zeroAmountTest = await fetch('http://localhost:3001/api/fcx/purchase/enhanced', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    console.log('✅ 零金额验证:', zeroAmountResult.error === '购买金额必须大于0');

    // 测试超限金额
    const largeAmountTest = await fetch('http://localhost:3001/api/fcx/purchase/enhanced', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    console.log('✅ 超限金额验证:', largeAmountResult.error === '单笔购买金额不能超过10000美元');
