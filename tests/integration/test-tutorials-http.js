...
async function testTutorialsAPI() {
  ...
  try {
    ...
    if (publicResponse.ok) {
      ...
      if (publicData.tutorials && publicData.tutorials.length > 0) {
        ...
        publicData.tutorials.slice(0, 2).forEach((tutorial, index) => {
          console.log(
            `     ${index + 1}. ${tutorial.title} (${tutorial.device_model})`
          );
        });
      }
    } else {
    ...
    const adminResponse = await fetch(
      `${baseUrl}/api/admin/tutorials?status=draft`
    );
    ...
    // 3. 测试按设备型号筛选
    console.log('\n3️⃣ 测试按设备型号筛选');
    const deviceFilterResponse = await fetch(
    ...
    if (deviceFilterResponse.ok) {
      ...
      console.log(
        `   iPhone 14 Pro 教程数量: ${deviceFilterData.tutorials?.length || 0}`
      );
    } else {
    ...
    if (searchResponse.ok) {
      ...
      console.log(
        `   包含"屏幕"的教程数量: ${searchData.tutorials?.length || 0}`
      );
    } else {
    ...
    console.log(
      `   - 教程详情: ${
        typeof detailResponse !== 'undefined'
          ? detailResponse.ok
            ? '✅'
            : '❌'
          : '⚠️ 未测试'
