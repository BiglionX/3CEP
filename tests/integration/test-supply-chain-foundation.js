...
async function runTest() {
  ...
  try {
    ...
    // 2. 测试库存调整API
    console.log('\n2️⃣ 测试库存调整API...');
    const adjustInventoryResponse = await fetch('http://localhost:3001/api/supply-chain/inventory', {
      method: 'POST',
      headers: {
    ...
    // 3. 测试库存查询API
    console.log('\n3️⃣ 测试库存查询API...');
    const inventoryQueryResponse = await fetch('http://localhost:3001/api/supply-chain/inventory?productId=test-product-001&warehouseId=test-warehouse-001');
    const inventoryQueryResult = await inventoryQueryResponse.json();
    console.log('✅ 库存查询结果:', inventoryQueryResult);

    // 4. 测试供应商申请API
    console.log('\n4️⃣ 测试供应商申请API...');
    const supplierApplicationResponse = await fetch('http://localhost:3001/api/supply-chain/suppliers/application', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    // 5. 测试库存预警功能
    console.log('\n5️⃣ 测试库存预警功能...');
    const lowStockAlertsResponse = await fetch('http://localhost:3001/api/supply-chain/inventory?status=low_stock');
