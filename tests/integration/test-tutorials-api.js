...
async function testTutorialsAPI() {
  ...
  try {
    ...
    const publicResponse = await fetch(
      'http://localhost:3003/api/tutorials?page=1&pageSize=5'
    );
    ...
    const adminResponse = await fetch(
      'http://localhost:3003/api/admin/tutorials?status=draft'
    );
    ...
    if (!dbError) {
      ...
      dbData.forEach((tutorial, index) => {
        console.log(
          `   ${index + 1}. ${tutorial.title} (${tutorial.device_model} - ${tutorial.status})`
        );
      });
    } else {
