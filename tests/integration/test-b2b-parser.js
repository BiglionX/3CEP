...
async function runTest() {
  ...
  try {
    ...
    const response1 = await fetch('http://localhost:3001/api/b2b-procurement/parse-demand', {
      method: 'POST',
      headers: {
    ...
    const response2 = await fetch('http://localhost:3001/api/b2b-procurement/parse-demand', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (result2.success) {
      console.log('   识别的物品类别:', result2.data.parsedRequest.items[0]?.category);
      console.log('   预算范围:', result2.data.parsedRequest.budgetRange ? 
        `${result2.data.parsedRequest.budgetRange.min}-${result2.data.parsedRequest.budgetRange.max}元` : '未识别');
      console.log('   特殊要求:', result2.data.parsedRequest.specialRequirements?.join(', ') || '无');
    }
    ...
    const response3 = await fetch('http://localhost:3001/api/b2b-procurement/parse-demand', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (result3.success) {
      console.log('   识别的物品数量:', result3.data.parsedRequest.items.length);
      console.log('   处理耗时:', result3.data.parsedRequest.processingTimeMs + 'ms');
    }
    ...
    const response4 = await fetch('http://localhost:3001/api/b2b-procurement/parse-demand', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    console.log('✅ 错误处理测试:', !result4.success ? '正确拦截' : '未正确处理');
