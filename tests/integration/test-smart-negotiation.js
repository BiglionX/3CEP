...
async function testNegotiationEngine() {
  ...
  try {
    ...
    const strategiesResponse = await fetch(
      `${BASE_URL}/api/b2b-procurement/negotiation/start`
    );
    ...
    if (strategiesResult.data) {
      ...
      strategiesResult.data.forEach((strategy, index) => {
        console.log(
          `   ${index + 1}. ${strategy.name} (${strategy.strategyType})`
        );
      });
    }
    ...
    console.log(
      '   预期折扣:',
      `${startResult.data.advice?.expectedDiscount.toFixed(2)}%`
    );
    ...
    for (let i = 0; i < rounds.length; i++) {
      ...
      const roundResponse = await fetch(
        `${BASE_URL}/api/b2b-procurement/negotiation/${sessionId}/round`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (roundResult.success) {
        ...
        const discount =
          ((currentPrice - roundResult.data.nextOffer) / currentPrice) * 100;
    ...
    const statusResponse = await fetch(
      `${BASE_URL}/api/b2b-procurement/negotiation/${sessionId}`
    );
    ...
    const acceptResponse = await fetch(
      `${BASE_URL}/api/b2b-procurement/negotiation/${sessionId}/accept`,
      {
        method: 'POST',
      }
    );
    ...
    if (acceptResult.success) {
      ...
      console.log(
        '   最终折扣:',
        `${acceptResult.data.discountRate.toFixed(2)}%`
      );
    ...
    const recommendationResponse = await fetch(
      `${BASE_URL}/api/b2b-procurement/negotiation/recommendations?targetPrice=10000&limit=3`
    );
    const recommendationResult = await recommendationResponse.json();
    console.log(
      '✅ 供应商推荐:',
      recommendationResult.success ? '成功' : '失败'
    );
    ...
    if (recommendationResult.success && recommendationResult.data) {
      ...
      recommendationResult.data.forEach((supplier, index) => {
        console.log(
          `   ${index + 1}. ${supplier.supplierName} - ${supplier.score}分`
        );
