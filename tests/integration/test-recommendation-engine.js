...
async function runIntegrationTest() {
  ...
  try {
    ...
    const serverCheck = await execCommand(
      'curl -s http://localhost:3001/api/health || echo "server_down"'
    );
    ...
    const healthCheck = await execCommand(
      'curl -s "http://localhost:3001/api/fcx/recommendations?action=health-check"'
    );
    ...
    console.log(
      '   错误处理:',
      !invalidResult.success ? '✅ 正确拦截' : '❌ 未正确处理'
    );

    // 7. 清理测试数据
    console.log('\n7️⃣ 清理测试数据...');
    const cleanupResponse = await execCommand(
      'curl -s -X DELETE "http://localhost:3001/api/fcx/recommendations?days=1"'
    );
    const cleanupResult = JSON.parse(cleanupResponse);
    console.log(
      '   数据清理:',
      cleanupResult.success ? '✅ 完成' : '⚠️ 部分完成'
    );
