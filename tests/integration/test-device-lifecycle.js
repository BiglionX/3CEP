...
test.describe('设备生命周期功能测试', () => {
  ...
  test('设备档案创建和查询', async ({ page }) => {
    ...
    const createResponse = await page.request.post(
      `/api/devices/${TEST_QRCODE_ID}/profile`,
      {
  ...
  test('生命周期事件记录功能', async ({ page }) => {
    ...
    for (const event of events) {
      const response = await page.request.post(
        `/api/devices/${TEST_QRCODE_ID}/lifecycle`,
        {
          data: event,
        }
      );
  ...
  test('设备状态自动更新', async ({ page }) => {
    ...
    // 验证维修统计已更新
    const updatedProfileResponse = await page.request.get(
      `/api/devices/${TEST_QRCODE_ID}/profile`
    );
  ...
  test('错误处理和边界情况', async ({ page }) => {
    // 测试不存在的设备
    const nonExistResponse = await page.request.get(
      '/api/devices/nonexistent/profile'
    );
    ...
    // 测试缺少必要参数
    const missingParamResponse = await page.request.post(
      `/api/devices/${TEST_QRCODE_ID}/lifecycle`,
      {
        data: {
          notes: '缺少事件类型',
