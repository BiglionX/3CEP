...
async function testAPIFunctionality() {
  ...
  const apiTests = [
    ...
    },
    {
      ...
      test: async () => {
        return new Promise(resolve => {
          http
            .get(`${baseUrl}/api/devices/test_device_001/lifecycle`, res => {
              let data = '';
              res.on('data', chunk => (data += chunk));
              res.on('end', () => {
                try {
  ...
  for (const test of apiTests) {
    const result = await test.test();
    console.log(
      `   ${result.success ? '✅' : '❌'} ${test.name} (${test.method} ${test.url})`
    );
    ...
    if (result.success) {
      ...
      if (result.data && result.data.data) {
        console.log(
          `      返回数据: ${JSON.stringify(result.data.data).substring(0, 100)}...`
        );
      }
    } else {
  ...
  const pageTests = [
    {
      ...
      test: async () => {
        return new Promise(resolve => {
          http
            .get('http://localhost:3001/scan/test_device_001', res => {
              let data = '';
              res.on('data', chunk => (data += chunk));
              res.on('end', () => {
                resolve({
                  ...
                  statusCode: res.statusCode,
                  hasExpectedContent:
                    data.includes('设备档案') && data.includes('说明书'),
                });
              });
            })
            .on('error', error => {
                  ...
                  resolve({
                    ...
                    missingFields: hasAllFields
                      ...
                      : requiredFields.filter(
                          field => !profile.hasOwnProperty(field)
                        ),
                  });
                } else {
    ...
    },
    {
      ...
      test: async () => {
        return new Promise(resolve => {
          http
            .get(
              'http://localhost:3001/api/devices/test_device_001/lifecycle',
              res => {
                let data = '';
                res.on('data', chunk => (data += chunk));
                res.on('end', () => {
