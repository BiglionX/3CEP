...
async function runIntegrationTest() {
  ...
  try {
    ...
    const ageInYears = (currentDate - manufactureDate) / (1000 * 60 * 60 * 24 * 365);
    
    // 基础价格估算
    let basePrice = 6000; // iPhone 15 Pro预估价格
    ...
    const conditionMultiplier = conditionMultipliers[mockCondition.screen] || 0.9;
    ...
    const brandMultiplier = brandMultipliers[mockDeviceProfile.brandName] || 1.0;
    ...
    const fusedValue = (finalValue * depreciationWeight) + (adjustedMarketPrice * marketWeight);
    
    console.log('   ✅ 市场数据融合逻辑验证成功');
    console.log(`   💰 市场参考价: ¥${marketReferencePrice}`);
    console.log(`   📈 新鲜度系数: ${(freshnessCoefficient * 100).toFixed(1)}%`);
    ...
    // 模拟置信度计算
    const sampleCountScore = mockMarketData.sampleCount >= 20 ? 1.0 : 
                            mockMarketData.sampleCount >= 10 ? 0.8 : 
                            mockMarketData.sampleCount >= 5 ? 0.6 : 0.3;
    
    const freshnessScore = mockMarketData.freshnessScore;
    
    const priceRange = (mockMarketData.maxPrice - mockMarketData.minPrice) / mockMarketData.avgPrice;
    const stabilityScore = priceRange <= 0.2 ? 1.0 : 
                          priceRange <= 0.4 ? 0.8 : 
                          priceRange <= 0.6 ? 0.6 : 0.3;
    
    const overallConfidence = (sampleCountScore * 0.3) + 
                             (freshnessScore * 0.4) + 
                             (stabilityScore * 0.3);
    ...
    const mockApiResponse = {
      ...
      data: {
        ...
        deviceInfo: {
          ...
          manufacturingDate: mockDeviceProfile.manufacturingDate.toISOString().split('T')[0]
        },
    ...
    console.log(`   📊 置信度: ${(mockApiResponse.data.confidence * 100).toFixed(1)}%`);
