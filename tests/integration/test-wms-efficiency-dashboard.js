...
async function testWMSEfficiencyDashboard() {
  ...
  try {
    ...
    const kpiResponse = await fetch('http://localhost:3001/api/wms/dashboard/kpi-definitions')
    ...
    // 测试2: 获取默认看板数据
    console.log('\n📊 测试2: 获取默认看板数据')
    const dashboardResponse = await fetch('http://localhost:3001/api/wms/dashboard/performance')
    const dashboardData = await dashboardResponse.json()
    
    if (dashboardData.success) {
      console.log('✅ 看板数据获取成功')
      console.log(`   - 仓库总数: ${dashboardData.data.summary.totalWarehouses}`)
      console.log(`   - 活跃仓库: ${dashboardData.data.summary.activeWarehouses}`)
      console.log(`   - 平均评分: ${dashboardData.data.summary.avgCompositeScore.toFixed(2)}`)
      console.log(`   - 告警数量: ${dashboardData.data.alerts.length}`)
      console.log(`   - 仓库指标数量: ${dashboardData.data.warehouseMetrics.length}`)
    } else {
    ...
    if (filteredData.success) {
      ...
      console.log(`   - 时间范围: ${filteredData.data.filters.dateRange.startDate} 至 ${filteredData.data.filters.dateRange.endDate}`)
    ...
    // 测试4: POST方式获取数据
    console.log('\n📤 测试4: POST方式获取数据')
    const postResponse = await fetch('http://localhost:3001/api/wms/dashboard/performance', {
      method: 'POST',
      headers: {
    ...
    // 测试5: 按分类获取KPI定义
    console.log('\n🏷️ 测试5: 按分类获取KPI定义')
    const categoryResponse = await fetch('http://localhost:3001/api/wms/dashboard/kpi-definitions?category=efficiency')
    const categoryData = await categoryResponse.json()
    
    if (categoryData.success) {
      console.log('✅ 按分类获取KPI定义成功')
      console.log(`   - 效率类KPI数量: ${Object.keys(categoryData.data.kpiDefinitions).length}`)
    } else {
    ...
    const checks = [
      {
        name: '关键指标展示',
        passed: dashboardData.success && dashboardData.data.warehouseMetrics.length > 0,
        description: '能展示关键指标'
      },
      {
        name: '日期筛选支持',
      ...
      {
        name: '告警功能',
        passed: dashboardData.success && Array.isArray(dashboardData.data.alerts),
