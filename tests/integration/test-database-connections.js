const { initializeDatabaseConnections, checkDatabaseHealth, dbConnectionManager } = require('../src/data-center/core/database-connection');
...
async function testDataBaseConnections() {
  ...
  try {
    ...
    try {
      ...
      console.log(`  lionfix设备表记录数: ${lionfixResult.rows[0].device_count}`);
    } catch (error) {
    ...
    try {
      ...
      console.log(`  fixcycle用户表记录数: ${fixcycleResult.rows[0].user_count}`);
    } catch (error) {
