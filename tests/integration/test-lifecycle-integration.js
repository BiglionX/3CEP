...
async function testEnvironmentSetup() {
  ...
  const tests = [
      ...
      test: async () => {
        try {
          const { RepairOrderService } =
            await import('@/fcx-system/services/repair-order.service');
    ...
    },
    {
      ...
      test: async () => {
        try {
          const { CrowdfundingFcxPaymentService } =
            await import('@/services/crowdfunding/fcx-payment.service');
  ...
  const testCases = [
    {
      ...
      test: async () => {
        ...
        return {
          success: true,
          details:
            '工单服务已修改，completeOrder方法中添加了recordLifecycleEvents调用',
        };
      },
    },
    {
