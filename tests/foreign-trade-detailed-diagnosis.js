...
async function detailedDiagnosis() {
  ...
  try {
    ...
    for (let i = 0; i < statCards.length; i++) {
      ...
      const hasClickHandler = await card.evaluate(el => !!el.onclick || el.classList.contains('cursor-pointer'));
      console.log(`  卡片 ${i + 1}: ${text?.substring(0, 30)}... | 可点击: ${hasClickHandler}`);
    }
    ...
    for (let i = 0; i < buttons.length; i++) {
      ...
      const onclick = await button.evaluate(el => el.onclick?.toString() || '无');
      const className = await button.getAttribute('class');
      
      console.log(`  按钮 ${i + 1}: "${text?.trim()}"`);
      console.log(`    类名: ${className?.substring(0, 50)}...`);
      console.log(`    点击事件: ${onclick.includes('router.push') ? '有路由跳转' : '无路由跳转'}`);
    }
    ...
    for (let i = 0; i < Math.min(orderItems.length, 2); i++) {
      ...
      const onclick = await order.evaluate(el => el.onclick?.toString() || '无');
    ...
    if (orderItems.length > 0) {
      ...
      console.log('跳转成功:', afterClickUrl.includes('/order/') && afterClickUrl !== initialUrl);
    ...
    if (createBtn) {
      ...
      const afterClickUrl = page.url();
      console.log('点击后URL:', afterClickUrl);
      console.log('跳转成功:', afterClickUrl.includes('create') && afterClickUrl !== initialUrl);
