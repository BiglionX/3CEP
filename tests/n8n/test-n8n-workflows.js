...
const testCases = [
  ...
  },
  {
    ...
    webhookPath: '/webhook/b2b-procurement-advanced',
    payload: {
      description: '需要采购以下物品：20台笔记本电脑（ThinkPad系列）、50套办公椅、100个无线鼠标、200个机械键盘，总预算控制在10万元以内，要求两周内交付',
...
const templateTestCases = [
  ...
  },
  {
    ...
    expectedFields: ['status', 'data', 'message'],
    expectedDataFields: ['diagnosis', 'suggested_solutions', 'confidence', 'estimated_time']
  }
];
    ...
    if (result.success) {
      ...
      if (result.parsedItems && result.parsedItems.length > 0) {
        ...
        result.parsedItems.forEach((item, index) => {
          console.log(`     ${index + 1}. ${item.productName} - ${item.quantity}${item.unit || '个'}`);
        });
      }
    } else {
...
async function testTemplateWorkflow(templateTestCase) {
  ...
  try {
    ...
    if (!hasExpectedFields) {
      const missingFields = templateTestCase.expectedFields.filter(field => 
        !result.hasOwnProperty(field)
      );
    ...
    if (result.data && typeof result.data === 'object') {
      ...
      // 验证 data 内部字段
      const hasExpectedDataFields = templateTestCase.expectedDataFields.every(field => 
        result.data.hasOwnProperty(field)
      );
      
      if (!hasExpectedDataFields) {
        const missingDataFields = templateTestCase.expectedDataFields.filter(field => 
          !result.data.hasOwnProperty(field)
        );
...
async function checkApiHealth() {
  ...
  try {
    const response = await fetch(`${BASE_API_URL}/api/b2b-procurement/parse-demand`, {
      method: 'GET'
    });
...
async function runAllTests() {
  ...
  if (!n8nHealthy || !apiHealthy) {
    ...
    if (!apiHealthy) console.log('   - 智能体 API 服务正在运行 (默认端口 3000)');
    return;
  }
  ...
  allResults.forEach((result, index) => {
    ...
    console.log(`${index + 1}. ${testType} ${result.name}: ${status} (${result.duration || 'N/A'}ms)`);
    if (result.success) {
      passedTests++;
  ...
  console.log('═'.repeat(50));
  console.log(`工作流激活: ${workflowStatus.activated}/${workflowStatus.total}`);
  ...
  if (overallPassRate < TEST_DATA_PASS_RATE) {
    console.log(`⚠️  测试通过率 ${overallPassRate}% 低于要求的 ${TEST_DATA_PASS_RATE}%`);
  }
...
async function testSpecificWorkflow(workflowType = 'basic') {
  const testCase = workflowType === 'advanced' 
    ? testCases[1] 
    : testCases[0];
...
async function testSpecificTemplate(templateType = 'quote') {
  const templateTestCase = templateType === 'diagnosis' 
    ? templateTestCases[1] 
    : templateTestCases[0];
...
async function runTemplateTestsOnly() {
  ...
  let passedTests = 0;
  let totalTests = templateResults.length;
  
  templateResults.forEach((result, index) => {
    const status = result.success ? '✅ 通过' : '❌ 失败';
    console.log(`${index + 1}. [模板] ${result.name}: ${status} (${result.duration || 'N/A'}ms)`);
...
if (args.includes('--health')) {
  Promise.all([checkN8nHealth(), checkApiHealth()])
    .then(([n8n, api]) => {
