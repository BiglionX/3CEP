...
SELECT
    ...
    CASE
        ...
        WHEN COALESCE(ps.reltuples, 0) = 0 THEN '空表'
        ELSE '异常'
    END as status
...
SELECT
    ...
    row_count AS "行数",
    CASE WHEN has_primary_key THEN '有' ELSE '无' END AS "主键",
    status AS "评估"
...
SELECT
    ...
    SUM(CASE WHEN status = '正常' THEN 1 ELSE 0 END) AS "正常的表",
    SUM(CASE WHEN status = '空表' THEN 1 ELSE 0 END) AS "空表",
    SUM(CASE WHEN status = '异常' THEN 1 ELSE 0 END) AS "异常的表"
FROM table_check_results;
